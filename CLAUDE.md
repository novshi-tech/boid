...
auto-merge は `task.exit` gate (executing → done に進む直前に host で実行) として実装されている。この gate が PR の mergeable 検証と `gh pr merge` を 1 本にまとめて担う。

conflict 発生時のフロー (`done → executing → done` の reopen ループ):

1. `task.exit` gate が executing 退場時に `gh pr merge` を試行する。conflict で失敗した場合は exit code != 0 で done への遷移をブロック
2. オペレータは `boid task reopen <id> --message "merge origin/main で conflict を解消して再 push"` で done から executing に戻し、新しい instruction を append する
...
5. boid job done で hook が完了 → executing → done が再評価され、`task.exit` gate が再実行されて conflict 解消後の merge が走る
