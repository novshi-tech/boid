...
- **ローカルファイルを書き換える範囲をサンドボックスで絞れる。** エージェントは普段のディレクトリを直接読みますが、書き込みは指定したスコープ (典型的には git worktree) に閉じ込められます。暴走したエージェントもホームディレクトリや他プロジェクトには触れられません
- **複数の依頼を並行して進められる。** タスクごとに git worktree を切ることもでき、それぞれを別ブランチ・別ディレクトリで同時に走らせても作業がぶつかりません
