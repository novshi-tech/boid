...
// workspace_home_import.go is the daemon-side half of the workspace HOME
// migration: replace one workspace's HOME volume with the contents of its
// pre-volume host directory, and guarantee that the next dispatch re-runs
// init.sh over it. See docs/plans/workspace-home-volume-persistence.md
// 論点f for the full design rationale.
//
// The daemon does the engine work; the CLI only reads the legacy host
// directory and streams a tar. The completion marker, the per-workspace
// init flock, and the volume identity label are all daemon-owned state, so
// keeping the mint/stamp/marker-write on the daemon side of the wire is what
// keeps them consistent — a CLI driving the engine directly would race the
// very things this migration has to keep in sync.
...
type WorkspaceHomeImportResult struct {
	...
	// whatever the old one had, which is the re-init "leg 1".
	HomeID string
	...
	// MarkerRemoved / MarkerRemoveError report the re-init "leg 2" (marker
	// removal). A failure here is not fatal (leg 1, the fresh volume
	// identity, has already re-armed the init on its own) but it IS reported,
	// because either leg alone suffices and an operator should be told when
...
//
//   - --no-same-owner: ownership recorded in the archive is discarded and
//     everything lands owned by the uid the container runs as, which is the
//     same uid a job container runs as. This is what makes the tar stream a
//     valid answer to the rootless-podman uid-mapping trap (also GNU tar's
//     default for a non-root extractor, stated anyway so the contract does
//     not depend on which uid the container happened to get).
//   - --same-permissions: without it a non-root GNU tar applies the process
//     umask to every extracted mode. A 0600 credentials file survives a
//     umask by luck (a umask can only clear bits it already has), but a
//     0666 or 0755 entry does not, and "the permissions are mostly right"
//     is not a contract. WITH it, the modes are the archive's, full stop.
...
// tarStream, and guarantees the next dispatch runs its init.sh once. See
// docs/plans/workspace-home-volume-persistence.md 論点f for the full design
// rationale behind the steps below.
//
// The completion marker's skip condition never depends on volume contents,
// so a migration that only copies bytes would leave a settled workspace
// skipping init forever, keeping its host-era toolchain and paths. Guarding
// against that takes two independent mechanisms (either one alone is
// enough to force a re-init), applied in an order chosen so each step can
// refuse before anything destructive happens:
//
//  1. Remove the volume — also the in-use check (a running container makes
//     this fail with ErrWorkspaceHomeInUse before anything is destroyed).
//  2. Delete the completion marker (leg 2). Not fatal if it fails.
//  3. Create the volume again, with a fresh identity (leg 1).
//  4. Extract the tar into it.
//
// Once step 1 succeeds, every later outcome re-initializes: either the
// marker is gone, or it survives pointing at an identity that no longer
// exists.
//
// Steps 0, 1.5, 5 and 6 (numbered where they occur in the body) are the
// crash-recovery bookkeeping around this: a durable in-progress record is
// written before anything is destroyed and cleared once the new home is
// complete, so a daemon that is killed mid-migration leaves a record
// Runner.SweepInterruptedWorkspaceHomeMigrations can finish at the next
// daemon start rather than a silently half-extracted home. See
// workspace_home_migration_sentinel.go for what each phase means.
//
// Once the old volume is removed, the migration is not reversible — the
// source directory is read-only and untouched, which is why a failure past
// step 1 leaves the workspace's home empty (never partial) rather than
// restoring anything: the next dispatch prepares it from scratch exactly
// like a brand-new workspace, and re-running the migration is safe.
//
// r.homeInFlight (registered at the top of this function) excludes both a
// concurrent dispatch and a concurrent `boid workspace remove` of the same
// workspace: resolveWorkspaceHome's fast path holds no lock between
// resolving a volume and asking the engine to mount it, so without this
// registry a migration landing in that gap could have a dispatch's job and
// this migration's tar extraction writing into one volume at once. See
// workspaceHomeInFlight for why the registry is in-memory and why a
// dispatch waits while a migration or removal refuses.
func (r *Runner) ImportWorkspaceHome(ctx context.Context, workspaceID string, tarStream io.Reader) (WorkspaceHomeImportResult, error) {
	...
	// Registered before anything else, including the capability probes and
	// the meta dir: this is the one refusal that has to be able to happen
	// while the workspace is still untouched. See workspaceHomeInFlight.
	...
	// Immediately after the registration: from here a `boid workspace remove`
	// of this workspace is refused, so this answer cannot go stale before the
	// volume exists. See Runner.ConfirmWorkspaceExists.
	//
	...
	// mkdirAllDurable, not os.MkdirAll: the record written at step 0 is only
	// as durable as the directory holding it, and this is the one caller that
	// destroys something on the strength of that record. See mkdirAllDurable
	// for what it flushes.
	...
	// The last refusal that costs nothing, checked before the flock: an
	// archive with no members would extract perfectly into the volume this
	// function is about to destroy and re-create, and report success over a
	// home that now holds nothing. Checked above the lock so waiting for the
	// first block of a network stream does not hold a file lock a dispatch's
	// init could be waiting on.
	//
	...
	//
	// It goes in as phase "recorded" — the migration has begun and nothing
	// has been destroyed. That is not a formality: this exact record is what
	// survives when the removal at step 1 is refused and the cleanup unlink
	// then fails, and a recovery that read it as "the home was destroyed"
	// would delete a home no part of this function ever touched.
	//
	// It fails the migration rather than proceeding without a durable
	// record: the record's only value is that it outlives the process, and
	// destroying the home behind a sentinel that may not survive a power cut
	// is the exact state the sweep exists to make impossible. Stopping here
	// is the cheapest possible failure — nothing has been destroyed, so the
	...
	if err != nil {
		...
		// first — so the record is removed with the refusal (tidiness, not
		// safety: the record is still in phase "recorded", so a cleanup that
		// fails here just leaves a file the next recovery deletes; see
		// workspace_home_migration_sentinel.go's route B). The one case that
		// is not literally "nothing destroyed" is an engine that removed the
		// volume and then failed to say so; leaving the record in "recorded"
		// is still right there, because the next dispatch's own VolumeCreate
		// mints a fresh identity that the surviving completion marker no
		// longer matches, forcing init to re-run anyway.
		discardWorkspaceHomeMigrationRecord(recordPath, slug)
	...
	//
	// The removal has succeeded, so from here until step 4 returns, whatever
	// appears under this volume name was put there by a migration that may
	// not finish. This is the transition that authorizes recovery to throw
	// it away, and it is committed before the replacement volume is
	// created — which keeps the recorded destructive interval strictly
	// wider than the interval in which such a volume exists.
	//
	// Fail-closed, exactly like step 0: proceeding to extract behind a
	// window that may not survive a power cut would leave a partial home
	// under a record saying nothing was destroyed. Stopping here is safe
	// because the volume is gone rather than partial, so the next dispatch
	// builds it from empty.
	rec.Phase = workspaceHomeMigrationHomeDestroyed
	windowErr := writeWorkspaceHomeMigrationRecord(recordPath, rec)

	// --- step 2: re-init leg 2 -----------------------------------------------
	//
	...
	if windowErr != nil {
		// The volume is gone — step 1 removed it and step 3 never ran — so
		// this record has nothing left to authorize and must stop claiming
		// otherwise before it is unlinked: writeWorkspaceHomeJSONFile fsyncs
		// the directory after the rename, so the commonest way that write
		// fails is with "home-destroyed" already in place and merely
		// unflushed, not genuinely absent.
	...
	// --- step 3: re-init leg 1 — a NEW incarnation with a NEW identity ------
	//
	...
	if err := importer.ImportWorkspaceHome(ctx, WorkspaceHomeImportRequest{
		...
		// --- step 4b: make the failure state real --------------------------
		//
		...
		if discarded {
			// The wreckage is gone, so the window closes here, before the
			// unlink — the phase is only moved on the branch where the
	...
	//
	// The home is a home again, so this record must stop authorizing anybody
	// to destroy it — and that has to be true before the unlink rather than
	// as a consequence of it, because the unlink can itself fail (see
	// workspace_home_migration_sentinel.go). Run after the extraction rather
	// than before it, so no crash inside steps 1-4 can be mistaken for a
	// completed migration.
	//
	// Not fatal, unlike the two writes before it: the migration has
	// succeeded and the operator's home is complete and correct, so failing
	// this would report a disaster for a workspace that is fine. What a
	// failure costs is only what the leftover record then costs, reported
	// by step 6.
	windowClosed := true
...
// "home-absent" phase durably first, and only then unlinks.
//
...
//
// # Why the daemon checks this at all
//
