...
// happy path — same content/mode contract as PublishIfAbsent's own first
// test above, just via the unconditional-overwrite primitive.
func TestWriteAtomic_CreatesFileWithContentAndMode(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "ca.pem")

	if err := atomicfile.WriteAtomic(path, 0o644, []byte("cert-v1")); err != nil {
	...
	if string(data) != "cert-v1" {
		t.Errorf("content = %q, want %q", data, "cert-v1")
	}
	info, err := os.Stat(path)
	if err != nil {
		t.Fatalf("stat: %v", err)
...
// TestWriteAtomic_SecondCallOverwrites pins the actual behavioral
// difference from PublishIfAbsent: a second WriteAtomic call against an
// existing file REPLACES its content — the contract cmd/start.go's daemon-
// ca.pem bootstrap file needs (every boot re-derives and rewrites it,
// unlike a publish-once secret).
func TestWriteAtomic_SecondCallOverwrites(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "ca.pem")
	...
	if err := atomicfile.WriteAtomic(path, 0o644, []byte("cert-v2")); err != nil {
	...
	if string(data) != "cert-v2" {
		t.Errorf("content = %q, want %q (second write must overwrite the first)", data, "cert-v2")
	}
}

// TestWriteAtomic_NoTempLeftover mirrors PublishIfAbsent's own leftover
// check: the temp staging file must never survive a successful call.
