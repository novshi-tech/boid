...
// entries — internal/reap's own drain step uses this to remove entries it
// has already confirmed destroyed, so a subsequent `boid reap` run does not
// re-attempt (and re-report as an error) an id that is already gone. Writes
// to a temp file in the same directory, fsyncs it, then os.Rename onto
// path: any reader always observes either the pre-rewrite or post-rewrite
// content in full, never a partial write.
//
// A package-level function, not a *Ledger method, deliberately: reap's
// caller constructs a fresh, unshared Ledger per read, so there is no
// in-memory cache to keep in sync. entries may be empty (path ends up
// holding zero lines — still a valid, empty ledger).
