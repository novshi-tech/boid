...
func (h *JobLogSSEHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || ch == nil {
		if !finished {
			...
			// offer right now — same underlying condition as
			// WSAttachHandler's own !finished branch, see its doc comment
			// for the full rationale. A bare return here would silently end
			// the SSE stream with no signal at all — the Web UI's
			// EventSource just stops receiving further "message" events and
			// never learns whether the job actually finished or the stream
			// merely dropped. A distinctly-named SSE event (not the default
			// "message" event sendLines' own `data:` lines use, so this
			// never gets mistaken for ordinary log output by the existing
			// job-log EventSource listener) lets the client tell the two
			// apart — web/templates/jobs.templ's own
			// `boid-stream-unavailable` listener renders a notice instead
