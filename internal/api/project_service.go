...
type ProjectAppService struct {
	...
	Meta     interface {
		...
		// git repository's HEAD — the bare-repo counterpart to Load, used by
		...
		// used by FetchProject (`boid project fetch <id>`, reload after
		// `git fetch --all`) — the one LoadBareRepo caller that already has
		// a specific id to check the freshly-read meta against. See
		// orchestrator.ProjectStore.LoadBareRepoExpectingID's doc comment
		// for the cache-corruption bug this closes: the meta is only
		// committed to the shared cache once expectedID is confirmed to
		...
		// Status / MarkDegraded expose the daemon's in-memory project health
		// tracking, so a failed clone/fetch/load never causes the DB row
		// itself to be deleted. hydrateProject reads Status to populate
		// Project.Status/StatusMessage on every read path;
		// CreateProjectFromGitURL/FetchProject call MarkDegraded when a
		// clone/fetch/load step fails.
		...
		// project.yaml) for a project.yaml-less project — used by
		// CreateProjectFromGitURL (initial registration,
		// GitHeadReadFailurePathAbsent) and FetchProject (post-fetch reload,
		// same failure kind) in place of LoadBareRepo /
		// LoadBareRepoExpectingID. Also clears any prior degraded status —
		// see orchestrator.ProjectStore.SetSynthesizedMeta's own doc comment.
		SetSynthesizedMeta(id string, meta *orchestrator.ProjectMeta)
		// Explain returns id's field-provenance view — see
	...
	// DataDir is the daemon's data-home
	// ("<data_dir>/repos/<workspace-slug>/<project-name>.git/"), wired in
	// internal/server/wire.go to dataHomeFor(cfg) — the same value
	...
	// rejecting an unknown name with 400 (an unresolvable reference must
	// not be persisted silently and only warned-about + skipped at dispatch
	// time). Wired in internal/server/wire.go to Server.HostCommands. nil
	// skips the validation entirely — tests that do not exercise it can
	// leave this unset, same convention as CaptureUpstreamURL/Hydrator above.
	HostCommands func() map[string]orchestrator.HostCommandSpec
	// WorkspacesConn is the daemon's single *sql.DB handle. ApplyWorkspace
	...
	// DISPATCHER reads it from. Wired in
	...
	// this type. Wiring a second copy onto WorkspaceHandler would put two
	// answers to "where does this daemon keep init.sh" in the wiring graph.
	InitScripts WorkspaceInitScriptStore
	// mu serializes every workspace-mutating entry point — CreateWorkspace,
	// UpdateWorkspace, RemoveWorkspace, SetProjectWorkspace, ReloadProjects,
	// and CreateProject's default-workspace-assign step — against each
	// other, protecting the in-memory workspace-assignment cache (s.Meta)
	// from being clobbered by a stale snapshot landing after a fresher
	// write. The DB-level assign+existence-check race is closed separately
	// by Projects.AssignWorkspaceIfExists running as its own transaction;
	// this mutex's job is only the Go-side cache. UpdateWorkspace in
	// particular needs the whole-body lock because its force=true path is
	// not a single atomic statement (Load then, separately, Save) — a
	// concurrent RemoveWorkspace landing in that window would otherwise go
	// unnoticed and get silently resurrected by the subsequent Save. None
	// of these methods call each other, so nesting the same non-reentrant
	// mutex across all of them carries no deadlock risk.
	mu sync.Mutex
	// projectMu serializes the whole create-or-delete pipeline of
	// CreateProject, CreateProjectFromGitURL, DeleteProject, and
	// FetchProject against each other, deliberately SEPARATE from mu above:
	// mu protects only the in-memory workspace-assignment cache across a
	// handful of quick, DB-only operations, whereas
	...
	// mutation daemon-wide for the duration of a slow clone.
	//
	// Without some lock across this whole pipeline, two concurrent
	// registrations whose project.yaml `id:` values happen to collide can
	// both call s.Meta.LoadBareRepo (which caches by meta.ID, not by
	// bareRepoPath) then both call s.Projects.CreateProject; the DB's
	// primary key makes exactly one insert win, but the losing call's
	// rollback unconditionally executes s.Meta.Remove(meta.ID), deleting
	// the winning call's cache entry too. The winner is then left with a
	// valid DB row but no cached Meta until the next `boid project
	// reload`. projectMu makes this impossible by construction: only one
	// create-or-delete body executes at a time.
	//
	// FetchProject also holds this lock for its whole get -> git-fetch ->
	// reload -> cache-commit sequence, closing a "half-old/half-new
	// Project object" race against a concurrent DeleteProject +
	// re-registration at the same WorkDir.
	//
...
// ProjectStore health snapshot. Every read path (hydrateProject and
// hydrateProjectWithWorkspace's success
...
func (s *ProjectAppService) CreateProject(workDir string) (*orchestrator.Project, error) {
	...
	// normalized). A project with no remote cannot be registered. Reject
	// with a clear remediation message rather than silently registering a
	// project that will need a remote before it can ever dispatch.
	...
	//
	// This cache write races the same way every other workspace-cache
	// writer does — see the mu field's doc comment — so it must run under
	// the same mutex as
...
// Unlike CreateProject (the legacy work_dir-based endpoint — `boid project
// init` no longer calls it at all; it only scaffolds locally and prints
...
//
// Failure means the register itself fails, no half-added project: clone,
// project.yaml load, and DB/workspace-assignment all happen synchronously
// in this one call, and any failure rolls back everything this call
// itself created (the bare repo directory, the in-memory Meta cache
// entry, the DB row) so a failed `project add` never leaves a
// half-registered project behind. This is a different case from the
// on-startup-auto-prune-retirement invariant that governs
// already-registered projects going degraded later (FetchProject, daemon
// startup) — a project that never successfully finished registering has
// no DB row to preserve in the first place.
func (s *ProjectAppService) CreateProjectFromGitURL(ctx context.Context, gitURL, workspaceSlug, nameOverride string) (*orchestrator.Project, error) {
	...
	if normErr != nil {
		// The raw gitURL argument is exactly what a caller submitting
		...
		// via %q here, unconditionally, would put the credential right back
		// into this 400's message (and from there, the CLI/API caller's
		// terminal output and any log that captures it).
		// SanitizeURLForLogging redacts every recognized credential shape
	...
	// Refuse before any write (a refuse must land BEFORE any DB/filesystem
	// mutation, never after a partial write) when the
	...
	// SafeBareRepoPath validates name as a
	...
	// Serialize the whole clone -> load -> DB-insert -> workspace-assign
	// pipeline below — see the projectMu field's doc comment for the
	// concrete cache-corruption race this closes. Held for the rest of
	// this method (through every return path).
	s.projectMu.Lock()
	defer s.projectMu.Unlock()
	...
	// fallbacks below fired — the wholly-missing-project.yaml path or the
	// present-but-id-less-project.yaml path. In both cases its cache write
	// is deliberately deferred until AFTER CreateProject succeeds (see the
	// call site below), unlike the ordinary LoadBareRepo success path
	...
	// overwrite an ALREADY-REGISTERED project's cache entry the moment
	// this call's derived id collides with one:
	// removeCachedMetaUnlessRegistered's failure-path check only stops the
	// entry from being REMOVED afterward, it does not restore whatever
	// this call clobbered on the way in, and SetSynthesizedMeta
	// additionally clears any degraded status that established project
	...
	// This cache write races every other workspace-cache writer the same
	// way — see the mu field's doc comment. projectMu (held for this whole
	...
	//
	// AssignWorkspaceIfExists, not the plain SetProjectWorkspace this used
	// to call — workspaceSlug's
	...
	if err := s.Projects.AssignWorkspaceIfExists(project.ID, workspaceSlug); err != nil {
		...
		//
		// The bare repo is removed BEFORE the DB row and Meta cache entry,
		// not after — mirroring DeleteProject's own fix for the exact same
		// reason: deleting the DB row first and only logging an
		// os.RemoveAll failure as a warning would orphan a permission- or
		// I/O-failing bare repo directory on disk with no row left to
		// retry cleanup through, and re-adding the same URL/name would then
		// 409 forever (this method's own pre-flight "already registered at
		// this path" check). Removing the bare repo first means a removal
		// failure now fails the WHOLE call with the DB row (and cached
		// Meta) intact — the operator fixes the underlying issue and
		// retries `boid project rm` to finish cleanup.
		if rmErr := os.RemoveAll(bareRepoPath); rmErr != nil {
		...
		// remove too. If DeleteProject itself fails here (e.g. the DB is
		// unreachable), the row survives
...
// project.yaml (`boid project fetch <id>`). Unlike CreateProjectFromGitURL,
// a failure here does NOT roll anything back or delete the project — id is
// already a registered project the on-startup-auto-prune-retirement
// invariant protects; a fetch or reload failure marks it StatusDegraded
// (via s.Meta.MarkDegraded) and returns an error, leaving the DB row and
// any previously-loaded Meta exactly as they were.
func (s *ProjectAppService) FetchProject(ctx context.Context, id string) (*orchestrator.Project, error) {
	// Serialize this whole get -> filesystem-fetch -> reload ->
	// cache-commit sequence against CreateProject/CreateProjectFromGitURL/
	// DeleteProject's own critical sections — see projectMu's doc comment:
	// without it, FetchProject could read id's DB row here, then have a
	// concurrent DeleteProject + re-registration at the SAME WorkDir land
	// before this method's own fetch/reload resumed — producing a
	// returned Project that mixes id's OLD DB fields (read before the
	// race) with the NEW bare repo's metadata (loaded after it).
	// FetchProject never calls any of
	...
	// Classify legacy-vs-managed by whether WorkDir falls under this
	// daemon's own bare-repo storage root
	...
	// LoadBareRepoExpectingID, not the
	...
	if err != nil {
		// A project.yaml that was never committed (GitHeadReadFailurePathAbsent)
		...
		// behavior unchanged.
		//
		// Gated on id being VERIFIED url-derived — see that constant's own
		// doc comment: without this gate, an ORDINARY project.yaml-bearing
		// project whose file was deleted upstream by mistake would also
		// match GitHeadReadFailurePathAbsent and get silently switched to
		// the workspace default with no degraded signal, instead of
		// degrading as it always has. Checking id's PREFIX alone is not
		// enough either: a hand-authored `id: url-custom` that was
		// committed at some point, then had its project.yaml deleted
		// upstream, would also match a bare prefix check — confirm id
		// actually equals deriveProjectIDFromURL(project.UpstreamURL), the
		// same verification orchestrator.ProjectStore applies for a
	...
	// Refuse when project.yaml's own
	...
	if meta.ID != id {
		...
		s.Meta.MarkDegraded(id, msg)
		// LoadBareRepoExpectingID never cached this mismatched meta under
		// its own (drifted) id in the first place — so there is no phantom
		// cache entry to remove here, and no risk of clobbering an
...
func (s *ProjectAppService) SetProjectWorkspace(id, workspaceID string) (*orchestrator.Project, error) {
	...
	// Reject assignment to a slug with no corresponding workspaces row.
	// Without this, assigning a project to a nonexistent slug (typo, or a
	// workspace removed out from under it) would silently leave a
	// dangling project_workspaces reference: dispatch then runs in a
	// permanently degraded window (GetWithWorkspace logs "workspace.yaml
	// not found" on every call), and restarting the daemon never
	// self-heals it. Running the existence check (WorkspaceExists) and
	// the assign (SetProjectWorkspace) as two separate statements would
	// leave a window where a concurrent DELETE could land in between and
	// still produce the same dangling reference. AssignWorkspaceIfExists
	// folds both into a single DB transaction so that window does not
	// exist. DefaultWorkspaceSlug (and "" — clearing) are exempt from the
	// existence check inside AssignWorkspaceIfExists itself.
	//
	// `boid workspace assign` auto-creates a DB row from the legacy yaml
	// (if present) before calling this endpoint, so a "drop a yaml, then
	// assign" flow keeps working (cmd/workspace.go).
...
// CreateWorkspace inserts a brand-new workspace at slug (POST
...
func (s *ProjectAppService) CreateWorkspace(slug string, meta *orchestrator.WorkspaceMeta) (*WorkspaceDetail, error) {
	...
	// Serialize this whole create against
	...
	// Validate every meta.HostCommands reference against the daemon's live
	// aggregated snapshot. See the HostCommands field's doc comment for
	// why an unknown name must be rejected here rather than silently
	// persisted.
	//
	// WorkspaceMeta.Kits no longer exists and workspaceMetaStrict no
	// longer accepts a `kits:` key at all, so a caller that still sends
	// `kits:` gets a 400 straight out of the strict decoder, before this
	// method ever runs. The one remaining legacy-kits caller
	// (cmd/workspace.go's ensureWorkspaceExistsForAssign, `boid workspace
	// assign`'s auto-create convenience path) resolves it client-side
	// before submitting an already-materialized body.
...
// (Workspaces.LoadWithRevision) rather than two separate queries (a plain
// Load for meta, then GetWorkspaceSummary for revision), so this never
// straddles a concurrent PUT and returns a meta/revision pair that never
// coexisted in the DB.
...
// /api/workspaces/{slug}), enforcing optimistic concurrency: ifMatch must
// equal the workspace's current revision unless force is true. A missing
// ifMatch (and !force) is rejected with 428 Precondition Required rather
// than silently proceeding, since accepting an unconditional PUT by
// default would make the ETag check opt-in instead of the intended opt-out
// (--force).
//
// The non-force path no longer reads the current
...
func (s *ProjectAppService) UpdateWorkspace(slug string, meta *orchestrator.WorkspaceMeta, ifMatch string, force bool) (*WorkspaceDetail, error) {
	...
	// Serialize the whole update — including
	...
	// Same validation as CreateWorkspace — see that call site's comment.
	if err := s.validateHostCommandRefs(meta.HostCommands); err != nil {
		return nil, err
	}
...
// not exist in the daemon's live aggregated host_commands snapshot. No-op
// (nil) when s.HostCommands is unset (test opt-out, same convention as
// CaptureUpstreamURL/Hydrator) or when refs is empty.
...
// RemoveWorkspace deletes slug (DELETE /api/workspaces/{slug}). The
// reserved default workspace cannot be removed; any project still
// assigned to slug is re-pointed at default by
// orchestrator.WorkspaceRepository.Remove's transaction, so this never
// leaves a dangling project_workspaces reference at the DB layer.
//
...
// assignment.
//
// The whole snapshot -> DB remove -> cache-sync
...
//
// The script's deletion started life as a second
...
// instead and skip this redundant round trip — see those methods' own
// doc comments for why avoiding it matters.
...
// doc comment). Neither runs conditionally on dryRun: a dry run must hit
// the exact same validation a real apply would, not silently skip it and
// report success on a document real apply would go on to reject.
func (s *ProjectAppService) ApplyWorkspace(apply *orchestrator.WorkspaceEnvelopeApply, dryRun bool) (*orchestrator.WorkspaceApplyResult, error) {
	...
	// after the commit — see validateWorkspaceApplyInitScript for why the
	...
	if apply.FieldsPresent["projects"] {
		...
		if resolveErr != nil {
			// Refuse the WHOLE apply before any DB write when a name is
			// ambiguous — this return happens before
			// s.mu.Lock()/ApplyWorkspaceEnvelope below, so nothing has been
			// touched yet (no side effects to unwind).
	...
	// spec.init_script, LAST and outside the transaction: it is a file on
...
// id-or-name-or-substring resolution (see resolveProjectByNameExact's own
// doc comment for the concrete destructive scenario ID-priority matching
// would create here). A name that resolves to NO project is reported in
// missing rather than failing the whole apply: a not-yet-registered
// project must never fail apply.
//
// A name that resolves to MORE THAN ONE project is a different case
// entirely and is NOT folded into missing: two registered projects
// sharing a project.yaml name (e.g. both "api") would otherwise make
// every lookup for that name ambiguous, and folding that into "not
// registered" would empty the target set and destructively detach BOTH
// projects to the default workspace, even though both were explicitly
// listed in the applied document. Instead, an ambiguous name aborts
// resolveWorkspaceApplyProjectNames immediately with a StatusError naming
// every colliding project ID, and the caller (ApplyWorkspace) must refuse
// the entire apply before touching the DB rather than proceed with a
// wrong/partial resolution.
//
// Every check and lookup below reads from ONE snapshotRegisteredProjects()
// call, captured up front, rather than one read for
// refuseIfAnyRegisteredProjectNameUnavailable's availability check and a
// separate one per spec.projects[] entry inside resolveProjectByNameExact:
// two independently timed reads of live, mutable state (s.Meta's
// in-memory cache) could let a concurrent ReloadProjects remove a
// project's cached Meta.Name AFTER the availability check already
// verified every name was available, and the stale per-name lookup would
// then report that project "missing" — the exact "cannot verify" ->
// "does not exist" mistranslation this guards against. A single
// up-front snapshot, reused for both the check and every lookup, makes
...
func (s *ProjectAppService) resolveWorkspaceApplyProjectNames(projects []orchestrator.WorkspaceEnvelopeProject) (ids map[string]string, missing []string, err error) {
	...
	// Refuse the whole apply up front when the
	...
	// SAME snapshot the per-name lookups below use (see this
	...
	for _, ep := range projects {
		// Defense in depth: decodeWorkspaceEnvelopeSpec
...
// resolveWorkspaceApplyProjectNames performs. A repository failure here
// (e.g. a transient 500) aborts the whole apply before any DB write: any
// resolution failure that is not a genuine 404 must abort, never fold
// into missing — there is no per-name repository call left to fail
// separately.
...
// untouched. snapshot is the caller's single snapshotRegisteredProjects()
// read — this function performs no lookup of its own.
//
...
// behavior. snapshot is the caller's single snapshotRegisteredProjects()
// read — this function performs no lookup of its own, so calling it once
// per spec.projects[] entry costs no extra DB/cache round trip and, more
// importantly, cannot observe a different picture than a sibling call made
// moments earlier against the same snapshot.
//
// spec.projects[].name is defined by the Workspace envelope schema as a
// project.yaml `name:` value, never a project ID — resolving it through
// ResolveProjectRef's flexible id-or-name convenience matching (built for
// CLI/API ref arguments, where a user reasonably wants to type either)
// would let a project whose NAME happens to equal a DIFFERENT project's
// ID hijack that other project's identity via ResolveProjectRef's
// id-match-first precedence. Concrete failure: project A (id=proj-a,
// name=proj-b) and project B (id=proj-b) both registered; a document
// listing `name: proj-b` for A would instead resolve to B via
// ResolveProjectRef's ID-exact-match step (which runs BEFORE any name
// check), attaching B and destructively detaching A — the exact opposite
// of what the document specified. Substring matching is excluded too, not
// just ID priority: apply must be deterministic from the document alone,
// never fuzzy CLI convenience matching some unrelated project by partial
// name.
//
// Returns nil when no project in snapshot has this exact name — the
// caller folds that into "missing"; any OTHER resolution failure must
// abort the whole apply instead — there is no per-name repository call
// here that could fail that way, so the only failure mode left is
// snapshotRegisteredProjects' own, handled once by the caller before this
...
// atomic DB snapshot (GET /api/workspaces/export?all=true|?name=<slug>).
// slugs nil/empty exports every workspace.
func (s *ProjectAppService) ExportWorkspaceEnvelopes(slugs []string) ([]byte, error) {
	...
	// Detect (a) a project.yaml name shared by two or more registered
	// projects, and (b) a project.yaml name that collides with a DIFFERENT
	// registered project's ID (see resolveProjectByNameExact's doc comment
	// for the concrete destructive scenario), BEFORE emitting a
	// spec.projects[] entry a later `apply` could not unambiguously (or
	// correctly) resolve back. allProjects comes from the SAME transaction
	// snapshots did (SnapshotWorkspacesForExport's own doc comment), not a
	// separate ListProjects() call, which could otherwise describe a
	// different instant than the export snapshot under concurrent project
	...
	// hydration pass: the emit loop below must not call s.hydrateProject a
	// second time per project, which re-reads the live in-memory
	// ProjectMeta cache — a concurrent metadata reload between this
	// collision-detection pass and the emit loop could then make the two
	// passes observe different Meta.Name values for the same project (the
	// collision map built from one snapshot, the emitted name from
	// another). Reusing effectiveNames in the loop closes that window:
	...
	for _, snap := range snapshots {
		...
		for _, id := range snap.ProjectIDs {
			// p comes from the SAME transaction snap.ProjectIDs did
			// (WorkspaceExportSnapshot.Projects's own doc comment) — not a
			// separate, post-transaction GetProject call, which could drop
			// an assignment from an otherwise-"consistent" snapshot (e.g. a
			// concurrent delete landing in that window). A missing entry
			// here would mean that guarantee itself broke, so fail loudly
			...
			// cached Meta.Name empty). A basename fallback would make the
			...
			// the same export-succeeds/apply-fails round-trip gap this
			// closes for projects that predate the name-is-required fix.
		...
		// The workspace's init.sh. Read here
		...
		// the workspace it describes. That is the
...
// collision across allProjects: every entry is hydrated with Meta from
// the in-memory ProjectMeta cache (a project.yaml name is not a DB
// column), but the SET of projects being compared comes entirely from
// allProjects — the caller's tx-snapshotted SnapshotWorkspacesForExport
// result, not a separate ListProjects() call, which could describe a
// different instant than the export snapshot under concurrent project
// create/delete.
//
// byName maps a project.yaml name shared by two or more registered
// projects to every colliding project ID.
//
...
// equals that name — see resolveProjectByNameExact's doc comment for the
// concrete destructive apply-side scenario this closes on the export side.
//
...
// HERE, at this function's single hydration pass, so a caller can reuse
// the exact same observed name later without a second,
// independently-timed s.hydrateProject call: re-hydrating during emit
// could observe a different in-memory ProjectMeta cache state than
// collision detection did, under a concurrent metadata reload.
...
// project, the daemon-managed bare repository backing it. Leaving the bare
// repo directory behind on disk would make a plain `boid project rm`
// followed by re-adding the SAME URL/name always 409 (via
// CreateProjectFromGitURL's pre-flight "already registered at this path"
// check), defeating the documented degraded-recovery procedure ("rm the
// project, re-add it").
//
...
// isManagedBareRepoPath classification FetchProject uses).
//
// The bare repo is removed BEFORE the
// DB row, not after: deleting the DB row first and only logging an
// os.RemoveAll failure as a warning would leave a read-only or otherwise
// permission-failing repo directory on disk forever (nothing left to
// retry through, since the row that named it is already gone) while
// `boid project rm` still reports SUCCESS, and a subsequent
// `boid project add` of the same URL/name would still 409 with no remaining
// row through which cleanup could be retried. Removing the repo first
// means a removal failure now fails the WHOLE call with the DB row (and
// cached Meta) intact — the same "fix the issue, retry `boid project rm`"
// recovery path this restores for the 409 case above.
...
func (s *ProjectAppService) ReloadProjects() (*ProjectReloadResult, error) {
	// The ListProjects snapshot and the LoadAll cache write it feeds
	...
	// Re-capture upstream_url for every project on reload. This both keeps
	// upstream_url in sync with a project's
