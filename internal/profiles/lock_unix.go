...
// GOOS=windows (docs/plans/windows-client-build.md).
//
// Byte-for-byte the behaviour LockConfigMutation had inline before the
// split: LOCK_EX blocks until the holder releases, and release is
// best-effort (the fd is closed right after, which drops the lock anyway).
