...
// command invocation, with a remediation (`chmod 600`) that does not exist
// on the platform — an unactionable, unsilenceable warning trains the
// reader to skip warnings, including the ones that matter.
//
...
// DACL granting only the current user's SID at create time; that is not
// done yet.
