...
description: Runs a supervisor task (readonly orchestrator) for the boid orchestrator.
  ...
  builtin, and monitors them until completion. Spawns additional tasks based on
  child state and notifies the user when needed.
---

# boid Supervisor

A supervisor task **triages** a request, creates child executor tasks with the appropriate behavior, and **monitors them** until completion. The supervisor itself is **readonly** — it can read the working tree and run `git` queries, but it does not edit project files. Implementation always happens in child executor tasks.

This is the canonical behavior counterpart to the previous `/boid-plan` skill. The two skills share the same protocol; `boid-plan` is kept around only for backward compatibility with older `task_behaviors` configurations.

## Default Workflow: Local Merge Model

The default supervisor workflow assumes a **local merge model**: child executor tasks commit their changes on their own worktree branch and exit. The supervisor then fast-forwards / merges those branches into the base branch **locally**, and pushes when the whole supervise loop is done. No PR is required for the default case.

```
[supervisor]                [executor child]
   create child --auto_start ─────────►
                                          implement
                                          git commit
                                          (optional) git push
                             ◄───── done
   git fetch / merge / verify
   create next child or exit
```

Projects that need a PR-per-child model (e.g. an OSS project that wants every change reviewed) can override the executor's `default_instruction.message` to call a project-side release skill such as `/dev-pr-flow`. The boid repo itself does this — its `.boid/project.yaml` ships a "1 executor 1 PR" override. The skill default is local merge because it is the lowest-friction model for personal / closed projects.
...
5. **Integrate (local merge model)**: As each child reaches `done`, fast-forward / merge its branch into the base branch locally, verify, and continue.
6. **Re-plan (if needed)**: Based on child results, create additional or revised tasks. Consult the user via `boid task notify --ask` if stuck.
7. **Exit**: Once all children reach `done` / `aborted`, exit autonomously or via an exit-confirmation ask (see "Exit Handling").
...
Available behaviors are defined in the **`task_behaviors` section of `.boid/project.yaml`**. Read it directly from the sandbox, examine each behavior's `default_instruction.message` (what it does) and settings like `model`, and choose the one that fits the task. SKILL.md does not carry project-specific behavior names.

Canonical behavior names are **`supervisor`** (readonly orchestrator — this skill) and **`executor`** (writable implementation — see `/boid-executor`). `readonly` / `worktree` are no longer behavior-level settings — they are derived from the canonical name automatically.

Omitting `behavior` in `boid task create` routes to **`supervisor` by default**. Use this when you want to re-delegate as a sub-supervisor (i.e., have the child do its own triage + monitoring).
...
| `base_branch` | Branch to fork the worktree from. Inherits from project-top `base_branch` if omitted; falls back to the daemon's current HEAD branch |
...
Settings like model are governed by the behavior template (`task_behaviors` in project.yaml). `readonly` / `worktree` are derived from the canonical behavior name. User consultation always goes through `boid task notify --ask` regardless of how the agent is launched, so there is no need to split "interactive supervisor" and "autonomous supervisor" into separate behaviors.
...
## Integration: Local Merge Model (default)

In the default local merge model, the supervisor owns the integration step. When a child executor reaches `done`:

1. Find the child's worktree branch. The default branch pattern is `boid/<task_id8>` (first 8 chars of the child task id), but consult `task_behaviors` / project conventions if your kit configures a different prefix.
2. Fetch and merge locally into the base branch:

   ```bash
   git fetch --all --prune
   git checkout <base_branch>       # usually the supervisor's base branch
   git pull --ff-only origin <base_branch>
   git merge --ff-only boid/<child_id8>
   ```

3. Verify (build / test as appropriate for the project).
4. Optionally push immediately, or batch pushes until the whole supervise loop completes (one push per supervisor run is typical: "1 supervisor 1 push").
5. Move on to the next child.

If a merge cannot fast-forward (the child branched from an older base), choose one:

- **Reopen the child to rebase**: `boid task reopen <child_id> -m "rebase on latest <base_branch> and re-verify"`
- **Resolve locally as supervisor** (when the conflict is trivial and clearly the supervisor's call), using `git merge` with manual conflict resolution
- **Escalate via `boid task notify --ask`** if the integration decision is ambiguous

## Integration: PR Model (override)

Projects that prefer a PR-per-child model override the executor's `default_instruction.message` to call a project-side release skill (e.g. `/dev-pr-flow`) that pushes the branch and opens a PR. In that mode the supervisor's integration step looks instead like:

1. Locate the PR via `gh pr list --head boid/<child_id8>`.
2. If the PR is open and mergeable, run `gh pr merge --merge --delete-branch`, then `git fetch origin && git pull --ff-only origin <base_branch>`, verify, and either spawn the next child or move on to exit handling.
3. If the merge cannot proceed:
   - Plain conflict or CI failure with a clear remediation: `boid task reopen <child_id> -m "<focused fix instruction>"` so the same child re-runs (rebase / re-push to fix CI). Resume monitoring it.
   - Anything ambiguous (no PR, `mergeable=UNKNOWN` twice, unexpected branch protection, etc.): `boid task notify "$BOID_TASK_ID" --ask "..."` to consult the user; do not guess.

Treat aborted children the same way in both modes (no merge attempt; diagnose and either retry or escalate).
...
**Ask mode (`--ask`)**: blocking question. Transitions task `executing → awaiting`, fires the configured notify hook (Web UI alert / push notification), and saves the question in the task payload. Your session ends after the call; you are re-invoked with `BOID_USER_ANSWER` set when the user replies. This is the primary mode for the supervisor agent.
...
When all children have reached `done` / `aborted`, the supervisor agent **must execute exactly one of the following**. Do not let the session hang without action (users cannot notice the supervisor has completed unless something fires):
...
|---|---|
| `artifact.auto-merge.merged` | Until the dependency task's PR is merged by auto-merge (PR-mode override only) |
...
The branch to fork the worktree from (and the merge target of the PR, in PR-mode override). Inherits from the project-top `base_branch` if omitted; falls back to the daemon's current HEAD branch. Specify explicitly when you want derivative tasks to branch off from the current branch during plan execution:
