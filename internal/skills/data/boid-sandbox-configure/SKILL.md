...
- **必要な env** (default: なし)
  ...
  - 不要なら `env:` セクション自体を省く
  - 例外: `${boid:repo_slug}` は secret ではなく boid が token 登録時に project の
    origin remote から `host/owner/repo` 形式で導出するコンテキスト変数。 host
    command は host 側で中立ディレクトリ (repo checkout でない cwd) で実行されるため、
    `gh` の `GH_REPO` のように repo 文脈が要るコマンドはこの変数で渡す
- **reject ルール** (default: なし)
  - `reject: [{match: <glob>, reason: <string>}]` で特定の引数パターンを拒否できる
  - `match` は allow/deny と同じ glob 意味論 (joined args に対して)
  - `reason` は必須。 拒否時に `host_commands.<name>: rejected: <reason>` として
    エージェントに返るので、 代替手段を書く (例: gh の `--body-file` → `--body "$(cat <file>)"`)
