...
Prefer this over `boid task notify --ask` for portable Q&A. `notify --ask` remains available for the legacy session-resume path (your turn exits, the reply arrives in `$BOID_USER_ANSWER` on the next invocation).
...
Posts a user reply to an `awaiting` task. All three flags are required. Normally the user replies via the Web UI / push hook; supervisors rarely call this directly. Resolves both `notify --ask` (session-resume) and `task ask` (blocking RPC) awaitings — the daemon dispatches by `payload.awaiting.mode`.
