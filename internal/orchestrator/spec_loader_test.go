...
func TestMergeKitMeta_HostCommandConflict(t *testing.T) {
	t.Run("same command name in two kits returns error", func(t *testing.T) {
		base := &projectspec.ProjectMeta{ID: "proj", Name: "Project"}
		...
		if !strings.Contains(err.Error(), "project.local.yaml") {
			t.Errorf("error message should suggest project.local.yaml resolution: %v", err)
		}
	})

	t.Run("no conflict when kits have different commands", func(t *testing.T) {
		base := &projectspec.ProjectMeta{ID: "proj", Name: "Project"}
		...
		if len(result.HostCommands) != 2 {
			t.Errorf("expected 2 host commands, got %d", len(result.HostCommands))
		}
	})

	t.Run("project base can override kit command without error", func(t *testing.T) {
		base := &projectspec.ProjectMeta{
			ID:           "proj",
			Name:         "Project",
			HostCommands: projectspec.HostCommands{"go": {Path: "/custom/go"}},
		}
		kit := &projectspec.KitMeta{HostCommands: projectspec.HostCommands{"go": {Path: "/usr/bin/go"}}}

		result, err := projectspec.MergeKitMeta(base, []*projectspec.KitMeta{kit}, []string{"mykit"})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if result.HostCommands["go"].Path != "/custom/go" {
			t.Errorf("expected base to override kit, got path %q", result.HostCommands["go"].Path)
		}
	})
}

func TestApplyProjectLocalMeta_HostCommandOverride(t *testing.T) {
	t.Run("project.local.yaml can override kit-defined command", func(t *testing.T) {
		base := &projectspec.ProjectMeta{
			ID:           "proj",
			Name:         "Project",
			HostCommands: projectspec.HostCommands{"uv": {Path: "/usr/bin/uv"}},
		}
		local := &projectspec.ProjectLocalMeta{
			HostCommands: projectspec.HostCommands{"uv": {Path: "/custom/bin/uv"}},
		}

		got := projectspec.ApplyProjectLocalMeta(base, local)
		if got.HostCommands["uv"].Path != "/custom/bin/uv" {
			t.Errorf("local override not applied, got %q", got.HostCommands["uv"].Path)
		}
	})
}
...
func TestUnionBindMounts_ModePromotion(t *testing.T) {
	t.Run("ro + rw promotes to rw", func(t *testing.T) {
		base := &projectspec.ProjectMeta{ID: "proj", Name: "Project"}
		kitA := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/shared", Mode: "ro"}}}
		kitB := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/shared", Mode: "rw"}}}

		result := mustMergeKitMeta(t, base, []*projectspec.KitMeta{kitA, kitB}, []string{"kit-a", "kit-b"})
		if len(result.AdditionalBindings) != 1 {
			t.Fatalf("expected 1 binding, got %d: %+v", len(result.AdditionalBindings), result.AdditionalBindings)
		}
		if result.AdditionalBindings[0].Mode != "rw" {
			t.Errorf("expected rw after promotion, got %q", result.AdditionalBindings[0].Mode)
		}
	})

	t.Run("rw + ro keeps rw", func(t *testing.T) {
		base := &projectspec.ProjectMeta{ID: "proj", Name: "Project"}
		kitA := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/data", Mode: "rw"}}}
		kitB := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/data", Mode: "ro"}}}
		...
		if result.AdditionalBindings[0].Mode != "rw" {
			t.Errorf("expected rw to be preserved, got %q", result.AdditionalBindings[0].Mode)
		}
	})

	t.Run("ro + ro stays ro", func(t *testing.T) {
		base := &projectspec.ProjectMeta{ID: "proj", Name: "Project"}
		kitA := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/tools", Mode: "ro"}}}
		kitB := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/tools", Mode: "ro"}}}
		...
		if result.AdditionalBindings[0].Mode != "ro" {
			t.Errorf("expected ro to be kept, got %q", result.AdditionalBindings[0].Mode)
		}
	})

	t.Run("rw + rw stays rw", func(t *testing.T) {
		base := &projectspec.ProjectMeta{ID: "proj", Name: "Project"}
		kitA := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/cache", Mode: "rw"}}}
		kitB := &projectspec.KitMeta{AdditionalBindings: []projectspec.BindMount{{Source: "/opt/cache", Mode: "rw"}}}
		...
		if result.AdditionalBindings[0].Mode != "rw" {
			t.Errorf("expected rw to be kept, got %q", result.AdditionalBindings[0].Mode)
		}
	})
}

func TestReadProjectMetaWithKits_LocalKits_Conflicts(t *testing.T) {
	t.Run("conflicting host_commands across kits returns error", func(t *testing.T) {
		dir := t.TempDir()
		boidDir := filepath.Join(dir, ".boid")
		...
		_ = os.WriteFile(filepath.Join(boidDir, "project.yaml"), []byte("id: test-proj\nname: Test Project\nkits:\n  - kit-a\n  - kit-b\n"), 0o644)
		_ = os.WriteFile(filepath.Join(boidDir, "kits", "kit-a", "kit.yaml"), []byte("host_commands:\n  gh:\n    path: /usr/bin/gh\n"), 0o644)
		_ = os.WriteFile(filepath.Join(boidDir, "kits", "kit-b", "kit.yaml"), []byte("host_commands:\n  gh:\n    path: /usr/local/bin/gh\n"), 0o644)

		_, err := projectspec.ReadProjectMetaWithKits(dir, nil)
		if err == nil {
			t.Fatal("expected error for conflicting host_commands across kits, got nil")
		}
		if !strings.Contains(err.Error(), "gh") {
			t.Errorf("error should mention conflicting command 'gh': %v", err)
		}
	})

	t.Run("same source binding in two kits promotes to rw", func(t *testing.T) {
		dir := t.TempDir()
		boidDir := filepath.Join(dir, ".boid")
		for _, name := range []string{"kit-ro", "kit-rw"} {
			_ = os.MkdirAll(filepath.Join(boidDir, "kits", name), 0o755)
		}
		_ = os.WriteFile(filepath.Join(boidDir, "project.yaml"), []byte("id: test-proj\nname: Test Project\nkits:\n  - kit-ro\n  - kit-rw\n"), 0o644)
		_ = os.WriteFile(filepath.Join(boidDir, "kits", "kit-ro", "kit.yaml"), []byte("additional_bindings:\n  - source: /opt/shared\n    mode: ro\n"), 0o644)
		_ = os.WriteFile(filepath.Join(boidDir, "kits", "kit-rw", "kit.yaml"), []byte("additional_bindings:\n  - source: /opt/shared\n    mode: rw\n"), 0o644)
		...
		if len(meta.AdditionalBindings) != 1 {
			t.Fatalf("expected 1 binding after dedup, got %d: %+v", len(meta.AdditionalBindings), meta.AdditionalBindings)
		}
		if meta.AdditionalBindings[0].Mode != "rw" {
			t.Errorf("expected mode=rw after promotion, got %q", meta.AdditionalBindings[0].Mode)
		}
	})
}
