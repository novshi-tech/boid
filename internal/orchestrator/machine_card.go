...
func NewCardMachine() *StateMachine {
	rules := []Rule{
		...
		// command_finished/command_failed/command_force_released: a card
		// command's self-recorded terminal outcome
		// (recordCardRequestOutcome, card_request.go) — registered
		...
		// the identity index) — see card_state_actions.go. Registered for the
		// same reason as the command_* family just above; the write goes
		// through CreateAction directly, never Apply.
