...
// `Run: exec "$BOID_CONNECTOR_EXEC"`. The daemon never resolves
// BOID_CONNECTOR_EXEC itself at hydrate time — it is an env var StartExec
// sets on the connector job, substituted by the sandbox's own `sh -c` at
// runtime, the same "daemon does not interpret Run's contents" invariant
// every other trigger already has (Trigger.Run's own doc comment).
const signalTriggerRun = `exec "$BOID_CONNECTOR_EXEC"`

// signalTriggerNamePrefix names the derived trigger for source
// "<pack>/<connector>" as "signal:<pack>/<connector>" (one trigger per
// source).
const signalTriggerNamePrefix = "signal:"

// deriveSignalTriggers turns a project.yaml `signals.sources[]` list into
// the Trigger entries hydrate appends to meta.Triggers. Called from
...
// once the two lists are concatenated.
//
...
// UNRELATED judgment-side predicate for a workspace's OWN triggers[]
// entries that scan the inbox).
//
...
// declaration — one trigger per source cannot hold for two sources naming
...
// validation here is warning-only) for each derived (Connector != nil)
// trigger whose declared service is not in the workspace's OWN
// enabled-services list. Called from ProjectStore.GetWithWorkspace — NOT
// project.yaml parse/load time: GetWithWorkspace hydrates on every call,
// and the trigger sweep loop calls it roughly once per project per sweep
// tick (1 minute, TriggerSweepResolution) via hydrateMetaForTriggers, so a
// persistent misconfiguration logs THIS warning roughly once a minute for
// as long as it remains unresolved, not just once at `boid project fetch`
// time. Accepted as-is (no throttling added) rather than introducing a new
// dedup-state mechanism to solve a cosmetic log-volume concern — see
// docs/ja/reference/project-yaml.md's "service の有効化チェック" section
// for the documented, accurate frequency.
//
...
// an acceptable, safe-direction imprecision for a warning-only check that
// does not need to be exhaustive, only present.
