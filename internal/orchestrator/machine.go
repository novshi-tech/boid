...
// DefaultMachine returns the single unified StateMachine used for all task behaviors.
//
// Manual actions:
//
...
//
//	executing → done      : TasksReady (plan tasks)
//	executing → reworking : artifact && AnyFindingUnresolvedForState("executing") (CI loop)
//	executing → verifying : artifact && !AnyFindingUnresolvedForState("executing") (general)
//
// Auto transitions from verifying:
//
//	verifying → reworking : AnyFindingUnresolvedForState("verifying")
//	verifying → done      : NoUnresolvedFindings() (pass-through when no verify gate)
//
// Auto transitions from reworking:
//
//	reworking → reworking : !NoUnresolvedFindings() (self-loop until resolved)
//	reworking → done      : NoUnresolvedFindings()
func DefaultMachine() *StateMachine {
	return &StateMachine{
		...
		Rules: []Rule{
			...
			// Auto from executing: plan tasks → done
			{FromStatus: "executing", ToStatus: "done", Condition: TasksReady},

			// Auto from executing: artifact + open findings → reworking (CI loop)
			...
			// Auto from executing: artifact + no open findings → verifying
			{FromStatus: "executing", ToStatus: "verifying", Condition: func(p json.RawMessage) bool {
				return TraitNonNull(p, "artifact") && !AnyFindingUnresolvedForState("executing")(p)
			}},

			// Auto from verifying: open findings → reworking
			{FromStatus: "verifying", ToStatus: "reworking", Condition: AnyFindingUnresolvedForState("verifying")},

			// Auto from verifying: no unresolved findings → done (pass-through)
			{FromStatus: "verifying", ToStatus: "done", Condition: NoUnresolvedFindings()},

			// Auto from reworking: any unresolved → self-loop
			{FromStatus: "reworking", ToStatus: "reworking", Condition: func(p json.RawMessage) bool {
				return !NoUnresolvedFindings()(p)
			}},

			// Auto from reworking: all resolved → done
