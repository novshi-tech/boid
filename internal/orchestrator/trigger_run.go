...
// trigger_runs 台帳の store 層。single-flight と実行結果の記録がここに載る。
//
...
//     CreateTriggerRun がこの制約に弾かれたら ErrTriggerRunInFlight を返す。
...
type TriggerRun struct {
	...
	// data, not an error this store rejects.
	ExitCode *int
}
...
// is rejected by idx_trigger_runs_inflight_unique, the partial UNIQUE index
// on trigger_runs(project_id, trigger_name) WHERE finished_at IS NULL. Callers
...
// (store.go).
//
// run.JobID may be empty: the row is created FIRST, BEFORE dispatch, so
// single-flight is claimed via this INSERT succeeding — not via a later
// read. There is no job id yet at that point (StartExec hasn't been
// called); SetTriggerRunJobID fills it in once dispatch succeeds. See
// internal/api/trigger_loop.go's fireTrigger for the full insert →
// dispatch → fill-in-job-id sequence.
//
...
func CreateTriggerRun(dbtx db.DBTX, run *TriggerRun) error {
	...
	// Always normalize to UTC, not just when the caller left StartedAt
	// zero — a caller-supplied non-UTC time would otherwise be stored
	// as-is, so ORDER BY started_at (a TEXT lexicographic comparison) could
	// put a "+09:00" row out of order relative to a "Z" row once the
	// daemon's local timezone changed.
...
// succeeds (CreateTriggerRun's own doc comment describes the
// insert-then-dispatch split). Errors (including "no such row") are
...
// failure cleanup path: fireTrigger claims single-flight by inserting a row
// BEFORE calling StartExec, so a dispatch failure must undo that claim to
// preserve the pre-existing "fail-open,
...
func CompleteTriggerRun(dbtx db.DBTX, id string, finishedAt time.Time, exitCode int) error {
	...
	// Same UTC normalization as CreateTriggerRun's started_at — finished_at
	// feeds GCTriggerRuns' `finished_at < ?` retention cutoff, so a non-UTC
	// value here would make that comparison drift the same way started_at's
	// ordering did.
