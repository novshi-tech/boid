package tui

import (
	tea "github.com/charmbracelet/bubbletea"
	"github.com/novshi-tech/boid/internal/client"
)

// Screen represents a single screen in the navigation stack.
type Screen interface {
	Init() tea.Cmd
	Update(msg tea.Msg) (Screen, tea.Cmd)
	View(width, height int) string
	ShortHelp() string
}

// SharedState holds state shared across all screens.
type SharedState struct {
	Client      *client.Client
	TmuxEnabled bool
	Panes       map[string]string // jobID -> paneID
}

// pushScreenMsg requests the App to push a new Screen onto the stack.
type pushScreenMsg struct{ screen Screen }

// popScreenMsg requests the App to pop the top Screen from the stack.
type popScreenMsg struct{}

// PushScreen returns a tea.Cmd that pushes the given screen.
func PushScreen(s Screen) tea.Cmd {
	return func() tea.Msg { return pushScreenMsg{screen: s} }
}

// PopScreen is a tea.Msg that pops the top screen.
func PopScreen() tea.Msg { return popScreenMsg{} }
