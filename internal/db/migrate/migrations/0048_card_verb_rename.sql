-- card-lifecycle verbs `working`/`done` renamed to `start`/`complete`.
-- execution 型の同名の start/done は無関係 — このデータ backfill は
-- type='card' の行だけを対象とする。past action 履歴 (actions.type /
-- actions.from_status / to_status) は台帳なので書き換えない。書き換えるのは
-- 現在保存されている suggestion だけ: task_triage 相当の列 (suggestion_verb)
-- と detail JSON の suggestion.verb フィールド。
