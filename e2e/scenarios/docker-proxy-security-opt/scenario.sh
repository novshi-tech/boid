...
WS_SLUG="docker-proxy-security-opt"
...
env:
  DOCKER_PROXY_TEST_CASE: security-opt
capabilities:
  docker: {}
YAML
...
e2e_run "$E2E_BIN_DIR/boid" workspace assign "docker-proxy-security-opt" "$WS_SLUG"

task_create_output="$("$E2E_BIN_DIR/boid" task create <<'YAML'
project_id: docker-proxy-security-opt
title: SecurityOpt deny
behavior: smoke
YAML
)"
printf '%s\n' "$task_create_output"
