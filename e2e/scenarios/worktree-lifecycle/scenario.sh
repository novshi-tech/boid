#!/usr/bin/env bash
set -euo pipefail

PROJECT_DIR="$E2E_WORKSPACE_DIR/app"
GIT_LOG="$E2E_STATE_DIR/fake-git.log"

e2e_log "registering project from $PROJECT_DIR"
e2e_run "$E2E_BIN_DIR/boid" project add "$PROJECT_DIR"

e2e_log "creating worktree task"
task_create_output="$("$E2E_BIN_DIR/boid" task create <<'YAML'
project_id: worktree-lifecycle
title: Worktree Lifecycle Test
behavior: dev
YAML
)"
printf '%s\n' "$task_create_output"
task_id="$(printf '%s\n' "$task_create_output" | sed -n 's/^task created: \([0-9a-f-]*\) (.*/\1/p')"
[[ -n "$task_id" ]] || e2e_fail "failed to parse task id"

e2e_log "starting task $task_id"
e2e_run "$E2E_BIN_DIR/boid" action send --task "$task_id" --type start

e2e_log "waiting for executing state"
task_json="$("$E2E_BIN_DIR/boid-e2e" wait-task-status --timeout 15s --interval 100ms "$task_id" executing)"
printf '%s\n' "$task_json"
e2e_assert_contains "$task_json" '"status":"executing"'

e2e_log "waiting for hook job to be dispatched (implies worktree created)"
"$E2E_BIN_DIR/boid-e2e" wait-job-count "$task_id" 1

e2e_log "verifying worktree add was logged"
git_log="$(cat "$GIT_LOG")"
printf '%s\n' "$git_log"
e2e_assert_contains "$git_log" "worktree add"

e2e_log "waiting for done state"
task_json="$("$E2E_BIN_DIR/boid-e2e" wait-task-status --timeout 30s --interval 100ms "$task_id" done)"
printf '%s\n' "$task_json"
e2e_assert_contains "$task_json" '"status":"done"'

e2e_log "waiting for worktree remove to be logged"
deadline=$((SECONDS + 10))
while true; do
  git_log="$(cat "$GIT_LOG" 2>/dev/null || echo "")"
  if [[ "$git_log" == *"worktree remove"* ]]; then
    break
  fi
  (( SECONDS >= deadline )) && e2e_fail "timed out waiting for worktree remove in fake-git.log"
  sleep 0.1
done
printf '%s\n' "$git_log"
e2e_assert_contains "$git_log" "worktree remove"

e2e_log "worktree lifecycle verified successfully"
