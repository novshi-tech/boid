...
e2e_run "$E2E_BIN_DIR/boid" workspace assign "docker-proxy-volume-bind-escape" "$WS_SLUG"

task_create_output="$("$E2E_BIN_DIR/boid" task create <<'YAML'
project_id: docker-proxy-volume-bind-escape
title: Volume bind escape deny
behavior: smoke
YAML
)"
printf '%s\n' "$task_create_output"
