...
WS_SLUG="docker-proxy-host-network"
...
env:
  DOCKER_PROXY_TEST_CASE: host-network
capabilities:
  docker: {}
YAML
...
e2e_run "$E2E_BIN_DIR/boid" workspace assign "docker-proxy-host-network" "$WS_SLUG"

task_create_output="$("$E2E_BIN_DIR/boid" task create <<'YAML'
project_id: docker-proxy-host-network
title: Host network deny
behavior: smoke
YAML
)"
printf '%s\n' "$task_create_output"
