...
WS_SLUG="docker-proxy-cross-job-isolation"
...
e2e_run "$E2E_BIN_DIR/boid" workspace assign "docker-proxy-cross-job-isolation" "$WS_SLUG"

task_create_output="$("$E2E_BIN_DIR/boid" task create <<'YAML'
project_id: docker-proxy-cross-job-isolation
title: Cross-job isolation
behavior: smoke
YAML
)"
printf '%s\n' "$task_create_output"
