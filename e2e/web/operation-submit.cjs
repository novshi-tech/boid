...
(async () => {
 ...
 try {
  ...
  await page.route('http://boid.test/**', async route => {
   ...
   if (mode === 'old') return route.fulfill({contentType:'text/html',body:'<section id="task-operations"><div class="operation-result" data-operation-id="old">Earlier operation</div></section>'});
  ...
  await page.getByText('Discuss',{exact:true}).click();
  await page.waitForFunction(() => document.querySelector('#operation-submit-feedback')?.textContent.includes('could not be confirmed'));
  assert.equal(await page.locator('#task-operations').textContent(),'Accepted');
  assert.equal(count,3);
  assert.deepEqual(errors,[]);
  console.log('PASS: response loss, retained instruction/CSRF/command, duplicate suppression, correlated response and rejection of unrelated history');
