...
func TestRunConfigGet_FullDump_ReflectsWhatWasSet(t *testing.T) {
	...
	if err := runConfigSet(setCmd, []string{"web.public_url", "https://example.com"}); err != nil {
	...
	if got := out.String(); !strings.Contains(got, "public_url: https://example.com") {
		t.Errorf("expected the just-set web.public_url in full dump, got:\n%s", got)
	}
}
...
func TestRunConfigGet_SingleKey(t *testing.T) {
	...
	cmd := configGetCmd
	cmd.SetOut(&out)
	if err := runConfigGet(cmd, []string{"web.public_url"}); err != nil {
		t.Fatalf("runConfigGet: %v", err)
	}
	if got := strings.TrimSpace(out.String()); got != "https://example.com" {
		t.Errorf("get web.public_url = %q, want https://example.com", got)
	}
}
