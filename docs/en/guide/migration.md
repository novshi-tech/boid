...
2. **(Changed in Phase 2.5 PR7)** Existing `kits:` refs (e.g. `github.com/.../foo`) are only name-validated (`ValidKitName`) and shown as an informational note in migrate's dry-run/apply output. `WorkspaceMeta.Kits` itself was removed, so nothing is carried into the workspace any more — if such a kit used to supply host_commands/env, add those to workspace.yaml by hand after migrating. `additional_bindings` is retired and cannot be restored through migration.
3. If `host_commands` is non-empty, its reference names are merged into the workspace and its definitions into the daemon-wide `~/.config/boid/host_commands.yaml`. `additional_bindings` is retired and is not applied; move persistent toolchains to the workspace home `init.sh`.
...
- If the slug already exists: its current content is fetched with `GET /api/workspaces/<slug>`, merged with the fields this migration generated, and written back with `PUT /api/workspaces/<slug>` (`If-Match: <revision>`). `env`, capabilities, and host-command references are merged according to the migration rules; retired `additional_bindings` are not written.
...
| `additional_bindings` | Not migrated; retired. Use the workspace home `init.sh` for persistent toolchains. |
...
- `boid project migrate` still name-validates and displays legacy `kits:` refs informationally, but no longer resolves them into the workspace. Host-command references and definitions are migrated; `additional_bindings` is ignored because the mechanism is retired.
