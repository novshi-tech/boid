...
**Note:** `boid project migrate <dir> --apply` has a path that opens the host-side `boid.db` directly — under the compose daemon, the daemon's real DB lives inside the `boid_state` volume, so `--apply` can silently write to a different (or newly-created) DB on the host instead. It falls back to a socket-based path when the daemon is reachable, which does apply correctly — verify daemon reachability (e.g. `boid task list`) before relying on `--apply`.
