...
> **Updated 2026-05-14:** Claude Code's upcoming pricing change moves `claude --print` (non-interactive) to a separate metered credit pool, so hook-launched agent sessions now run on a **real PTY in interactive mode** again. The `--resume <session_id>` continuation contract and the state-machine Q&A channel survive unchanged; what changed is that session termination is driven by daemon-side signal delivery — on a successful `notify --ask` or `boid agent stop` from the agent the daemon sends SIGUSR1 to the runtime's process group, `run-agent.py` catches it and SIGTERMs only the `claude` process (bash and the EXIT trap survive via `trap '' USR1`, so the trap's `boid job done --output-file payload_patch.json` is the canonical CompleteJob caller). References to `claude --print` below describe the historical design.
