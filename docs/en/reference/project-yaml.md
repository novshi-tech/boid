...
>
> **Phase 2.5 PR7** (`docs/plans/workspace-db-consolidation.md`) removed `WorkspaceMeta.Kits` from the type outright — a *workspace's* own `kits:` field no longer exists at all, and `POST`/`PUT`/`import /api/workspaces` now reject a `kits:` key in the request body with a 400 ("unknown field kits"). `boid project migrate` still collects and name-validates a legacy project.yaml's `kits:` refs (top-level and `task_behaviors.<name>.kits`), but no longer resolves or folds them into the migrated workspace at all — they are shown in the migration's dry-run/apply output purely as an informational "add this by hand if still needed" note. The one remaining caller that still honors a hand-authored/e2e-fixture workspace shadow yaml's legacy `kits:` list is `boid workspace assign`'s auto-create convenience path (`cmd/workspace.go`'s `ensureWorkspaceExistsForAssign`), which resolves it client-side against the installed kits directory before submitting an already-materialized (kits-free) body.
...
`<sub-path>` is optional — if the kit lives at the repository root, omit it. `boid project migrate` extracts only the last path segment (e.g. `claude-code`) from `ref` for name validation and its informational output; it is no longer carried into the migrated workspace at all (see the Phase 2.5 PR7 note above).
