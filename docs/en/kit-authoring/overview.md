...
> **Retirement notice (Phase 2.5 PR6/PR7, 2026-07)**: the kit *mechanism* has been retired. `boid kit init` / `list` / `remove` and `boid workspace configure` are gone, and `WorkspaceMeta.Kits` was removed from the code. This page documents the `kit.yaml` file format only for legacy compatibility paths. **For new configuration, set `host_commands` / `env` directly on a workspace and use the workspace-home `init.sh` for persistent toolchains; `additional_bindings` is retired.**
>
