...
> Status (2026-06-24): **Implemented**。 PR1 (変更1+3) / PR2 (変更2+4) / PR3 (skill + doc) の 3 本に分割して実装。 実装で確定した決定は末尾「実装メモ」を参照。
>
> 関連:
...
- **D4 (qid 整合)**: qid は awaiting payload に置いて **episode 内で安定**。 再 ask は payload の既存 `question_id` に re-attach し、 新規 qid は生成しない。 回答 (`PendingAnswer`) も同じ awaiting trait に同居するので、 切断中に届いた回答と再 ask が同一 qid で噛み合う。
- **B1 緩和**: `Register` は同一タスクに **異なる qid** が pending の時だけ `ErrAskPending`。 同一 qid の再アタッチは許可 (`blocking_ask_registry.go`)。 handle 化は不要だった — 万一の Cancel clobber も durable `PendingAnswer` が backstop するので、 最悪 fast-path が 1 往復だけ slow-path に劣化するだけ。
- **回答受け渡し**: `answerBlocking` は `Notify` 成否で fast/slow を分岐。 fast (live agent) は即 flip executing + strip、 slow (切断) は `SetPendingAnswer` で durable parking + awaiting 維持。 flip は「エージェントが実際に回答を受領した時」 (`consumePendingAnswer` or fast-path) にのみ起こす。
- **grace 既定値 / config キー**: `task_ask.disconnect_grace`、 既定 **30m**。 `blockForAnswer` の Wait ctx-cancel で `time.AfterFunc` の一発 reaper を arm し、 grace 後に `graceAbortCheck` (awaiting / 同一 qid / 回答なし / live agent なし の全成立時のみ abort)。
- **startup 回収**: `MarkStaleAwaitingTasksAborted` を新設 (既存 `MarkStaleExecutingTasksAborted` は据え置き)。 restart 後は registry が空で awaiting は全てゾンビなので daemon_shutdown code で abort → 既存 auto-reopen sweep が拾う。
- **shim exit code は不採用**: 切断時は boid プロセス自体がハーネスに kill されるため clean exit code を返す経路が無い。 再 ask の指示は SKILL.md (モデルレベルの再試行) で行う。 bash の retry ループはハーネスがコマンドごと kill するので無効。
