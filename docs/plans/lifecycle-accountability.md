...
- **stuck executor**: `executing` 状態のまま長時間 (例: 10 分) active な job が
  無い子。 `boid task get <child> --field status` と `boid job list --task <child>`
  で最終 job の更新時刻を確認する
...
executor skill (`/boid-executor`) が `done_request` 発行前に必ず書く構造化レポート。
親 supervisor は `boid task get <child> --field payload.artifact.report` で structured
に読む。
...
1. **明示 notify**: agent が `boid task notify --ask "done_request: ..."` を発行 →
   daemon が awaiting 遷移 → claude SIGTERM
...
| Phase | 内容 | 触る箇所 | 完了条件 |
|---|---|---|---|
| 1 | boid-supervisor skill の polling を child awaiting 監視に拡張 (answer / reopen / abort / 上向き notify の手順を明文化)。 異常子検知ロジック (stuck executor / 未報告 done_request) を追加。 supervisor sandbox から `boid job log` が読めることを E2E で確認 | skill md のみ | skill 更新 + E2E 確認 |
| 2 | boid-kits/claude-code Stop hook 削除 (`boid-stop-settings.json` 削除) + notify hook 発火条件への parent_id gate (daemon hardcode) | boid-kits + boid 本体 (notify hook 発火条件) | kit / 本体 更新 + 動作確認 |
...
4. **failure_report の意味付け**: 現案では `--ask` メッセージ prefix で failure を
   伝えるが、 将来 action type を明示的に追加する余地は残す
