...
ステータス: 構想 (draft) — 着手判断前 → **移行戦略 ①–⑤ 完了、⑥ (Phase 6) は PR1–8 landed・PR9 (finale)
実装 PR 提出済み・CI green 待ち** (2026-07-23 時点、詳細は「移行戦略」節と
[phase6-container-backend.md](phase6-container-backend.md))。残る ⑦ (k8s backend + 別ホスト構成) と
⑧ (リモートランナー) は Phase 7 送り・未着手。
作成日: 2026-07-06
...
- **移行は strangler 方式 + 一気コンテナ化** (2026-07-22 改訂、論点 D): sandbox backend を
  interface 化し、現行 userns backend を実装の 1 つに抽出する。ただし Phase 6 は
  **daemon も含めて一気にコンテナ化**する方針に確定した (単一ホスト compose、実装計画は
  [docs/plans/phase6-container-backend.md](phase6-container-backend.md))。
  daemon をコンテナ化すると userns backend を config フラグでホット切替するのは nested userns
  になり不可なため、strangler の並走は **「host daemon + userns 旧デプロイ (v0.0.13) を移行期間だけ
  温存する deploy-level rollback」**の形にする。恒久 2 構成サポートはせず、container backend が
  dogfood で安定したら旧デプロイごと撤去する (当初は「別バックエンド追加なので並走が正当・一気
  切替はしない」としていたが、blast radius の小ささと顧客展開速度・2 構成恒久保守回避を優先して転換)。
...
- **ローカル**: docker compose で「daemon + egress proxy + workspace (job)」のスタック。
  当初は「第一歩は daemon をホストプロセスのまま残し、コンテナ化は後続」としていたが、
  **一気コンテナ化に方針転換した** (2026-07-22、論点 D)。daemon は docker socket を mount して
  job を sibling コンテナ (docker-out-of-docker) として生成する。移行期間中の rollback は
  host 旧デプロイへの deploy 切替で行う (基本方針参照)。
...
- 対話 attach は既存 WebSocket endpoint (`GET /api/jobs/{id}/attach/ws`,
  ...
  保証されないため、**ローカル・リモート両経路で WS に一本化する**
  (二経路併存の保守負担を避ける、Phase 3 実装計画で決定)
...
- **コンテナ daemon の初回ペアリング**: 特別な機構は入れず、
  `docker exec <container> boid web pair` (or `kubectl exec`) で code 発行 →
  リモート CLI `boid login <url>` で入力、の既存フローで足りる。
  コンテナに exec できる人 = 管理者、なので認可の起点として自然
...
- **daemon 側設定ファイルの CLI 直読み書き**: `workspace list/show/remove/configure`、
  ...
  → **Phase 2.5 (workspace DB 一元化) で解消**: workspace 系は API 化、
  kit 系は機構ごと廃止、web set-url/set-addr はローカル専用確定。
  Phase 3 実装時にはこの分類の残りゼロで進む
- **ローカル専用で確定**: start / stop / check / exec / fetch /
  runner-* (内部プラミング) / project migrate (SQLite 直開き)。
  Phase 2.5 で `kit init` と `workspace configure` も本カテゴリでなく **廃止** に移動
...
**kit 機構は Phase 2.5 (workspace DB 一元化) で退役する** (2026-07-16 決定、
[workspace-db-consolidation.md](workspace-db-consolidation.md))。
kit list / remove / init コマンドと `KitRegistry` / `MergeKitRuntime` は本 phase で撤去、
`workspace configure` も同時に廃止 (kit の役割は image + workspace init script
(Phase 4) + broker イメージが持つ host_command 事前既知集合で置き換わる)。
ただし kit 遺物の `additional_binding` は Phase 4 ($HOME workspace volume 契約先行)
までは userns backend が必要とするため列と merge 経路を保持し、Phase 4 で退役する。
...
> 2026-07-22 (論点 D): 一気コンテナ化への方針転換に伴い、以下のうち container backend に紐づく
> 論点の多くは [docs/plans/phase6-container-backend.md](phase6-container-backend.md) の「決定事項」で
> plan レベルの解が出た (診断成果物の回収 = 種類ごと置き場分離 / 対話 attach = docker attach・logs /
> 孤児 reconcile = ReapOrphans / イメージ所有権と version skew = daemon と job が共有する versioned
> イメージへの boid 焼き込み / egress・docker proxy の broker 側配置)。各項目の現行の扱いは Phase 6
> plan doc を正とする。以下は戦略レベルの記録として残す。
...
- **workspace 単位 PAT 分離 と config surface 圧縮** (2026-07-10 追加):
  現行 `gateway.hosts[]` は daemon global の `host → secret_key` mapping で、
  同一 host に対して workspace ごとに token を切り替えられない (顧客展開で
  複数 organization を 1 daemon に束ねると identity 分離不能)。また host + forge
  の 2 軸は SaaS Cloud 前提では冗長。cutover 動作確認後の改善候補として棚上げ。
  workspace = secret namespace = identity 方式で設計案を確定
  (詳細は [docs/plans/git-gateway-cutover.md](git-gateway-cutover.md) の
  「post-cutover 改善候補」節を参照)
- **認証付き CLI の broker RPC 設計**: stdin / ファイル引数の転送方式、
  repo コンテキストの伝搬 (`-R` 強制 or RPC 伝搬)、許可コマンド集合と引数ポリシーの粒度
- ~~**リモート CLI の細部**~~ → **解決済み** (2026-07-16、
  [cli-remote-connection.md](cli-remote-connection.md))。profile 管理・token 保存・
  Bearer 認証新設・二分類 (三値必須化) が確定。workspace / kit / web set-* 系の
  分類は Phase 2.5 で解消 (workspace = リモート可 / kit = 廃止 / web set-* = ローカル専用)
...
2.5. **workspace DB 一元化 + kit 機構退役** (2026-07-16 追加、
   実装計画: [docs/plans/workspace-db-consolidation.md](workspace-db-consolidation.md)):
   workspace 責務を machine-local yaml から DB に移し、workspace 系操作を
   Web UI / CLI から一様な API 経由にする。workspace が持つのは image (nullable) +
   host_command 参照名 + env + allowed_domains + extra_repos + capabilities の
   環境非依存な設定のみ。host_commands の実定義は broker 側 (最終的には broker
   コンテナイメージ) に集約する。kit 機構 (kit list / remove / init / KitRegistry /
   MergeKitRuntime) はここで退役 (追加バインドは Phase 4 で退役予定だったが、
   本 phase で機構ごと消える形に前倒し)。Phase 3 (CLI リモート) の
   「二分類の残り」(workspace list/show/remove 等の設定ファイル CLI 直読み書き) を
   先に解消して、③ 実装時の分岐をゼロにする。workspace configure コマンドも廃止
   (プリセットイメージ選択に統合されるため)
3. **boid CLI のリモート接続** (**2.5 完了前提**、2026-07-07 追加・2026-07-16 依存改訂、
   実装計画: [docs/plans/cli-remote-connection.md](cli-remote-connection.md)):
   CLI を TCP + Bearer token クライアントに寄せ、attach 経路を
   WS endpoint に統一する (目標アーキテクチャの節参照)。
   2.5 で workspace / kit / web set-* 系の分類が固まった前提で、Phase 3 実装時の
   「二分類の残り」がゼロになる。
...
4. **$HOME workspace volume の契約先行** (**完了 2026-07-17**、独立トラック・1–3 と並行可、2026-07-09 追加):
   ...
   全 6 PR (#787-#791 + PR6) landed)
5. **shim 固定ディレクトリ化 + タスクコンテキストの boid コマンド化**
   (2026-07-09 追加、 実装計画:
   [phase5-shim-and-task-context.md](phase5-shim-and-task-context.md)):
   shim を `/opt/boid/bin` 等の固定ディレクトリ + PATH 解決に移し、
...
6. **一気コンテナ化 (daemon 込み)** (**PR1–8 landed、PR9 (finale) は実装 PR 提出済み・CI green 待ち
   (2026-07-23 時点)**、2026-07-22 再構成・論点 D、実装計画:
   [docs/plans/phase6-container-backend.md](phase6-container-backend.md)):
   sandbox backend の interface 化 + container backend に加え、**daemon 自体のコンテナ化と
   egress / docker proxy の broker 側配置 (旧ステップ 7 から前倒し) を同時に行い**、単一ホストの
   docker compose スタック (daemon + job + egress proxy) として動かす。契約は 1–5 で移行済みのため
   enforcement 差し替えに集中でき、共有 versioned イメージ + compose ネットワークにより旧構想で
   必要だった足場 (gateway/proxy の loopback 到達ハック、socket→TCP の二度手間、boid バイナリ
   bind + version skew + shim 同居) が問題ごと消える。移行中の安全網は host 旧デプロイへの
   deploy-level rollback (基本方針参照)。PR9 (finale) は container e2e CI (`e2e-container` job) を
   実 docker で動かし、job dispatch + git gateway 経由の clone 試行まで到達させた (この過程で見つかった
   複数の実装ギャップは修正済み) が、clone 先ディレクトリの permission 問題が残っており **CI はまだ
   green になっていない** — 次セッションで継続。**host 旧デプロイ・userns
   backend・host daemon 起動経路は撤去計画のみ策定** ([phase6-cutover-followups.md](phase6-cutover-followups.md))
   で、実撤去は dogfood 安定確認後の後続 PR 群 (未着手) に送る。
7. **k8s backend (operator パターン) + 別ホスト構成**: k8s Job/Pod 生成、mirror +
   `git clone --reference` の mirror 更新ワーカー (別ディスク構成でのみ必要)、別ノード RWX volume の
   整合。ステップ 6 が単一ホスト compose に閉じた分がここに来る。**Phase 7 送り、未着手**。
8. 将来拡張: リモートランナー (pull 型常駐 agent)。**Phase 7 以降送り、未着手**。

各段階で現行 backend に戻れる状態を維持する (ステップ 6 は host 旧デプロイへの切戻し —
[phase6-cutover-followups.md](phase6-cutover-followups.md) の①「dogfood 期間の並走」段階まではこの
rollback 契約が有効。②「userns backend 撤去」着手以降は失効する)。
...
- **sandbox 内の mount 先は固定の中立 path** (`/workspace` 等) にする。
  ...
  (2026-07-13 追記・実装済み: `/workspace` は「clone 先の親 dir」に精緻化され、
  実際の clone は project ごとの `/workspace/<name>` に着地する — 全 project が
  文字通り同一の絶対 path を共有すると、Claude Code の
  `~/.claude/projects/-workspace/` セッションログ slug が project 横断で衝突する
  問題があったため。`<name>` は project.yaml の `meta.name`、空なら
  `filepath.Base(WorkDir)`。詳細は
  [git-gateway-cutover.md の post-cutover 改善候補 §4](git-gateway-cutover.md)。
  コンテナ版に移行する際も同じ `/workspace/<name>` 契約を踏襲する想定)
