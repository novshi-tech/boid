...
- **workspace volume の quota / GC**: パッケージキャッシュと session jsonl は際限なく育つ。
  workspace 削除 = volume 削除は自明だが、生きている workspace の prune / quota 方針が未定
- **初期化スクリプトの詳細契約**: 対話ログイン (claude login 等) の UX、
  再初期化・部分再実行の扱い、スクリプトの語彙 (何を保証し何を許すか)。
  加えて、ホストで実行される trusted side のコードである以上
  sandbox から書き込めない置き場の明文化と、
  同時 dispatch 時の初期化の直列化・冪等性の保証が要る
...
- **private 依存の取得** (2026-07-09 追加): go private module / npm private registry 等。
  git 系依存は insteadOf 書き換えで gateway にルーティングできるが、
  registry token 系を workspace volume に置くなら「sandbox 内 credential は
