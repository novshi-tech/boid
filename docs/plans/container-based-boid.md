...
ステータス: 構想 (draft) — 着手判断前 → **移行戦略 ①–⑥ 完了** (2026-07-23 時点、詳細は「移行戦略」節)。
残る ⑦ (k8s backend + 別ホスト構成) と ⑧ (リモートランナー) は Phase 7 送り・未着手。
作成日: 2026-07-06
...
6. **一気コンテナ化 (daemon 込み)** (**完了 2026-07-23**、2026-07-22 再構成・論点 D、実装計画:
   [docs/plans/phase6-container-backend.md](phase6-container-backend.md)、全 9 PR landed):
   ...
   deploy-level rollback (基本方針参照)。PR9 (finale) で container e2e CI (`e2e-container` job) を
   実 docker 相手に green にし、sibling 疎通 3 要件・reap 全消しを検証。**host 旧デプロイ・userns
