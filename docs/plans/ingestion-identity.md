...
- **`triggers` の反映経路**。 `project.yaml` の変更が `boid project fetch` を要する現状と、
  daemon が保持するスケジュールの同期。 トリガを止める / 変えるときの挙動も含む。 加えて
  **version skew** — project.yaml は非 strict パースなので、 `triggers` を知らない旧 daemon は
  **無警告で無視する** (「トリガが 1 個も動いていないのに設定エラーが出ない」)。 一方
  workspace envelope (`workspace_envelope.go`) は `KnownFields(true)` の strict decode なので、
  workspace の default project 定義に `triggers` を書けるようにするならスキーマ追加が必須で、
  書けないと**エラーで落ちる**。 両者で挙動が逆になる点を設計時に踏まえる
...
- **無変更の観測をどちら側が抑止するか**。 workspace が毎巡同じ観測を押すと、 次巡の tick から
  見て「新しい事実が来た」が毎回真になり、 呼び直しの抑止が効かなくなる。 現行は workspace 側の reconcile
  (差分だけ push) が担っており、 その比較先が自前の fold から daemon の読み戻しに変わる。
  **「押す前に現況と比べる」は残る** — 消えるのはローカルの fold のほうだけ、 という整理を
  workspace 側 memo と揃える
- **トリガ実行の失敗時方針**。 トリガは 10 分ごとに sandbox job を起こす (現行
  `cycle_preflight.sh` と同じコスト)。 コマンドが落ちたときフェイルオープン (次も普通に回す)
  かフェイルクローズ (止めて知らせる) かは、 現行 khi が step ごとに使い分けている
- **`noted` の retention**。 10 分周期の判断のたびに長寿命 card へ積まれ得るが、 actions は
  append-only で、 終端でない task は 30 日 GC に乗らない。 **payload が完全に不透明なので
  daemon は「最新の kind だけ残す」圧縮すらできない** — 汎用化 (J-5) の対価がここに出る
  (「汎用化の線引き」節が予告した具体例)。 件数上限か期間か、 「最新 N 件だけ返す」read 口か
- **トリガの時間帯窓**。 khi の現行 cron は `*/10 8-19` で夜間は回らないが、 `every:` に窓の
  概念が無い。 そのまま移すと 24 時間回る。 `triggers` に窓を持たせるか、 スクリプトが時刻で
  自制するか
- **手動 1 巡の口**。 「今すぐ 1 回走らせる」を daemon に頼めるようにするか
...
- **論点 e の enforcement の限界を明記**。 daemon が機械的に効かせられるのはサイズ / フィールド
  粒度まで
- **`description` と action payload のサイズ上限**
...
- **履歴の投入**。 過去の却下履歴を写さないと、 **`answered` (J-6) が塞ごうとしている穴を
  移行自身が一度開ける** (移行直後に再提案の嵐になる)。 子の到達点も同様
