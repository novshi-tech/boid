...
- **終端の判定は `next_cursor` では**できない (2026-08-20 追記)。 実装の契約は
  「**マッチ 0 件のときは `filter.Since` をそのまま返す**」で
  (`internal/orchestrator/action_list.go`)、 これは polling する呼び手が「何も新しいものが
  無ければ同じカーソルを送り直せばよい」ようにするための意図的な設計である。 その帰結として
  **`next_cursor` は空文字に戻らない**:

  | 状況 | `next_cursor` |
  |---|---|
  | 初回 (`Since` 空)・0 件 | `""` |
  | 初回・N 件 | 最後の行のカーソル |
  | **継続・0 件 (= 終端)** | **渡したカーソルと同じ** |
  | 継続・N 件 | 最後の行のカーソル |

  したがって「**空文字なら終了**」で全件読みのループを書くと**永久ループ**する。
  正しい停止条件は「**返った行数が limit 未満**」である。 khi 段階 2 の実装がこれを踏んで
  実機でハングした (2 分タイムアウトで確認)。 **全件読みをする呼び手は必ずこの形にすること。**
