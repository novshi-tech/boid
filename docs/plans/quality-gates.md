...
| lint (errcheck/staticcheck 等) | ゼロ | **Tier 2** |
...
**ゲートを増やすほど flake の被害も増える**。 「赤でも flake かも」 と思われた時点でゲートの効力は死ぬため、 Tier 積み増しの前提条件として扱う。 現状 `e2e/run.sh` は sorted loop で全シナリオ直列実行、 リトライなし。 既知 flake もある ([[e2e-daemon-restart-resume-flake]])。

- 最小: **失敗シナリオの 1 回自動リトライ** (retry 発生は log に明示し、 サイレントに握りつぶさない)。 retry しても赤 = 本物。
- retry 頻発シナリオの計測 (CI ログ集計で足りる。 専用基盤は作らない)。
...
実装メモ:
- 唯一の workflow `.github/workflows/blackbox-e2e.yml` の `unit` ジョブに step 追加が最小。 templ は CI 上で `go install github.com/a-h/templ/cmd/templ@<pinned>` が要る。
...
- `-race` は unit ジョブの時間が伸びる。 遅ければ別ジョブに分離して並列化。

懸念: ほぼ無し。 既存 vet 違反・アーキ違反・race 検出があれば初回だけ掃除が要る。
...
機構ガードの底上げとして CI 追加。 errcheck / gosimple / unused / ineffassign / staticcheck 等。

実装メモ (サンドボックス制約が効く):
- 型情報込みの lint は `internal/db` を import するパッケージがサンドボックス内でビルド不可なため、 enforcement は CI ジョブ一択 ([[sandbox-cannot-build-sqlite-packages]])。 `host_commands` 経由のホスト側実行は [[no-project-specific-kits]] の趣旨からも見送り。
...
- `.golangci.yml` は新規。 初回は厳しすぎない linter セットから (厳しすぎると既存コードで大量 finding → 形骸化)。
...
| G5 golangci-lint | Tier 2 | 機構底上げとして CI 追加 |
...
6. **Tier 2** — golangci-lint CI 追加。 並行可。
7. **Tier 1 構造的ねじれの解消** — binding 系を触る PR のついでに段階的に。
8. **Tier 3** — 機構が固まってから。 G1 skill 版 → (必要なら) G2 memory 整備。

## 未決事項

- Tier 0 の既存違反の量 (vet / アーキ / race / templ drift の初回掃除コスト見積もり)。
- 前提 P のリトライ実装位置 (`run.sh` のループ内 vs scenario 単位 wrapper) と retry ログの形式。
