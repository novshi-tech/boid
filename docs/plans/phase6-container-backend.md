...
ステータス: **landed (2026-07-23)**。PR1–9 全 9 PR landed、Phase 6 実装完結。各 PR の
landed commit は「PR 分割案」節に pin 済み。残作業は撤去 (旧デプロイ・userns backend・host daemon
起動経路) のみで、[phase6-cutover-followups.md](phase6-cutover-followups.md) に計画 doc として分離。
作成日: 2026-07-22
...
**landed**: 2026-07-23 (このセクションの記述自体がその PR)
...
- **container e2e job を実装する過程で、real docker に対して一度も検証されていなかった実装ギャップが
  ...
  - **daemon コンテナが起動直後に crash していた** (実 docker で初めて判明した、PR6 の compose スケルトンが
    ...
    早期に棄却できた点で有用だった。
  - 上記はいずれも「§決定5/§決定3 は書かれていたが実装・検証が伴っていなかった」ギャップであり、real
    docker での e2e 検証を経て初めて発見・修正できた — PR6/PR7 レビューの想定通り、この PR が
    「本当に動くこと」の最終確認レイヤーだった。
