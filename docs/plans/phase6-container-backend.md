...
- **job container の UID/GID (確定、nose 決定 2026-07-22)**: container では namespace 隔離を container
  ...
  1000:1000) で非 root 起動**し host owner と揃える。workspace HOME は bind mount 不可 (dockerproxy policy)
  で named volume 経由になるため、**その named volume の owner を daemon uid に合わせる** (現行は $HOME を
  chown せず daemon uid 所有に頼る `workspace_home.go:61-68` の前提を volume owner 明示に置き換える)。
  docker userns-remap は host docker daemon 全体設定を汚し他用途 container に波及するため**使わない**。
  claude CLI の inner uid 0 拒否対策 `IS_SANDBOX=1` は container でも継続。
...
- **security 群 5 論点の実装残余 (方針は決定 4/5/6・安全網で確定済み、2026-07-22)**: named volume owner を
  daemon uid に揃える具体手 (volume 作成時 owner 指定 or 初回 chown job) / dockerproxy token の配送様式
  (broker と同じ env か gateway と同じ URL path か) / `boid.install_id` label の source と deploy-level
  reaper CLI (`boid reap`) の formalize / egress proxy dual-home の compose 表現。rootless docker (daemon
  非 root 化) は Phase 7 のハードニング候補。方針は確定済みで着手時に formalize する。
