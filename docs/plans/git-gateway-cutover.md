...
`/workspace/<peer-name>` を advertise する。ただし peer 側は上記と同じ
`Runner.Projects` 経由でしか project を引けないため、`meta.name` は
実質常に空 — `filepath.Base(WorkDir)` フォールバックのみが effectively
効く。peer にも `meta.name` を効かせるには `Runner` に
`orchestrator.MetaHydrator` 相当の依存を足す必要があり、これは
post-cutover 改善候補として先送り (self project 側の name 衝突解消が本
refactor の主目的で、peer 側はディレクトリ命名の一貫性向上に留まるため)。
