...
ステータス: 完了 (PR1〜PR8 全て main 済み、2026-07-14)
作成日: 2026-07-09
...
- clone 先はホスト側 runtime dir の bind mount、sandbox 内は中立 path `/workspace`
  (2026-07-13 追記: `/workspace` は後に親 dir 化され、実際の clone 先は
  project ごとの `/workspace/<name>`。「post-cutover 改善候補 §4」参照)
...
- sandbox_builder: worktree / project bind (`projectVisibilityMounts` step1/3/6/7)
  ...
  cd 先も `/workspace` (2026-07-13 追記: 後に `/workspace/<name>` へ親 dir 化。
  「post-cutover 改善候補 §4」参照)
...
- broker の cwd 検証 root を `/workspace` に変更 (`broker.go:531-546`、
  2026-07-13 追記: 後に `/workspace/<name>` へ親 dir 化。値は
  `TokenContext.SandboxRoot` 経由なので broker.go 側は自動追随)、
...
### PR8: 大掃除 (cutover 安定後) — 完了

cutover (PR6) 安定稼働確認後 (2026-07-13) に着手・完了。diff が大きいため
stacked 3 PR に分割して land した:

- **PR8-A** (protocol/broker/policy 削除): `internal/sandbox/git_builtin.go`
  (1071 行) とそのテスト一式、`git_shim.go` (`RunGitShim`、PATH overlay 用)、
  protocol の `GitOp` / `GitRequest` 型、broker.go の git dispatch 分岐と
  `GitBinding` リモートスナップショット機構、policy 登録 (`gitPolicy`,
  `OpGitFetch` 等) を削除。policy_test の wantOps 相当と
  `dispatcher/policy_translate_test.go` の drift test (`TestOpConstantsMirror`)
  も更新
- **PR8-B** (worktree runtime 削除): `worktree_manager` / `worktree_store` /
  `worktree_resolver` / `worktree_model` (dispatcher 4 ファイル)、
  `worktree_lock.go` / `project_lock.go` (`BranchLockManager` /
  `AcquireForTask`、orchestrator)、`TaskWorkflowService.Locks` /
  `Worktrees` フィールドと `releaseProjectLock` / `cleanupWorktree` /
  `sweepChildBranches`、GC の `cleanWorktrees` / `GCResult.Worktrees` /
  `Worktrees` カウント表示、`TokenContext.WorktreeDir`・`WorkspacePeers` の
  path 検証、`Visibility.UseWorktree` を削除
- **PR8-C** (DB migration + docs): `worktrees` テーブルの drop migration、
  `store.go`/`DeleteTask` に残っていた `worktrees` テーブル参照の除去、
  関連 docs の棚卸し
...
**ステータス**: 実装済み (2026-07-13 nose 決定、refactor PR)。

**問題**: 本 doc がここまで書いてきた「clone 先は sandbox 内の中立 path
`/workspace`」は全 project 共通の固定 path だった。これが2つの問題を生む。

1. sandbox cwd (= claude code 等 harness の作業ディレクトリ絶対 path) が
   全 project で同じ `/workspace` になるため、Claude Code の
   `~/.claude/projects/-workspace/` セッションログ slug が全 project 共通に
   なり、session jsonl / memory / skills が project 間で混ざる
2. workspace peer を動的に clone する際、エージェントが置き場所に迷い
   `$HOME` や `/tmp` (どちらも tmpfs、RAM 上) に置きがちで、peer 数が増える
   ほど RAM を圧迫する

**解決**: `/workspace` を「clone 先の親 dir」に意味変更し、実際の clone は
`/workspace/<name>` (project ごとの leaf dir) に着地させる。

```
/workspace/                        ← 親 dir (job-scoped runtime bind、ext4 実体)
  ├── bm-next/                     ← self project の clone (旧: /workspace 直下)
  └── bm-next-lp/                  ← workspace peer の clone (エージェントが動的配置)
```

**`<name>` の決定則**: `project.yaml` の `meta.name` (kebab-case 前提、
未強制)。空なら `filepath.Base(WorkDir)` にフォールバック
(`dispatcher.projectDirName`)。

**self project の name 解決経路**: `orchestrator.Visibility.ProjectName` —
`orchestrator.PlanHook` / `dispatcher.BuildSessionJobSpec` が JobSpec 構築時に
workspace-hydrated `ProjectMeta.Name` から詰める。dispatcher 側
(`Runner.Dispatch`) では二重に解決しない — `dispatcher.Runner.Projects`
(`orchestrator.DBProjectCatalog`) は `SELECT ... FROM projects` のみで
`ProjectMeta` を一切 hydrate しないため、そちら経由での解決は常に空になる
落とし穴がある (`cloneDirNameForVisibility` のコメント参照)。

**workspace peer の name 解決経路**: `dispatcher.buildPeerAdvertise` が
`environment.yaml` の `workspace_projects[].clone_dir` に
`/workspace/<peer-name>` を advertise する。peer 側は `Runner.Projects`
(`orchestrator.DBProjectCatalog`) 経由でしか project を引けず `meta.name` は
常に空だったため、当初は `filepath.Base(WorkDir)` フォールバックのみが
effectively 効いていたが、**実装済み** (post-cutover 改善候補 §4 残タスク 1,
refactor PR): `Runner` に `orchestrator.MetaHydrator` 依存 (`Runner.Hydrator`)
を追加し、`buildPeerAdvertise` が `GetWithWorkspace` 経由で peer の
`meta.name` を解決するようになった。self project 側と対称。`Runner.Hydrator`
が nil、または個別 peer の hydration が失敗した場合は fail-soft で
`filepath.Base(WorkDir)` フォールバックに degrade する (advertise は
best-effort — peer 1 件の hydration 失敗で job 全体を止めない)。

**advertise field 名**: `clone_dir` (絶対 path)。`environment.yaml` の
`workspace_projects[]` 各 entry、および self project 側は
`filesystem.clone_dir` に出す (`filesystem.project_dir` は host 側 path の
ままなので別 field が必要)。

**影響範囲**: コアは `sandboxCloneTargetDir` 定数経由に集約済みだったため
少ない。`internal/dispatcher/sandbox_builder.go`
(`sandboxCloneDir`/`projectDirName`/`cloneDirNameForVisibility` 追加、
`cloneMounts`/`resolveWorkDir`/`buildCloneSpec`/argv remap/environment.yaml
の各消費箇所を更新)、`internal/dispatcher/gitgateway_wire.go`
(`buildPeerAdvertise` に `CloneDir` 追加)、`internal/orchestrator/jobspec.go`
...
- ~~workspace peer の `meta.name` サポート~~ — 実装済み (上記参照)。
