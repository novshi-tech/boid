...
7. **`~/.boid` tmpfs overlay は当初「本 phase で退役」の想定だった
   (2026-07-20 当初決定)**。 Phase 4 のステップ 5 完了前の衝突対策
   (workspace home bind の上に `$HOME/.boid` だけ tmpfs) は、 context
   ファイル書き / attachments bind が両方消えれば不要になる想定だった。
   退役の順序は「新 CLI が着地 → スキル / adapter bootstrap 側の CLI 化 →
   context ファイル書き / tmpfs overlay 撤去」。
   **改訂 (2026-07-21、5b-6 PR #803 codex review)**: 実際に撤去したところ
   `$HOME/.boid/output/payload_patch.json` (job_done の file fallback、
   決定 6 で 5b-7 まで残す前提のもの) が workspace 内の並行 job 間で
   共有される固定パスになり、 defensive cleanup で塞ごうとしたら
   fail-open・祖先 symlink escape の 2 つの実 vulnerability を新たに
   生んだ。 tmpfs overlay は撤去せず、 5b-7 (`job_done` payload-patch RPC
   直渡し) が file fallback を完全に不要にするまで維持する方針に変更。
   詳細は「PR 分割案 > 5b」6 の landed 注記を参照。
   **再改訂 (2026-07-21、5b-7 PR #804)**: 上記の想定 (5b-7 が file fallback を
   「完全に不要にする」) は実装時に外れた — 5b-7 のスコープは「新経路の並走」
   であり、 file fallback (と、 それを隔離する tmpfs overlay) は 5b-7 後も
   意図的に残る (5b-7 自身の PR 本文に明記)。 file fallback の完全撤去、
   ひいては tmpfs overlay の撤去は Phase 6 (enforcement backend swap) 前後に
   先送りされたままで、 5b-7 はその条件を満たしていない。 「PR 分割案 > 5b」7
   の landed 注記を参照
...
- サンドボックス内 `~/.boid/context/*` ファイル配布と `~/.boid/attachments/` bind
  は廃止。 **tmpfs overlay は 5b では撤去しない** (決定 7 改訂、5b-6 PR #803
  codex review — `payload_patch.json` の file fallback が並行 job 間で
  安全に隔離される必要があるため。 5b-7 (PR #804) は payload-patch RPC 直渡し
  を新経路として追加したが file fallback 自体は意図的に維持したため、
  Phase 4 完了時点の暫定オーバーレイが完全に消えるのは file fallback を
  一括退役する Phase 6 前後まで先送りされている — 決定 7 の再改訂参照)
...
6. **file 配布と attachments bind の撤去 + tmpfs overlay 撤去**。 `contextFiles` から
   ...
   **(landed, PR #803)**: `contextFiles`/`buildEnvironmentYAML`/
   `marshalTaskYAML`/`marshalInstructionsYAML`/`EnvironmentInput`/
   `attachments_path.go` を削除 — context file 配布 (task/instructions/
   environment/payload) と attachments RO bind はどちらも完全撤去。
   ...
   Web UI の attachments 参照 (paste-attach JS + templ 2 枚) も
   `~/.boid/attachments/<name>` パス表記から `[attachment: <name>]` +
   `boid task attachments get <name>` 表記に書き換え (bind 撤去に伴う実害の
   事前防止)。
   **`$HOME/.boid` job-scoped tmpfs overlay (decision 7) は撤去しなかった**:
   初回実装では workspace-home-bind 分岐から撤去し `payload_patch.json` の
   job 開始時 defensive cleanup (`sandbox.Spec.RemoveFiles`) で代替したが、
   PR #803 の codex review が Blocker (並行 job 間の payload_patch.json
   混信・消失) + Major 2 件 (cleanup 失敗の fail-open、 永続 HOME 上の祖先
   symlink 追跡による意図しない project への書き込み escape) を指摘。
   defensive cleanup を harden するのではなく、 tmpfs overlay をそのまま
   復元して `RemoveFiles` 機構ごと削除するほうが根本的に安全 (job ごとに
   独立した tmpfs mount には共有される host path が原理的に存在しないため、
   並行 job 間の相互干渉も祖先 symlink escape も構造的に不可能になる) と
   判断し、 5b-7 (`job_done` payload-patch RPC 直渡し) が file fallback を
   完全に不要にするまで維持する方針に変更。 `e2e/scenarios/
   workspace-home-boid-isolated` は改名・書き換えせず元の「job ごとに隔離
   される」契約のまま維持、 並行 job の非干渉性は
   `TestHomeMounts_ConcurrentJobsGetIndependentBoidTmpfs`
   (`internal/dispatcher/sandbox_builder_test.go`) で unit level に pin。
   `BOID_INSTRUCTIONS` env は reader ゼロを確認した上で同 PR で撤去済み
   (下記未解決論点)
7. **`job_done` の payload_patch 直渡し版 RPC + CLI** (`boid task update
   ...
   CLI 経路を一次に
   **(landed, PR #804)**: 新 broker op `BoidOpTaskUpdatePayloadPatch`
   (JobID スコープ、`task_instructions`/`env`/`payload` と同パターン) +
   `api.TaskAppService.UpdateTaskPayloadPatch` を追加。 既存 `--payload-file`
   (`BoidOpTaskUpdate` → top-level shallow merge) とは別 op として分離し、
   `orchestrator.MergePayloadPatch` を直接呼んで job_done/hook 完了パイプライン
   (`Coordinator` の `HandlerResult.allowedTraits`) と同じ trait-mode-aware
   merge を即座に適用する。 allowedTraits は呼び出し job の `HandlerID` から
   firing hook の `traits.produces` を逆引きして導出し、 hook が見つからない
   場合 (project meta 無し・behavior 無し・hook 未宣言) は unrestricted merge に
   フォールバックする — これは `orchestrator.synthesizeAgentHook` が生成する
   virtual/synthesized agent hook (`Traits` がゼロ値) の実際の挙動と一致する
   ため、明示 `hooks:` を持たないプロジェクト (boid 自身の `.boid/project.yaml`
   含む) でも意味論が壊れない。 file 経路 / `$HOME/.boid` tmpfs overlay は
   **意図的に維持** — decision 7 が当初想定していた「5b-7 で file fallback が
   完全に不要になる」は外れており (決定 7 の再改訂を参照)、 file 経路の一括
   退役は Phase 6 前後に先送りされたまま。 `boid-task/SKILL.md` の
   *Writing the final report* / release 報告の 2 箇所を `--payload-patch` 一次に
   書き換え、 `references/builtins.md` に新フラグの意味論 (trait gate 込み) を
   文書化。 `boid-orchestrate/SKILL.md` は payload_patch を一切書かないため
   変更なし (確認済み、スコープ外ではなく「該当箇所が無かった」)。
   wiring-seams.md #17 (payload_patch direct-pass merge parity) を新設
...
- **`BOID_INSTRUCTIONS` env の廃止判定**: **解決済み (5b-6 codex review Minor 1
  ...
  即時撤去。 `internal/dispatcher/sandbox_builder.go` の env セット箇所を削除、
  `boid task instructions` RPC が (env ミラー無しの) 唯一の経路になった
