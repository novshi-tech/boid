...
- `AllowedRegistries` 実装 (`POST /images/create` / pull、必要に応じて)
...
3. **image build の扱い** — 決着済み: サンドボックス（proxy）では image build を
   一切許可しない（`POST /build`・`POST /session` を deny）。BuildKit の gRPC トンネルは
   検査不能なため fail-closed で拒否する。build が必須のプロジェクトは host_commands で
   `docker build` サブコマンドのみを opt-in する（ホスト直実行のリスクを承知の最終手段）。
