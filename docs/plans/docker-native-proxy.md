...
type DockerPolicy struct {
    ...
    // AllowedDevices: 許可するデバイスパスのプレフィックスリスト。空は全 deny。
    AllowedDevices []string `yaml:"allowed_devices"`

    // AllowedRuntimes: 許可するコンテナランタイム名。空は既定 (runc) のみ許可。
    AllowedRuntimes []string `yaml:"allowed_runtimes"`

    // AllowSecurityOpt: seccomp/apparmor の unconfined や no-new-privileges 無効化など
    // 危険な SecurityOpt を許可するか。既定 false（= 危険な値は全 deny）。
    AllowSecurityOpt bool `yaml:"allow_security_opt"`

    // AllowHostNamespaces: NetworkMode/PidMode/IpcMode/UsernsMode/CgroupnsMode の
    // host 共有を許可するか。既定 false。
    // （AllowHostNetwork は後方互換のため残すが、本フラグに統合してもよい）
    AllowHostNamespaces bool `yaml:"allow_host_namespaces"`

    // AllowedRegistries: docker pull / FROM で許可するレジストリのプレフィックスリスト。
    // 空は制限なし（将来拡張）。
    AllowedRegistries []string `yaml:"allowed_registries"`

    // MaxBodyBytes: 検査対象エンドポイントのリクエストボディ読み取り上限。
    // 超過したリクエストは deny する（巨大ボディによる DoS 対策）。0 は既定値を使う。
...
capabilities:
  docker:
    ...
    allow_host_namespaces: false  # network/pid/ipc/userns/cgroupns の host 共有を deny
    allow_security_opt: false     # seccomp/apparmor unconfined 等を deny
    allowed_cap_add: []           # capability 追加は全て deny
    allowed_devices: []           # デバイス追加は全て deny
    allowed_runtimes: []          # 既定 (runc) 以外のランタイムを deny
```
...
検査方法: `Type == "volume"` で `VolumeOptions.DriverConfig` がある要素は、
`Options` の `device` を host パスとして `AllowedBindPaths` と照合し、
`o` に `bind` が含まれる場合は特に厳格に扱う。同様の検査を
...
`allowed_bind_paths` を `$WORKTREE` のみ（既定推奨）にすると、Ryuk の
`docker.sock` bind が deny され、**TestContainers がそのまま壊れる**。

対応のいずれかを Phase 1 で必ず実装する:

- **(推奨) `TESTCONTAINERS_RYUK_DISABLED=true` を sandbox 環境変数に自動設定** する。
  サンドボックスは task 終了時に boid が破棄するため、Ryuk による reap は不要。
- もしくは Ryuk に **proxy socket を bind させる**（Ryuk が叩くのも proxy 経由になる
  ようにし、Ryuk の `/var/run/docker.sock` mount を proxy socket へ向ける）。

どちらを採るにせよ、「Phase 1 完成と同時に TestContainers が動く」ことを E2E で担保する。
...
- `docker-proxy-testcontainers`: TestContainers ベースのテストが Ryuk 込みで正常完走する
...
- **TestContainers (Ryuk) 互換性** を担保（環境変数 or Ryuk への proxy socket 受け渡し）
