...
A (executor) → done → A の PR を base に merge (root or sub sup が gh API)
              ...
              で boid/<subid8> を更新 (host 側 git なので readonly でも通る)
                         ↓
              sub sup が B (depends_on A) を dispatch → B の worktree が
...
なお現状の boid コアには `depends_on` / `ComputeTaskBlocked` / `task_dependencies`
DB schema 等の依存関係制御機構が残存している (廃止予定だが未着手)。 本計画は
コア依存制御に依存しない設計なので、 コア廃止計画は別計画で進める (本計画の前提
条件ではない)。
...
関連: boid コアの `depends_on` / `ComputeTaskBlocked` / `task_dependencies` DB
schema 等は廃止予定だが、 本計画のスコープ外。 別計画で扱う (本計画はコア依存
制御に依存しない設計)。
...
1. **boid コアの依存関係制御 (depends_on) 廃止**: `ComputeTaskBlocked` / DB schema
   (`task_dependencies` 表) / API パラメータ / docs を順次廃止。 本計画はコア依存
   制御に依存しない設計だが、 残存している以上、 別計画で整理する。 廃止前に
   sub-sup が依存関係をコアに頼らず扱えることを確認する。
