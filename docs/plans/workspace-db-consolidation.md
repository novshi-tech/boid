...
ステータス: **完了 (PR1-PR7 landed、2026-07-17)** — Phase 2.5 完結。残作業は
`AdditionalBindings` フィールド撤去 (PR8) のみで、Phase 4 (HOME 契約先行) に持ち越し
作成日: 2026-07-16
更新日: 2026-07-17 (PR7 landed: `WorkspaceMeta.Kits` 撤去、Phase 2.5 完結)
...
| PR7 | **`WorkspaceMeta.Kits` フィールド削除**、`Kits` を含む spec loader / dispatcher 経路のクリーンアップ (~500 行想定)。旧 workspace yaml / kits/ ディレクトリ削除 (次リリースで) — **landed** (2026-07-17)。詳細は本節末尾の footnote 参照 | PR6 |
...
### PR7 landed 記録 (2026-07-17) — Phase 2.5 完結

**核心判断**: `boid workspace assign` の auto-create 経由 (`ensureWorkspaceExistsForAssign`) で
`kits:` を書いた手書き/e2e フィクスチャ shadow yaml から DB row を作る e2e シナリオが
30 本近くあり (`host-command-smoke` / `docker-proxy-*` / `git-gateway-*` 等)、
これらは全て「実在する kit ディレクトリを名前で解決する」正当なユースケースだった
(PR6 の「影響なし」確認はまさにこの経路)。そのため `WorkspaceMeta.Kits` フィールド
自体は削除しつつ、`kits:` の**解決能力そのもの** (`MaterializeWorkspaceKitsForPersist`)
は残し、呼び出し元をクライアント側 (daemon の wire を経由しない) に限定する設計にした。

- **撤去**: `WorkspaceMeta.Kits []string` フィールド (`workspace_meta.go`)。
  `workspaceMetaStrict` からも `Kits` を削除 — `POST`/`PUT`/`import /api/workspaces`
  は `kits:` を含む body を strict decode で 400 reject するようになった
  (decision 12: wire 上のフォールバックは残さない)
- **変更**: `MaterializeWorkspaceKitsForPersist` のシグネチャを
  `(kitsDir string, meta *WorkspaceMeta) error` (旧: `meta.Kits` を読んで
  クリア) から `(kitsDir string, kitRefs []string, meta *WorkspaceMeta) error`
  (kitRefs は呼び出し元が明示的に渡す) に変更。**(MINOR 1、codex review 訂正)**
  「呼び出し元は 2 箇所」と書いていたが、これは「kit 解決ロジックが使われる
  経路が 2 箇所」という意味であって、`MaterializeWorkspaceKitsForPersist` と
  いう公開 helper 自体のシンボルレベル callsite は実際には 1 箇所のみ
  (`cmd/workspace.go`)。もう一方の経路は同じロジックを内部で担う下位
  helper (`materializeKitRuntimeIntoWorkspace`) を直接呼んでおり、
  `MaterializeWorkspaceKitsForPersist` 自体は経由しない:
  - `cmd/workspace.go` の `ensureWorkspaceExistsForAssign` — 新設した
    `extractLegacyWorkspaceKitRefs` で shadow yaml の raw bytes から `kits:` を
    別途抽出し (strict validation は kits 抜きの残りに対して実行)、
    `MaterializeWorkspaceKitsForPersist` で materialize してから (kits を
    含まない) body を POST する
  - `internal/orchestrator/workspace_migration.go` の
    `preflightWorkspaceMigration` (一度きりの cutover migration) — 新設した
    `readWorkspaceYAMLSnapshot` で各 workspace yaml の raw bytes を一度だけ
    読み、meta と `kits:` の両方をその単一スナップショットから decode する
    (当初 `legacyWorkspaceYAMLKits` として `yamlStore.Load` とは別に 2 度目の
    read を行う実装だったが、2 つの独立した read の間で atomic rename が
    割り込むと「旧 meta + 新 kits」のような実在しない組み合わせを拾いうる
    TOCTOU があったため、codex review round 1 で単一 read に統合した——
    `WorkspaceMeta` に `Kits` フィールドが無いため元々 `yamlStore.Load` 経由
    では `kits:` を拾えない、という制約自体は変わらない)。
    `MaterializeWorkspaceKitsForPersist` を経由せず
    `materializeKitRuntimeIntoWorkspace` を直接呼ぶ (kit.yaml の snapshot を
    全 workspace で使い回すため、1 回の `snapshotAllKitYAMLs` 呼び出しを
    こちらで済ませている)。crash recovery の input hash にも
    `WorkspaceKitRefs map[string][]string` を追加し、`kits:` だけの変更が
    hash に反映されない回帰を防いだ
- **撤去**: `internal/api/project_service.go` の `CreateWorkspace` /
  `UpdateWorkspace` / `ImportWorkspace` 内の `MaterializeWorkspaceKitsForPersist`
  呼び出しと `ProjectAppService.KitsDir` フィールド — wire body がもう `kits:`
  を運べないため常に dead だった。`internal/server/wire.go` の
  `KitsDir: cfg.KitsDir` 配線も削除 (`cfg.KitsDir` 自体は
  `MigrateWorkspaceYAMLToDB`/`LoadHostCommandsFromKits` で引き続き使用)
- **簡素化**: `cmd/project_migrate.go` の `mergeLegacyFieldsIntoWorkspace` から
  `.Kits` union ロジックを撤去。migrate 自身が生成する legacy kit
  (project.yaml 自身の `host_commands`/`additional_bindings` を同梱) は、
  kit ディレクトリ経由の再解決を経ずに workspace の対応フィールドへ
  **直接** 畳み込むよう変更 — このデータは migrate の時点で完全に既知
  (project.yaml 自身の値) なので、そもそも kit round-trip が不要だった。
  legacy kit.yaml のディスク書き込み自体は継続 (host_commands.yaml 集約
  レジストリへの登録用途で必要)。`materializeWorkspaceMetaForOfflineWrite`
  ヘルパは不要になったため削除、online/offline 両パスとも
  `plan.workspaceMeta`/`merged` をそのまま使うよう統一
- **判断**: legacy project.yaml の外部 kit 参照 (`kitRefStrs`、
  top-level/behavior-level `kits:`) は名前検証のみ行い、
  workspace への自動解決は撤去。dry-run/apply 出力に informational な
  note として表示するのみ (decision 12 の精神: 退役済み機構への
  ad-hoc な再解決は追加しない)
- **撤去**: `KitResolver` interface (`spec_loader.go`)、
  `ProjectStore.resolver` フィールド、`NewProjectStore(resolver KitResolver)`
  の引数 (→ `NewProjectStore()`)。`ReadProjectMetaWithKits` からも
  `resolver KitResolver` 引数を削除 (元々未使用だった)
- **撤去**: `orchestrator.KitMeta` / `KitMetaInfo` struct (`spec_types.go`) —
  grep で確認した結果コメント以外に参照ゼロの dead code だった
  (`readKitRuntimeRaw` は別の内部型 `kitRuntimeRaw` を使っており無関係)
- **撤去 (Step H)**: `internal/config` の `DefaultHarness()` /
  `SetDefaultHarness()` / `ValidateHarnessName()` / `ErrDefaultHarnessNotSet` /
  `EnvDefaultHarness` および `Config.DefaultHarness` フィールド — PR6 で
  flag された通り呼び出し元ゼロを再確認した上で削除。ジェネリックな
  `WriteFileAtomic` (host_commands.yaml の永続化にも使われている) は
  `internal/config/atomic_file.go` に切り出して存続
- **テスト**: `TestProjectMigrate_ExistingWorkspaceYAML_Merge` /
  `TestProjectMigrate_BehaviorKitsAggregated` は `.Kits` 依存部分を削除して
  Env/project.yaml 側の assertion のみ残した。
  `TestCreateWorkspace_AcceptsKitDerivedHostCommand` /
  `TestProjectAppService_CreateWorkspace_MaterializesKits`
  (`internal/api/workspace_service_test.go`) は server 側 kit materialize
  の廃止に伴い削除
- **確認済み**: ローカル e2e 40 シナリオ全 pass (`kits:` 使用シナリオ含む)、
  `go build/vet/test -race` 全 pass
