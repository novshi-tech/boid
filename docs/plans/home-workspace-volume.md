...
- **embedded skills sync は add/update のみ、削除・改名の反映は未実装** (2026-07-17 codex 指摘、
  PR #789 review): boid 更新で embedded skill の name set が変わったとき、workspace HOME の
  古い skill dir は残り続ける。 harness は古い SKILL.md を発見できてしまう。
  現状の実装挙動は operational な想定範囲 (skill 削除は稀、残っても security 破綻でなく古い挙動)
  として PR3 では skip したが、embedded skill manifest を持つ形での exact sync 実装を Follow-up
  で行う。 非 embedded の sibling skill (ユーザが手動で置いたもの) は保持する必要があり、
  managed vs 非 managed の識別が実装の要点。
- **workspace remove と dispatch の lifecycle 競合は未対応** (2026-07-17 codex 指摘、
  PR #791 review): 現状 `boid workspace remove` は非 `--force` でも常に prompt
  することで最低限の防御は行うが、daemon 側で workspace 単位の lifecycle
  lock/refcount は実装していない。 in-flight job がある workspace の remove
  は「DB row 削除 → stale dispatch が home 再作成 → orphan 残る」というレースが
  残る。 完全対応には daemon 側で workspace lock を dispatch/remove 間で共有し、
  active job がある workspace の remove を拒否 or 待機させる機構が必要
  (Phase 5 以降 or 別 PR で対応)。
