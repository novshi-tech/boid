...
ステータス: **draft (2026-07-24 作成、実装未着手、 codex review round 1〜6 反映済み)**。
...
dispatch は `ready` のみ許可 (`reserving` を含む他 4 status は dispatch 対象外)。 workspace 側にも
...
- 一意性 verify (予約 tx の COUNT) は `reserving` row も占有として数える (それが予約の目的)。
  stale な `reserving` は上記 2 経路の sweep で解消される
...
- `boid project rm <id>` → **DB row 削除を先行** (新規 dispatch を即時遮断) → repo cache dir 削除は
  **exclusive lock 取得後** = 進行中 job の shared lock 解放 (job 終了) を待つ。 **進行中 job の強制
  中断は行わない** (round 6 Major 5 対応で「進行中 fetch/dispatch 中断」の旧記述を撤回 — rm は
  exclusive、 job は終了まで shared を保持するので、 lock 契約上の実挙動は「job 終了待ち」が正。
  job の中断は既存の job cancel 機構の領分で、 rm には持ち込まない)
...
**status 転記先 (round 6 Major 3 対応 — workspace 側の保存先を新設)**: `applyEntryStatus` の転記先は
`target_kind` で分かれる:
...
に戻して staging を破棄) を実行した後、 `closeJournal` 前に crash すると、 次回 recovery では
...
func recoverAtomicReplace(entry *JournalEntry) (outcome string, err error) {
    ...
    case step.BackedUp || (!fileExists(entry.TargetPath) && fileExists(entry.BackupPath)):
        ...
        // (round 6 Blocker 1: restore 実行後 closeJournal 前に crash しても、 次回実行は
...
- §a: auto-prune 撤去 + 5-status (reserving/ready/degraded/apply-pending/apply-error)、 reserving の
  sweep は in-process registry 判定 + 確定 tx の affected-row 検証 (round 6 Major 1 で時間 heuristic
  撤回)、 additive migration + **`work_dir` column は deprecated 凍結 (新 rows は `''`、 URL は既存
  ...
  `project rm` は「新規 dispatch 即時遮断 + 進行中 job は終了待ち」に契約訂正 (round 6 Major 5)、
...
- §g: apply_journal 完成版 (operation catalog 9 種の表 + 全 case の restart 規則、 filesystem_step
  ...
  apply-pending / reserving の startup 一括転記
...
現時点で未対応の Minor は無し (round 6 の 4 件も本文 fix 済み: `--run` の entrypoint override /
sqlite3 package 前提の明記 / audit の実処理行訂正 / phase 3 記述の op 種別限定)。
...
### Round 6 (Blocker 2 / Major 5 / Minor 4) → Round 7 draft (本 doc)

round 6 で round 5 の B1/B3/B4 + M1/M4〜M9 は解消と判定済み。 残 B2 (recovery 冪等性の別角度) と
新規指摘への対応:

**Blocker 2 件本文 fix**:
- 1: atomic replace recovery の再実行が restore 済み旧 content を `recovered-ok` と誤分類 → §g で
  `filesystem_step` に `recovery_restored` marker を追加。 restore 経路に入ると決めた時点で
  (filesystem に触る**前**に) marker を永続化し、 再実行は最優先でこの marker を check して restore
  経路 (`restoreAndFail`、 冪等) に固定。 「recovery は outcome 分類も含めて冪等」に契約を強化
- 2: `work_dir` の意味論反転が非目的と矛盾 → §a で契約確定: `work_dir` は **deprecated 凍結**
  (新 rows は `''`、 意味論の転用はしない、 読み手は PR-3 refactor で全廃、 DROP は scope 外の将来
  cleanup)。 URL は既存 `upstream_url` column に保存 (新 column 無し)、 `ProjectRef.CachePath` は
  runtime derive で DB に持たない

**Major 5 件本文 fix**:
- 1: GC reaper が 1h 超の正当な clone を殺す → §a で時間 heuristic を撤回、 in-process
  `activeReservations` registry 判定に変更 (registry に無い reserving row のみ残骸として sweep)。
  確定 tx は `WHERE status='reserving'` + affected-row 検証で sweep 競合を fail-loud
- 2: identity 再検証が path/SHA のみで ABA を通す → §b の再検証に DB re-load by project ID
  (存在 + ready + revision 一致) を追加。 rm→同名再登録は新 ID で検出、 ChangeURL は revision bump
  で検出
- 3: workspace recovery status の保存先が無い → §g で status 転記先を target_kind 別に確定:
  workspace は additive migration (`workspaces.status`/`status_reason` 追加、 0030 に column が無い
  事実を明記) + apply-error workspace は dispatch block、 hostcommands は journal 記録のみ
- 4: `Register` の呼び出しと interface signature 不一致 → `Register(ctx, ref *ProjectRef) error` に
  統一 (caller が予約 tx 確定値で ref を構築、 ID の権威は DB row、 RepositoryCache は ID を発行
  しない)。 §a 疑似コードも同型に
- 5: `project rm` の「進行中 dispatch 中断」と lock 契約の矛盾 → §a CLI 契約を「DB row 削除先行で
  新規 dispatch 即時遮断 + cache dir 削除は exclusive lock 取得後 = 進行中 job の終了待ち、 強制
  中断はしない」に訂正

**Minor 4 件全対応**:
- 1: `--run` helper に `--entrypoint /bin/sh` override を追加 (daemon service の entrypoint が
  `/usr/local/bin/boid` のため)
- 2: break-glass の sqlite3 前提 → runtime image への package 追加 (PR-2) を手順に明記
- 3: `.WorkDir` audit の `container_backend.go:945,980` (コメント行) → `990-992,1006` (実処理行) に訂正
- 4: phase 3 総括の「各 op が temp + fsync + atomic rename」→ 置換系 op に限定する記述に修正
