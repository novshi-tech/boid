...
- **daemon 起動時に filesystem / remote の観測結果を根拠に DB row を hard delete しない**
...
`--reference` で bare repo と objects DB を共有できる (同 daemon volume 内、 host mirror `--reference`
の origin/* 追跡ブランチ問題 [[git-gateway-clone-perf-local-mirror-idea-rejected]] は発生しない)。
...
### 決断案 (実装 PR 前に本フェーズで確定要)

3 択:

**案 X: podman を primary target に格上げ** (推奨)
- compose.yml の `/var/run/docker.sock` bind を engine-agnostic 変数化 (`${BOID_DOCKER_SOCK_SRC:-/var/run/docker.sock}`)
- deploy-container.sh に engine 検出 + podman 検出時に `/run/user/<uid>/podman/podman.sock` を set
- CI に podman path (`e2e-container-podman` job) を追加、 docker と同 job matrix
- memory 学び pin 2: 「podman rootless は docker rootful のドロップイン代替ではない、 primary target なら CI 経路を明示的に足す」

**案 Y: docker を primary、 podman は best-effort**
- compose.yml は現行 `/var/run/docker.sock` 固定のまま
- podman 対応は operator の環境変数 override or 手動 script fork で
- nose host (podman-only) では別途 host-side 対策要 (podman socket を `/var/run/docker.sock` に symlink 等)
- 影響: 本 doc §論点 h の「実機検証 gate」を PR-4 前に nose host で走らせる際、 別途 podman workaround 要

**案 Z: docker engine を nose host に導入して podman と並存**
- rootful docker daemon + rootless podman の並存 (現行 nose host に多数の podman container あり)
- 本フェーズで最小 diff、 実装 PR は変更なし
- trade-off: rootful docker daemon は system-wide、 nose 判断で「podman rootless の security を維持したい」との衝突あり (session 前半で議論済み)

**推奨: 案 X**。 理由: nose の「podman のほうがセキュリティに優れる、 podman で動かせることは必須」判断 (session 前半) に沿い、 かつ本 doc §論点 h の実機検証 gate が nose host で自然に通せる状態にする。

### 実装 PR 位置づけ (§論点 e の PR 分割との関係)

案 X 採用の場合、 実装は **PR-1 (volume-only compose stack) の中に含める** (compose.yml の engine-agnostic 化と
deploy-container.sh の engine 検出は同 file 変更、 論理的に分離不可)。 CI 対応 (podman job 追加) は
PR-1 の一部 or 別 PR (`ci-podman-matrix.yml` 相当)。

### 未解決論点

- **案 X/Y/Z の選択**: 本 doc landed 前に nose 確定
- **CI podman job の scope**: 案 X 採用時、 docker job と同 scenario 全部を podman でも回すか、 minimal
...
- **論点 i**: 案 X/Y/Z の選択 (Podman を primary target にするか)、 CI podman job の scope

これらは **PR-1 着手前の設計 review round で individual に nose 判断** を得るべきもの (論点 i は特に priority 高、
案 X 採用の場合 PR-1 に engine socket 対応が inline される)。 Phase 7 (k8s) 領域は別 doc で扱う。
