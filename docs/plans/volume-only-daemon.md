...
ステータス: **draft (2026-07-24 作成、実装未着手、 codex review round 1〜7 反映済み)**。
...
- **schema-breaking DB migration** — 本 doc は additive migration のみ許容 (`projects.project_name` /
  `status` / `status_reason` / `revision` と `workspaces.status` / `status_reason` の ADD COLUMN、
  `apply_journal` テーブル追加)。 既存 column の型変更・削除・意味論反転は scope 外
...
ALTER TABLE projects ADD COLUMN status TEXT NOT NULL DEFAULT 'ready';
    -- 'reserving' | 'ready' | 'degraded' | 'apply-pending' | 'apply-error' | 'removing'
...
**既存 column の分担 (round 6 Blocker 2 対応で契約確定 — 意味論反転はしない、 非目的と整合)**:

- **`work_dir` は deprecated 凍結**: 新 rows は `''` (空文字) を書く (`NOT NULL` 制約は additive に
  維持)。 URL を入れる**意味論の転用はしない** (非目的の「既存 column の意味論反転は scope 外」を
  守る)。 読み手は PR-3 の refactor で全廃 — 下記 `.WorkDir` audit の全 consumer が `ProjectRef`
  経由に置き換わるため、 `work_dir` を読む code は残らない。 column 自体の DROP は schema-breaking
  なので本 doc の scope 外 (将来の schema cleanup で別途)
- **URL の保存先は既存 `upstream_url` column** (`0028` で追加済み) — 新 column は追加しない。
  `ProjectRef.UpstreamURL` はこの column から読む
- **`ProjectRef.CachePath` は runtime derive** (`repos/<workspace>/<project_name>.git`)、 DB には
  持たない (workspace/name から一意に決まるため永続化は冗長 + rename 時の二重更新を排除)

**status 値** (6 種、 round 5 Major 2 で `reserving`、 round 7 Major 1 で `removing` を統合):
...
| `removing` | `project rm` 実行中の tombstone | rm が set (新規 dispatch 即時遮断)、 削除完遂で row ごと消滅。 crash 時は startup recovery が削除を再開 |

dispatch は `ready` のみ許可 (`reserving` / `removing` を含む他 5 status は dispatch 対象外)。 workspace 側にも
第 2 の gate がある (`workspaces.status='apply-error'` は dispatch block、 §g の status 転記先参照)。
...
activeReservations.Add(projID)   // in-process registry (round 6 Major 1 対応)、 commit 前に登録
defer activeReservations.Remove(projID)
reserveTx.Commit()

// 2. tx 外で clone + validate (long-running I/O)。 Register は caller 構築の ProjectRef を受ける
//    (round 6 Major 4 対応で §i の signature と統一): ID の権威は予約 tx で insert した DB row で
//    あり、 RepositoryCache は ID を発行しない。 Register は clone + ref.CachePath の充填を行う
ref := &ProjectRef{ID: projID, Name: name, WorkspaceID: workspaceID, UpstreamURL: url}
if err := repoCache.Register(ctx, ref); err != nil {
...
// 3. 確定 tx (短時間)。 WHERE に status='reserving' を含め、 affected rows を検証 (round 6 Major 1):
//    0 件 = 予約 row が並行 sweep 等で消えた異常 → clone 成果物を cleanup して error return
//    (silent success にしない)
res, uerr := db.Exec(`UPDATE projects SET status='ready' WHERE id=? AND status='reserving'`, projID)
if uerr != nil {
    return fmt.Errorf("project add: finalize update: %w", uerr)
}
if n, _ := res.RowsAffected(); n == 0 {
    repoCache.Remove(ctx, ref)  // clone 済み cache を掃除
    return fmt.Errorf("project add: reservation row disappeared during clone (swept?); aborted")
}
```

**`reserving` の lifecycle (round 5 Major 2 + round 6 Major 1 対応)**: transient state を恒久残留
させない。 sweep の判定は**経過時間ではなく in-process registry** で行う (時間 heuristic だと 1h を
超える正当な clone を実行中に殺してしまうため、 round 6 Major 1 で timeout 方式は撤回):

- **in-process registry**: 生きている `project add` request は予約 tx の commit 前に
  `activeReservations` (daemon 内 registry) へ projID を登録し、 完了/cleanup 時に外す。 予約 row を
  作るのはこの daemon process だけ (単一 daemon、 §b) なので registry は権威 — **registry に無い
  `reserving` row は定義上残骸** (前 process の crash 残り or cleanup DELETE 失敗の残り)
- **startup sweep**: daemon 起動時 (§g の apply_journal recovery と同じ startup recovery phase) に
  `status='reserving'` の row を全て DELETE (+ 対応する repo cache の作りかけ dir を削除)。 起動直後は
  registry が空 = 生きている request は存在し得ないので全 row が残骸
- **GC reaper**: daemon 再起動が無い場合でも、 既存 24h GC cycle で「`status='reserving'` かつ
  registry に無い」row を sweep。 実行中の clone は何時間かかっても registry に居るので sweep されない
- **確定 tx の affected-row 検証** (上記疑似コード): 万一 sweep と競合して予約 row が消えていた場合、
  確定 UPDATE が 0 件になるのを検出して fail-loud (clone 成果物も掃除)
- 一意性 verify (予約 tx の COUNT) は `reserving` / `removing` row も占有として数える (name の
  再利用は row 削除後のみ)。 stale な `reserving` は上記 2 経路の sweep で、 stale な `removing` は
  startup recovery の削除再開で解消される
...
- `boid project rm <id>` → **tombstone 方式の 4 step lifecycle** (round 7 Major 1 対応で確定 —
  「DB row 削除先行」だと現行 `DeleteProject` (`project_catalog.go:314-328`) が `jobs.project_id`
  FK NOT NULL (`0021`) のため task/standalone job rows を project row より先に消し、 進行中 job の
  完了通知が `CompleteJob` 冒頭の `GetJob` (`api/workflow_job.go:11`) で not found になって
  lifecycle 更新に到達しない。 rows を job 終了まで生存させる順序に変更):
  1. `UPDATE projects SET status='removing'` — **新規 dispatch を即時遮断** (dispatch gate は
     `ready` のみ)。 project/task/job rows は全て生存したままなので、 進行中 job の完了通知は
     現行 `CompleteJob` 経路がそのまま成立する
  2. repo の **exclusive lock を取得** = 進行中 job の shared lock 解放 (job 終了) を待つ。
     **進行中 job の強制中断は行わない** (round 6 Major 5 — 中断は既存 job cancel 機構の領分)
  3. cache dir 削除
  4. DB 削除を単一 tx で — 現行 `DeleteProject` の依存 rows → project row の削除順をそのまま使用
     (この時点で当該 project の running job は存在しない)
  - crash recovery: `status='removing'` の row は startup recovery (§g と同 phase) が削除を再開
    (冪等: cache dir 削除 + `DeleteProject`。 startup 時は running job が無いので exclusive lock は
    即時取得できる)
- `boid project list` → URL + status 表示
- `boid project fetch <id>` → **new**
- `boid project status <id>` → **new**

### `work_dir` column の凍結と `.WorkDir` audit (round 5 Major 6 対応で機械列挙の完全版に差し替え)
...
**project entity の `.WorkDir`** (上記「既存 column の分担」の凍結契約に伴い、 読み出しを
`ProjectRef` 経由に置き換えて全廃する本体) と、 **`JobSpec.WorkDir`** (project.WorkDir から派生した
job cwd が入る field。 型は変わらないが値の出所が `RepositoryCache.Materialize` の checkout path に
変わる)。
...
| `internal/api/project_service.go:820,823` | upstream URL backfill (service layer、 823 は error-log field) |
...
| `internal/server/wire.go:274,277,1071,1122` | upstream backfill (274、 277 は error-log field) + session dispatch (1071) + exec dispatch (1122) |
...
| `internal/dispatcher/container_backend.go:990-992,1006` | `realized.Workdir` → docker create `WorkingDir` (945/980 はコメント、 round 6 Minor 3 で実処理行に訂正) |
...
再取得直後の **identity/path 再検証** を dispatch flow の必須 step とする (round 6 Major 2 対応で
DB identity 照合を追加 — path/SHA だけだと「窓の間に rm → 同名・同 SHA で再登録」の ABA を通して
しまい、 削除済み project の stale `ProjectRef` で dispatch できてしまう):

1. **DB re-load by project ID**: `projects` row を ID で引き直し、 存在 + `status='ready'` +
   `revision` が snapshot と一致することを確認。 rm → 再登録の ABA は新 row が新 ID を持つため
   旧 ID の lookup 失敗として検出され、 `ChangeURL` 等の構成変更は `revision` bump で検出される
2. cache path が存在すること (`os.Stat(ref.CachePath)`)
3. `git -C <cache> rev-parse --verify <resolvedSHA>^{commit}` が成功すること

いずれか失敗 → 窓の間に対象 repo が削除・移動・GC・再構成された → dispatch を **retryable error**
で fail し、 呼び出し側は fetch (step 1) からやり直す。 再検証が通れば、 以降は shared lock が
rm/mv/GC (いずれも exclusive) を排除するので checkout 完了まで対象は安定。
...
7. **identity/path 再検証** (上記契約): DB re-load by ID (存在 + ready + revision 一致) +
   cache path stat + `rev-parse --verify <SHA>^{commit}`。
...
case "$1" in
    --run)
        shift
        # --entrypoint override が必須 (round 6 Minor 1 対応): daemon service の entrypoint は
        # /usr/local/bin/boid (build/container/compose.yml) なので、 override しないと
        # `boid sh -c ...` が実行されてしまう
        exec "${COMPOSE_CMD[@]}" run --rm --user root --entrypoint /bin/sh daemon -c "$*"
        ;;
    --exec)
        shift
        # exec は running container 内で command を直接実行するので entrypoint の影響を受けない
...
#    前提: runtime image に sqlite3 package を追加する (round 6 Minor 2 対応 — 現行
#    build/container/Dockerfile の package 群に sqlite3 は無い。 PR-2 の compose stack 整備で追加)
./scripts/deploy-container.sh --run \
...
CREATE TABLE IF NOT EXISTS apply_journal (
    ...
    terminal_status    TEXT,                        -- recovery の判定結果 (closeAndApply が completed_at と同時に set)
    error              TEXT
);
...
- **自動 resume はしない**: recovery は「filesystem を一貫状態に復元する」ことだけを行い、 やり残した
  ...
  収束処理なので再実行で必ず収束する
- **close の一元化 + status 転記との atomic 化**: 各 case は outcome を返すだけで、 case 内の
  `return nil` による journal close bypass はしない。 `completed_at` + `terminal_status` の set と
  対象 project/workspace への status 転記は switch の後、 呼び出し側の `closeAndApply` が
  **単一 DB transaction** で行う (round 7 Blocker 1 対応: close と転記の間に crash 窓を作らない)
- **recovery action は全て冪等 — outcome 分類も含めて**: recovery 自体の途中 crash 後に再実行しても
  filesystem 状態と outcome の両方が同じ結果に収束する。 filesystem action は RemoveAll / 条件付き
  Rename / verify-then-set のみ。 atomic 置換系は「recovery が restore を行った」事実を
  `filesystem_step` の `recovery_restored` marker として **restore 実行前に永続化**することで、
  recovery 途中 crash 後の再実行が「restore 済みの旧 content」を swap 完了 (`recovered-ok`) と
  誤分類する経路を塞ぐ (round 6 Blocker 1 対応、 詳細は下記 filesystem_step 契約)
...
for _, entry := range rows {
    ...
    // terminal_status と completed_at は closeAndApply の単一 tx で同時に set されるので、
    ...
    if entry.TerminalStatus != "" {
        closeAndApply(entry, entry.TerminalStatus)
        continue
    }
    ...
    closeAndApply(entry, outcome)
}

// closeAndApply: journal close と status 転記を **単一 DB transaction** で commit する
// (round 7 Blocker 1 対応 — close と転記を別々に行うと、 close 成功後・転記前の crash で
// journal row が recovery 対象外 (completed_at 済み) になり、 workspace が ready のまま
// apply-error が永久に失われる。 同一 tx なら「completed_at 済みなのに status 未転記」という
// 中間状態は DB 上に存在し得ない):
func closeAndApply(entry *JournalEntry, outcome string) error {
    tx, err := db.BeginTx(ctx, nil)  // DSN _txlock=immediate (§a)
    if err != nil { return err }
    defer tx.Rollback()
    // 1. journal close: terminal_status + completed_at
    tx.Exec(`UPDATE apply_journal SET terminal_status=?, completed_at=? WHERE id=?`,
            outcome, now(), entry.ID)
    // 2. status 転記 (target_kind 別、 下記「status 転記先」): outcome が apply-error なら
    //    対象 project/workspace の status='apply-error' + status_reason=outcome
    applyEntryStatusTx(tx, entry, outcome)
    return tx.Commit()
}
...
// 同じ startup recovery phase で §a の status='reserving' sweep と、 status='removing' の
// 削除再開 (§a の project rm crash recovery) も行う。
```

**status 転記先 (round 6 Major 3 対応 — workspace 側の保存先を新設)**: 転記は上記 `closeAndApply`
の **journal close と同一 tx** で行われる (round 7 Blocker 1 — 転記だけが落ちる crash 窓は無い)。
また workspace op の journal entry は phase 2 tx で「開始」記録されるため、 phase 3 のどの時点の
crash でも recovery の scan が拾う (workspace 側に project の apply-pending 相当の fallback 転記が
無くても取りこぼしは生じない)。 転記先は `target_kind` で分かれる:

- `project` → `projects.status` / `status_reason` (§a の 5-status model)
- `workspace` → `workspaces.status` / `status_reason`。 現行 `workspaces` table
  (`0030_add_workspaces_table.sql`) には status column が**無い**ため、 additive migration を追加する:

  ```sql
  -- migration NNNN_workspaces_add_status.sql
  ALTER TABLE workspaces ADD COLUMN status TEXT NOT NULL DEFAULT 'ready';
      -- 'ready' | 'apply-error' の 2 値のみ (workspace に degraded / reserving 相当は無い)
  ALTER TABLE workspaces ADD COLUMN status_reason TEXT NOT NULL DEFAULT '';
  ```

  `status='apply-error'` の workspace への dispatch は **block** (fail-safe: init.sh 等の workspace
  基盤が中途半端な可能性があるため。 §a の project `ready` gate に加わる第 2 の gate)。 再 apply
  成功 (phase 4) で `ready` に戻る
- `hostcommands` → daemon 全域 scope なので status column は持たない。 journal row の
  `terminal_status` / `error` が記録先で、 `boid workspace status` が未解消 (completed だが
  apply-error) の hostcommands entry を表示する。 次回 apply の rewrite で収束
...
```
{"staged": bool, "backed_up": bool, "swapped": bool, "cleaned": bool, "recovery_restored": bool}
  staged            = staging_path (`.new`) への write + fsync 完了
  backed_up         = target → backup_path (`.old`) の rename 完了
  swapped           = staging → target の rename 完了
  cleaned           = backup 削除完了
  recovery_restored = recovery が restore 判定を確定した (restore 実行「前」に書く、 下記)
```

**`recovery_restored` marker (round 6 Blocker 1 対応)**: `backed_up` 経路の restore (backup → target
に戻して staging を破棄) を実行した後、 `closeAndApply` 前に crash すると、 次回 recovery では
「`backed_up` marker + target 存在 + staging 不在」が swap 完了の verify 条件と区別できず、 **restore
済みの旧 content を `recovered-ok` と誤分類**してしまう。 これを塞ぐため、 recovery は restore 経路に
入ると決めた時点で (filesystem に触る**前**に) `recovery_restored: true` を journal に永続化する。
再実行時は他のどの判定よりも先にこの marker を見て restore 経路 (冪等な restore-if-needed →
`apply-error`) に固定される — 旧 content が `recovered-ok` になる経路は存在しなくなる。

```go
func recoverAtomicReplace(entry *JournalEntry) (outcome string, err error) {
    step := parseSteps(entry.FilesystemStep)

    // 0. recovery_restored marker を最優先で check (round 6 Blocker 1 対応):
    //    前回 recovery が restore 経路に入っていたら、 verify 条件の再評価はせず restore 経路に
    //    固定する (restore 済みの旧 content を swap 完了と誤分類しない)
    if step.RecoveryRestored {
        return restoreAndFail(entry)  // 冪等な restore-if-needed → apply-error (下記と同一 body)
    }
    ...
    // (round 5 Blocker 2 の指摘への直接対応)。 recovery_restored=false がここで保証済みなので、
    // この verify が「restore 済みの旧 content」に誤 hit することはない (restore は必ず marker を
    // 先に書いてから行われる)
    ...
    case step.BackedUp || (!fileExists(entry.TargetPath) && fileExists(entry.BackupPath)):
        // 退避済み + swap 未完 → restore 経路。 **filesystem に触る前に marker を永続化**
        // (round 6 Blocker 1: restore 実行後 closeAndApply 前に crash しても、 次回実行は
        // 冒頭の step 0 check で restore 経路に固定される)
        if perr := persistStep(entry, "recovery_restored"); perr != nil {
            return "", fmt.Errorf("atomic replace recovery: persist marker: %w", perr)
        }
        return restoreAndFail(entry)
    default:
        // staged 以前 (target 無傷、 backed_up marker も backup file も無し) → staging 破棄のみ。
        // target を変更しない経路なので marker 不要 (再実行しても同じ分岐・同じ outcome に収束)
        if rerr := removeIfExists(entry.StagingPath); rerr != nil {
            return "", fmt.Errorf("atomic replace recovery: cleanup staging: %w", rerr)
        }
...
// restoreAndFail: 冪等な restore-if-needed + staging 破棄。 recovery_restored marker の永続化後に
// のみ呼ばれる (marker → filesystem の順序が誤分類排除の要)
func restoreAndFail(entry *JournalEntry) (outcome string, err error) {
    if !fileExists(entry.TargetPath) && fileExists(entry.BackupPath) {
        if rerr := os.Rename(entry.BackupPath, entry.TargetPath); rerr != nil {
...
3. phase 3: filesystem operations (journal step 記録は全 op 共通。 **temp + fsync + atomic rename
   なのは置換系 op のみ** — `clone_new` の staging→target rename / `write_initsh` /
   `rewrite_hostcommands`。 `fetch` / `change_url` / `delete_repo` / `create_home_volume` は
   それぞれ冪等 op で temp-rename 方式ではない、 round 6 Minor 4 対応で記述を op 種別に限定)
...
type RepositoryCache interface {
    // Register は caller 構築の ref (ID/Name/WorkspaceID/UpstreamURL は §a 予約 tx で確定済みの値)
    // を受けて bare mirror clone を行い、 ref.CachePath を充填する。 ID の権威は DB row (予約 tx)、
    // RepositoryCache は ID を発行しない (round 6 Major 4 対応で §a の呼び出しと signature を統一)
    Register(ctx context.Context, ref *ProjectRef) error
...
- downgrade の release → acquire 窓では他の exclusive op が割り込み得るため、 shared 再取得後の
  identity/path 再検証 (DB re-load by ID + cache path stat + `rev-parse --verify <SHA>^{commit}`)
  が呼び出し側の契約 (§b、 round 5 Major 3 + round 6 Major 2)
...
- §a: auto-prune 撤去 + 6-status (reserving/ready/degraded/apply-pending/apply-error/removing)、
  reserving の sweep は in-process registry 判定 + 確定 tx の affected-row 検証 (round 6 Major 1 で
  時間 heuristic 撤回)、 additive migration + **`work_dir` column は deprecated 凍結 (新 rows は `''`、 URL は既存
  `upstream_url`、 意味論反転はしない — 非目的と整合、 round 6 Blocker 2)**、 `ProjectRef` +
  `RepositoryCache` 経由 (`.WorkDir` audit は grep 機械列挙の完全版、 round 5 Major 6)、
  `Register(ctx, ref)` signature 統一 (ID の権威は予約 tx の DB row、 round 6 Major 4)、
  `project rm` は `removing` tombstone の 4 step lifecycle (status 変更で dispatch 遮断 → job 終了
  待ち → cache 削除 → DB 削除、 FK/CompleteJob 経路と整合 + crash 時は startup 再開、 round 6
  Major 5 + round 7 Major 1)、 一意性直列化 (DSN `_txlock=immediate` + workspace mutex + tx 分割)
- §b: behavioral subpath probe、 volume ownership 初期化 (init container が 3 static volume を
  ...
  fetch/dispatch lock (2-phase + downgrade 窓の再検証契約 — DB re-load by ID + revision 照合で
  ABA も排除、 round 5 Major 3 + round 6 Major 2)、 bare cache は cleanup 時 checkout dir 単純削除
...
- §g: apply_journal 完成版 (operation catalog 9 種の表 + 全 case の restart 規則、 filesystem_step
  ...
  re-apply へ委譲、 round 5 Blocker 2)、 **`recovery_restored` marker で recovery 再実行の outcome
  誤分類を排除** (round 6 Blocker 1)、 status 転記先の確定 (workspace は additive migration で
  status column 新設 + dispatch gate、 hostcommands は journal 記録、 round 6 Major 3) +
  **journal close と status 転記は `closeAndApply` の単一 tx** (round 7 Blocker 1)、
  apply-pending / reserving / removing の startup 一括処理
...
現時点で未対応の Minor は無し (round 7 の 2 件も本文 fix 済み: 非目的の additive scope に
workspace columns 追記 / audit に backfill error-log field 2 行追記)。
...
### Round 5 (Blocker 4 / Major 9 / Minor 3) → Round 6 draft (1626 lines): 全対応 (c734353 + 697c57a)

### Round 6 (Blocker 2 / Major 5 / Minor 4) → Round 7 draft (1745 lines): 全対応 (b97c683)

### Round 7 (Blocker 1 / Major 1 / Minor 2) → Round 8 draft (本 doc)

round 7 で round 6 の 11 件中 9 件解消と判定済み (M3 未解消・M5 部分解消が下記 B1/M1 として残存)。

**Blocker 1 件本文 fix**:
- 1: workspace recovery の status 転記が journal close と非 atomic (close 成功後・転記前の crash で
  apply-error が永久に失われる) → §g で `closeAndApply` に統合: `terminal_status`/`completed_at` の
  set と target status 転記を**単一 DB transaction** で commit (中間状態が DB 上に存在し得ない)。
  workspace op の journal entry は phase 2 tx で開始記録されるため fallback 転記無しでも取りこぼし
  無しであることも明記

**Major 1 件本文 fix**:
- 1: `project rm` の「DB row 削除先行」が現行 FK lifecycle (`jobs.project_id` NOT NULL →
  `DeleteProject` が job rows を先に消す → 進行中 job の `CompleteJob` が `GetJob` not found) と
  未整合 → §a で `removing` tombstone の 4 step lifecycle に確定: status='removing' で新規 dispatch
  遮断 (rows は全て生存、 完了通知経路は現行のまま成立) → exclusive lock で job 終了待ち →
  cache 削除 → DB 削除 (単一 tx、 現行 DeleteProject の削除順)。 status model は 6 種に、
  crash 時は startup recovery が削除を再開、 一意性 verify は removing も占有として数える

**Minor 2 件全対応**:
- 1: 非目的の additive migration scope に `workspaces.status`/`status_reason` を追記
- 2: `.WorkDir` audit に `project_service.go:823` / `wire.go:277` (backfill error-log field) を追記
