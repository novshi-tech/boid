...
**完了基準と Phase 3-d の位置付け**: Phase 3 本体の完了は **Phase 3-a + 3-b + 3-c
の 3 つが揃った時点** とする。 Phase 3-d (boid-kits `claude-code` キット廃止) は
3-b マージから 1 リリース猶予の互換期間が経過した後に実施する **互換期間後の cleanup
タスク** であり、 Phase 3 本体の完了基準には含めない。 Phase 3-d 完了は plan 全体の
...
- **[Phase 3-b]** boid-kits `claude-code` の `hooks/run-agent.sh` / `additional_bindings`
  ...
  **互換期間の運用** (Phase 3-b マージ後 → Phase 3-d):
  ...
  - kit ファイルは Phase 3-d まで残置、 docs としても残す。
  - 出口判定: 1 週間の nose 観測で問題なければ Phase 3-d で `boid-kits/claude-code/` 全体
