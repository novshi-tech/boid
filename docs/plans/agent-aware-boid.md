...
> - **Phase 3-d (session 概念導入 + shell adapter 1 等市民化 + 起動入口整理)**: 設計確定 (2026-06-19)、 未実装
> - **Phase 3-e (boid-kits claude-code キット廃止)**: 旧 3-d を後送り。 Phase 3-d 着地 + 1 リリース猶予経過後に着手
...
type HarnessAdapter interface {
    ...
    // boid → run-agent (シム) に送る stop シグナル名。 sandbox 側で
    // `trap '' <name>` を組み立てるために露出する。 claude は "USR1"。
...
> 注: 上記 6 メソッドは Phase 2 で実装された (`StopAgent` / `StopSignalName` /
> `ResumePayload` / `Interactive` / `SessionIDFromHookEnv` / `Usage`)。 Phase 3-b で
> これらは `Run(ctx, RunContext) (Result, error)` 1 本に統合され、 内部 helper として
> Run の中に閉じられる (設計判断 12 参照)。

このスケッチは実コード漏出 9 箇所 (プロトコル層 7 + 実行特性層 2) を全てカバーする
想定:

- プロトコル層 (SIGUSR1 系 7 箇所) → `StopAgent` + `StopSignalName` で吸収 (boid
  コアは StopAgent を呼ぶ / sandbox 側は StopSignalName を読むだけ、SIGUSR1 規約は
  adapter 内に閉じる)
...
- **プロセスツリーの大幅圧縮**: `daemon (goroutine) → boid runner-outer → pasta →
  boid runner-inner → boid runner-inner-child (L3 clone) → agent`。 daemon を除いた
  プロセス数で **現状 9〜10 → 5** (詳細な内訳は Phase 3 セクション「目標プロセス
  ツリー」参照)
...
階層数削減 (9〜10 → 5) 以上に、 bash 3 本 + python シムの **全廃** が本質。 boid-kits の
`claude-code` キット (`additional_bindings` + hooks) 廃止が射程に入る (Phase 3-e、 旧 3-d)。
...
**目的**: 真の harness plug-in を達成する。 sandbox 起動経路 (現状 9〜10 段) を 5 段に
...
**完了基準と Phase 3-d / 3-e の位置付け**: Phase 3 本体の完了は **Phase 3-a + 3-b + 3-c + 3-d
の 4 つが揃った時点** とする。 Phase 3-e (boid-kits `claude-code` キット廃止) は
3-d マージから 1 リリース猶予の互換期間が経過した後に実施する **互換期間後の cleanup
タスク** であり、 Phase 3 本体の完了基準には含めない。 Phase 3-e 完了は plan 全体の
Status から外し、 互換性方針の表で追跡する。

> **Phase 3-d / 3-e の順序確定 (2026-06-19)**: 旧計画では Phase 3-d = kit 廃止だったが、
> ジョブ起動経路の整理セッションで「ProjectCommand / BehaviorCommand 廃止 + shell adapter
> 1 等市民化 + WebUI/CLI 入口整理」 という新スコープを **kit 廃止より先に** 入れる必要が
> 判明した。 理由は、 旧 `boid exec` / WebUI Commands ボタンが kit の `additional_bindings`
> 経由で claude バイナリにアクセスしていたため、 kit を先に消すと exec 経路が claude バイナリ
> に届かなくなるため。 旧 3-d を 3-e に後送りし、 新 3-d をここに挿入した。
...
階層数は **daemon を除いた agent までのプロセス数** でカウントする (daemon は
ホストの常駐 goroutine なので「サンドボックス起動経路」の長さからは外す)。
...
現状 (TTY なし、 9 段。 `bash -lc` 省略):
[daemon] → bash outer.sh → pasta → [bash -c] → unshare → bash setup.sh
         → unshare → bash inner.sh → python run-agent.py → claude

理想 (5 段、 TTY 有無で同形):
[daemon] → boid runner-outer → pasta → boid runner-inner
         → boid runner-inner-child (L3 clone) → claude
```

削減効果は階層数 (9〜10 → 5) 以上に、 bash 3 本 + python シムの **全廃** にある。
...
6. `runner-state.json` dump 配線 (設計判断 10)
7. broker 直呼び (`internal/sandbox/brokerclient/` 切り出し、設計判断 9)

**互換期間なし**。 旧 bash 経路 + 新 go runner の並走は持たず、 一気切替。 障害時は
`git revert <PR-merge-sha>` で旧経路復活 (設計判断 11)。 全 7 種の変更を 1 PR にまとめる。
...
**スコープ外**: boid サンドボックス内 (= boid-on-boid 構成) での codex / opencode 起動
は検証対象に含めない。 sandbox 内 `unshare -U` 単発が EPERM になる既知問題
(残課題 [別軸] 参照) と直交するため、 まずは host で起動する経路だけで adapter 抽象の
妥当性を見る。 nested sandbox 動作は別宿題の解決後に追う。

##### Phase 3-d: session 概念導入 + shell adapter 1 等市民化 + 起動入口整理

**目的**: ジョブの起動経路を「経路 = adapter 選択」 で一意に決まる構造に整理する。 現状の
`JobSpec.HarnessType=""` (= 「adapter 通らない」 状態) を排除し、 hook / session / exec すべての
ジョブが何らかの adapter を経由する形に揃える。 WebUI / CLI からの起動入口を「単純な harness
直選択」 に整理し、 project.yaml の `commands:` (project トップ + behavior 配下両方) を廃止する。

**背景**: Phase 3-b マージ時点で HarnessAdapter は claude/codex/opencode の 3 種揃ったが、
adapter を経由するのは task の hook (agent 指定あり) のみで、 WebUI [Commands] / `boid exec` /
WebUI Commands ボタン on task の 3 経路はすべて `BuildCommandJobSpec` 経由で `HarnessType=""`
(= adapter 通らず素の `runExecArgv`) になっていた。 同じ skill (claude バイナリ起動) が起動
経路によって adapter 経由 / kit 旧経路に分かれる構造で、 Phase 3-e (kit 廃止) と整合しない。

**完了基準**:
- `JobSpec.HarnessType` invariant: hook / session / exec のすべてで非空 (shell / claude /
  codex / opencode のいずれか)
- `runner-inner-child` の `runAgent` から `runExecArgv` ブランチ消滅、 全ジョブが
  `adapter.Run()` 経由
- E2E 全シナリオ green (shell adapter 経由で hook fixture が動作)
- WebUI / CLI に新規入口 (`POST /sessions`、 `boid agent`) が揃い、 旧入口 (Commands ボタン、
  `boid exec <project> <command_name>`) は削除済

**変更スコープ**:

1. **shell adapter 新設** (`internal/adapters/shell/`)
   - `Run(ctx, RunContext) (Result, error)` で `RunContext.Argv` を `exec.CommandContext` で起動
   - `Setpgid: true` で別 pgrp 分離、 `signal.Notify(SIGUSR1)` → 子に `SIGTERM` 中継
   - exit code 正規化 (143→0)、 claude adapter と signal/setpgid helper を共通化
   - `Bindings() = nil` (claude バイナリ等を持ち込まない)、 `AgentEnv() = nil`、
     `Usage() = {}` (token 会計対象外)

2. **`RunContext.Argv []string` 追加**
   - shell adapter only が利用、 claude/codex/opencode は無視
   - 抽象化の純度は若干落ちるが、 adapter ごとの argv 組み立てロジックが既に Phase 3-c で
     分岐済 (`buildClaudeArgs` vs codex/opencode の各実装) のため、 shell が「spec.Argv を
     そのまま使う」 第 4 の流派として並ぶのは構造上自然

3. **registry 拡張** (`internal/adapters/registry/registry.go`)
   - `shell` を `For()` に登録、 4 adapter 体制 (shell / claude / codex / opencode)

4. **planner.harnessTypeForAgent fallback 変更**
   - 現状 (`internal/orchestrator/planner.go:250-261`): 未知 agent → `""` (= `runExecArgv`
     フォールバック)
   - 新: 未知 agent / 未指定 → `"shell"` (= shell adapter フォールバック)
   - hook 定義で `agent:` を書かないケースも `"shell"` に resolve される
   - e2e fixture kit 13 個は **書き換え不要** (自動 resolve で吸収)

5. **`JobKindSession` 新設** (`internal/orchestrator/jobspec.go`)
   - `hook` / `exec` に並ぶ第 3 の Kind
   - 「task 紐付けなし + adapter 経由」 という現状の事実上の概念に正式名称を付与
   - `BuildSessionJobSpec(SessionJobInput)` 新規 (`internal/dispatcher/`)
   - `PlanSession(SessionFireEvent)` 新規 (`internal/orchestrator/planner.go`)

6. **WebUI [New Session] 入口**
   - daemon API: `POST /sessions` (project-agnostic 不可、 必ず project_id 指定) +
     `POST /projects/{id}/sessions`
   - Web UI: harness dropdown (claude / codex / opencode) + 任意 instruction 入力 +
     readonly toggle (default off = writable)
   - resume: `POST /sessions/{session_id}/resume` で既存 session 再開

7. **`boid agent` CLI 新設** (`cmd/agent.go`)

   ```
   boid agent -p <project_id> claude
   boid agent -p <project_id> codex
   boid agent -p <project_id> opencode
   boid agent -p <project_id> claude --resume <session_id>
   boid agent -p <project_id> claude --instruction "ここに書く"
   boid agent -p <project_id> claude --readonly
   ```

   - daemon API `POST /sessions` 経由、 WebUI と同じ配線
   - PTY 直結、 session id を stdout に emit (resume 用)

8. **ProjectCommand / BehaviorCommand 廃止**
   - 削除: `internal/server/wire.go:380-399` (`ExecuteProjectCommand`)
   - 削除: `internal/server/wire.go:419-460` (`ExecuteTaskBehaviorCommand`)
   - 削除: `internal/dispatcher/command_job.go` の `BuildCommandJobSpec`
   - 削除: `internal/orchestrator/planner.go:119-163` の `PlanExec`
   - 削除: project.yaml schema の `commands:` (project トップ + behavior 配下両方)
   - 削除: Web UI の Commands ボタン UI / wire

9. **`boid exec` 簡素化**
   - 旧: `boid exec <project_id> <command_name>` (project.yaml の commands を名前で呼ぶ)
   - 新: `boid exec -p <project_id> -- <argv...>` (任意 argv を sandbox 内で実行)
   - 内部的に `BuildSessionJobSpec(harness="shell", argv=...)` 経由 (= session の薄ラッパー)
   - JobKind は `exec` のまま (TUI ラベル区別)

**session の trait 継承**:
- project レベル trait のみ継承 (env / host_commands / additional_bindings / kit_roots)
- behavior は task コンテキスト専用のため session では参照しない (instruction も意味をなさず)
- readonly: CLI/UI flag で都度指定 (default writable)、 task の readonly default=true (fail-safe)
  とは別ポリシー (session は対話的意思での起動なので開発体験優先)
- resume: `RunContext.SessionID` で adapter に渡す (Phase 3-b で配線済の経路に乗せる)

**互換期間なし**:
- ProjectCommand / BehaviorCommand 廃止は一気切替。 旧 API 呼び出しは 410 Gone を返す
- 既存 project.yaml の `commands:` セクションは parse warning 付きで無視
- 同僚 daemon が新版を pull した時点で WebUI Commands ボタンが消える
- 移行ガイド: `docs/ja/reference/` に「Commands ボタンの代替手段」 として task 作成 + behavior
  指定 / `boid agent` の使い分けを追加

**E2E への影響**:
- e2e fixture kit 13 個の hook 定義は **書き換え不要**。 サーバ側で `agent:` 未指定 →
  `"shell"` 自動 resolve、 runner-inner-child は shell adapter.Run() で従来の hook script を
  起動するため、 検証されてる中身 (broker / state machine / payload_patch.json) は変わらない
- 副次効果として **e2e が adapter ライフサイクル (signal / setpgid / exit code 正規化) を
  常時カバー** するようになる。 現状 claude adapter のこれら経路は CI で未検証だったため、
  Phase 3-d 完了で adapter 抽象の骨格テストが整う

**PR 構成案**:
- **案 X (1 PR 一気切替)**: shell adapter 新設 + RunContext.Argv 追加 + planner fallback 変更 +
  JobKindSession 新設 + ProjectCommand/BehaviorCommand 廃止 + boid agent CLI 新設 + WebUI 入口
  入れ替え + boid exec 簡素化 を 1 PR で。 レビューは重いが互換期間なしと整合
- **案 Y (2 PR)**: PR1 で shell adapter 1 等市民化 + JobKindSession 追加 (新入口は並列に
...
- **[Phase 3-d]**: session 概念導入 + shell adapter 1 等市民化着手時に詰める
- **[Phase 3-e]**: boid-kits claude-code 廃止時に詰める
- **[別 plan]**: 本 plan の範囲外、別 plan に切り出す
- **[別軸]**: Phase 3 と独立だが関連する宿題

> 旧 [Phase 0] / [Track A] / [Track B] / [Phase 2] ラベルは Phase 1 + 2 完了 (2026-06-17)
> 時点で全項目が既決となったため棚卸しで削除した (2026-06-18)。

- **[別軸]** daemon-restart-resume の既知 E2E flake ([[e2e-daemon-restart-resume-flake]])。
  Phase 2 完了時点では HarnessAdapter 化に伴う session continuation 意味論の整理は
  実施せず、 既知 flake として運用継続。 Phase 3 の進行と独立に追える。
...
- **[Phase 3-b]** `Run(ctx, RunContext) (Result, error)` の `RunContext` / `Result` 構造体
  ...
  **2026-06-18 ドラフト** (確定前、 実装時に再検討):

  ```go
  // package adapters
  type RunContext struct {
      // Identity / lifecycle 用
      JobID      string // broker job_done のキー、 transcript 紐付け
      TaskID     string // boid task get (abort_code lookup) 用
      RuntimeID  string // 廃止候補。 Run() 内で signal.Notify 配線するなら不要

      // Session 管理 (現 BOID_AGENT_SESSION_ID / BOID_USER_ANSWER 経由)
      SessionID  string // resume target、 空なら新規 (uuid 生成は adapter 内)
      UserAnswer string // Q&A 回答。 空かつ SessionID 有り = state-changed resume

      // Skill / behavior 選択 (現 BOID_INVOKED_BEHAVIOR / BOID_INVOKED_NAME)
      InvokedBehavior string // "executor" / "supervisor" / 任意 (free naming)
      InvokedName     string // session name suffix (verifier 等)
      ...
      PayloadPath string // payload.json 読み元 (~/.boid/context/payload.json 既定)
      OutputDir   string // payload_patch.json 書き出し先 (~/.boid/output 既定)

      // IO (PTY allocation は dispatcher 側で行い、 ここには末端 fd を渡す)
      Stdin  io.Reader
      Stdout io.Writer
      Stderr io.Writer

      // Env (agent process に exec 時 setenv するもの)
      Env map[string]string

      // Skills (additional_bindings 取り込み後の bind 先 dir)
      SkillsDir string // ~/.claude/skills の絶対パス

      // —— Run() が内部で自前で行うこと:
      //   1. signal.Notify(sigCh, SIGUSR1)  → 子に SIGTERM (PoC 検証済、
      //      inherited sigmask block / SIG_IGN は signal.Notify が自動上書き)
      //   2. signal.Notify(winchCh, SIGWINCH) → 子に SIGWINCH forward
      //   3. cmd.SysProcAttr.Setpgid = true で別 pgrp (group SIGUSR1 回避)
      //   4. exit code 正規化 (stop signal 起因なら 143→0)
      //   5. payload_patch.json への session_id 事前永続化 (abnormal terminate 対策)
      //   6. SkillsDir 配下の symlink 整備 (run-agent.py の ensure_skills_symlinks 相当)
  }

  type Result struct {
      ExitCode        int             // 0 (正常) / 143 (Q&A pause、 daemon 側で 0 化)
      PayloadPatch    json.RawMessage // payload_patch.json の中身そのまま
      Usage           adapters.Usage  // jsonl 解析、 Phase 3-b 時点で空でも OK
      StoppedByDaemon bool            // 上記 143→0 判断材料、 daemon が観測
  }
  ```

  **廃止される 5 メソッドの取り扱い**:
  - `StopAgent(runtimeID)`: 廃止。 daemon は `Runtime.Signal(runtimeID, syscall.SIGUSR1)` を
    直接呼ぶ (現状の `SignalJobRuntime` ラッパーを残し、 中身で hardcoded SIGUSR1)。
    Run() 側は signal.Notify で listen するので対称になる。
  - `ResumePayload(sessionID)`: 廃止。 `RunContext.SessionID` で直接受ける。 daemon →
    runner-inner-child → adapter.Run() の env 中継が不要になる。
  - `Interactive()`: 廃止。 dispatcher が PTY 要否を判断する必要があるため、 spec に
    `HarnessType string` を持たせて dispatcher 側 switch でハードコード判断 (claude は常に
    true)。 adapter package に `claude.Interactive = true` 定数を置く案も可。
  - `SessionIDFromHookEnv(env)`: 廃止。 RunContext.SessionID を直接渡す形に置き換わる
    ため、 env からの逆引きが不要になる。
  - `StopSignalName()`: 廃止。 runner-inner-child が `signal.Ignore(syscall.SIGUSR1)` を
    ハードコードして OK (claude 以外の harness を増やす Phase 3-c 時点で再検討)。
- **[Phase 3-b]** ~~`run-agent.py` の SIG_IGN 回避目的を go の `signal.Notify` + `setpgid` で
  完全再現できることの実機確認。~~ → **2026-06-18 PoC 完了**
  (`/tmp/sig-poc/main.go`)。 結論:
  - Go の `signal.Notify` は inherited sigprocmask block を **自動回避** する
    (Go runtime の専用 signal thread が独自に処理)。 Python で必要だった
    `pthread_sigmask(SIG_UNBLOCK)` は Go 版では不要。
  - `signal.Ignore(SIGUSR1)` を先に呼んでも `signal.Notify` は SIG_IGN を上書きできる。
    runner-inner-child が pre-ignore してから adapter.Run() に入る現行配線で問題なし。
  - `Setpgid: true` で別 pgrp に分離するのは Linux 標準挙動、 PoC 不要。
  - daemon の `LocalRuntime.Signal` 実装は既に `kill(-pgid, sig)` で pgrp 全体送信
    (`internal/dispatcher/runtime_local_linux.go:349`)、 Run() の手前で SIG_IGN な親
    プロセスは生存、 Run() の signal.Notify が catch する経路で配線が一貫する。
- **[Phase 3-b]** boid-kits `claude-code` の `hooks/run-agent.sh` / `additional_bindings`
  を adapter に取り込む順序と互換期間運用。 **2026-06-18 ドラフト**:

  **取り込み対象 (全部 `internal/adapters/claude/` に吸収)**:
  1. `run-agent.py` の全責務 (session resolve/update、 prompt 選択 4 系統、
     payload_patch.json read/write/merge、 skills symlink、 claude argv 構築、
     SIGUSR1/SIGWINCH handling、 start_new_session=Setpgid、 143→0 正規化、
     `boid task get` での abort_code 取得)
  2. `kit.yaml` の `additional_bindings`:
     - `${HOME}/.local/bin` (ro) — claude バイナリ
     - `${HOME}/.local/share/claude` (ro) — claude 共有データ
     - `${HOME}/.claude` (rw) — claude 設定 / state
     - `${HOME}/.claude.json` (rw) — claude メイン設定
     - `${HOME}/.local/share/boid/skills/<name>` → `${HOME}/.claude/skills/<name>` (各
       埋込スキル) は dispatcher が spec 組み立て時に hardcode で出す
  3. `kit.yaml` の `provides_agent: claude-code` と `requires.commands: [claude]` →
     adapter package で hardcode (claude バイナリ存在チェックは init で実施)
  4. `detect.sh` → 廃止 (adapter が常駐するため自動検知不要)

  **PR 構成案 (実装着手時に判断、 分割可)**:
  - **案 A (1 PR)**: 既存 5 メソッド廃止 + Run() 実装 + runner-inner-child 経路変更 +
    sandbox_builder hardcode を一気に。 1 PR で完結する代わりにレビューが重い。
  - **案 B (2 PR)**: PR1 で Run() 実装 (現有メソッドと並走で残置、 dispatcher は手付かず)、
    PR2 で runner-inner-child 経路変更 + 5 メソッド廃止 + sandbox_builder hardcode。
    PR1 単体は機能オフのため安全に merge 可能、 PR2 が切替点。 おそらく案 B を推奨。

  **互換期間の運用** (Phase 3-b マージ後 → Phase 3-e):
  - 新版 boid daemon は adapter.Run() 経由で claude を直接起動。 boid-kits の hook /
    additional_bindings は **dispatcher が一切参照しなくなる** (機能オフ)。 kit を pull
    したままでも害なし。
  - 旧版 boid daemon (同僚ユーザ等) は引き続き kit 経由で run-agent.py を起動。 並走可能。
  - kit ファイルは Phase 3-e まで残置、 docs としても残す。
  - 出口判定: 1 週間の nose 観測で問題なければ Phase 3-e で `boid-kits/claude-code/` 全体
    削除 PR (boid 本体側で 5 メソッド廃止も同 PR で済ませる)。
- **[Phase 3-c]** codex / opencode の 1-turn 動作確認手順と試作スコープ (完全 E2E までは
  目指さない)。
- **[Phase 3-c]** Usage() の harness 別実装で取れる情報 (token / cost / model 名等) の
  最小公倍数確認。
- **[Phase 3-d]** `RunContext.Argv []string` 追加の細部 (RunContext 型に置く vs spec から
  別配線で渡す)。 claude/codex/opencode は無視する設計で良いか、 invariant をどう守らせるか。
- **[Phase 3-d]** shell adapter と claude adapter の signal/setpgid handling 共通化 (内部
  helper 関数 vs base adapter embedding)。 Phase 3-c で codex/opencode も同じ処理を持っており、
  4 adapter で重複しないよう括り出すべき。
- **[Phase 3-d]** WebUI [New Session] dialog の harness 選択肢表示形式 (project ごとの利用可能
  harness をどう判定するか — adapter binary 存在チェック等)。
- **[Phase 3-d]** `boid agent` CLI の resume UX (`--resume <id>` 必須か、 default で「最新
  セッション」 再開も提供するか)。 session 一覧表示の CLI (`boid agent list`) も含めるか。
- **[Phase 3-d]** session の Bindings: shell adapter は nil、 claude/codex/opencode は既存。
  boid CLI 自身 (broker shim) の bind は共通で必要 → adapter 横断の base bindings をどこに
  置くか (registry の中? base adapter で embedding?)。
- **[Phase 3-d]** ProjectCommand / BehaviorCommand 廃止 PR の構造 (案 X = 1 PR 一気切替 vs
  案 Y = 2 PR、 新入口先に追加してから旧入口削除)。 着手時に判断。
- **[Phase 3-d]** `boid exec` の引数仕様変更 (`<project> <command_name>` → `-p <project> --
  <argv>`)。 既存ユーザ向け移行案内、 deprecation handler (古い形式呼び出しを検知してエラー
  メッセージで誘導)。
- **[Phase 3-e]** boid-kits `claude-code` キット廃止のロールアウト計画 (同僚 daemon への
  周知 / kit pin の案内)。
