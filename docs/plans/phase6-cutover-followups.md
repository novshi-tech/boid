...
## ⓪ broker TCP wire completion (PR9 finale gap) — **landed (2026-07-23)**
...
**完了**: followup PR (`feat/phase6-broker-tcp-wire`) で下記 4 ギャップ全て解消。 設計は §決定
「(b) per-job client cert を発行」を採用 (broker は arbitrary-RPC endpoint のため per-connection
client identity binding を維持 — gateway の server-only 降格とは異なる判断)。 実装点:

- broker TLS listener の bind host は `gatewayBindHost(usingContainerBackend)` に統一
  (`internal/server/server.go`)。
- per-job broker client cert (`mtls.CA.IssueShortLivedClientCert`、 24h 有効 — dockerproxy の 1h
  より長い。 broker RPC はジョブ全期間に渡り得るため) を `<runtimeDir>/broker-tls/<jobID>/` に
  materialize、 `/run/boid/bin/broker-tls/` に read-only bind mount、 5 つの `BOID_BROKER_TLS_*` env
...
`ContainerBackendOptions.BrokerTLSCA`/`BrokerTLSAddr`(late-bound pointer) 配線に必要だったため。
