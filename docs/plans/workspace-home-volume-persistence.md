...
**状態**: **完了** (2026-07-26 作成、2026-07-27 完了。PR1-PR8 landed、**PR9** 実装済みで全 9 PR 完結)
...
#### 決定 (PR9 で確定・実装済み、2026-07-27)

| # | 論点 | 決定 | 根拠 |
|---|---|---|---|
| **D1** | `init_script` の置き場 | **envelope spec (`WorkspaceEnvelopeSpec.InitScript`) にのみ追加**。`WorkspaceMeta` には足さない。hydrate は `ProjectAppService` の別経路 (`applyWorkspaceInitScript` / `exportWorkspaceInitScript`) | `WorkspaceMeta` は `workspaces` テーブルの列なので DB migration が要るうえ、「init.sh は file config であって DB resource ではない」という `workspaceInitScriptPath` の明文の決定と衝突する。`MergeInto` は触らない (doc comment に明記) |
| **D1-b** | presence semantics | 他の spec field と同じ **`omitempty` 無し**。キー無し = 触らない / `init_script: ""` = 削除 | `omitempty` を付けると「script を持たない workspace の export」が「キー未指定」と同型になり、**backup を apply しても stale な init.sh を消せない** — env/extra_repos で一度潰した失敗と同じ形。allow-list (`workspaceEnvelopeSpecFields`) にも追加済み |
| **D2** | 専用 CLI の構成 | **4 つ**: `get-init-script` / `set-init-script -f` / `edit-init-script` / `unset-init-script`。全て `scopeRemote` | get/set が最小限。`edit` は plan doc が「日常編集」と位置付けた用途そのもので、手作業の get→編集→set では **If-Match の revision を人間が持ち回る**ことになる (`config edit` が同じ問題を解いていて、失敗時に temp file を残す作法ごと流用できる)。`unset` は「init.sh 無し」が実在する状態 (素通しクラス) で、envelope 側に `init_script: ""` がある以上 CLI に対応する動詞が無いと 2 つの surface が食い違う |
| **D3** | 入口検証 | **NUL バイトのみ**弾く (400)。heredoc delimiter 衝突は検査しない | dispatch 時 (`buildWorkspaceInitRequest`) の 4 条件のうち、書き込み時に content だけで判定できるのは NUL のみ。delimiter は毎 run crypto/rand の 32 hex なので比較対象が存在しない (2^-128)。`HomeID`/`HomeTarget` は init request の内部不変条件。shebang は**検証しない** (boid は exec しないので無効果 — 無視される旨は help / doc / adapter エラー文面で伝える) |
| **D3-b** | サイズ上限 | **128 KiB** (`workspaceInitScriptMaxBytes`)。 初版の 1 MiB から**下げた** (codex 2 巡目 Major 3) | daemon は hash と heredoc wrap のために **全体を buffer する**ので上限自体は必要。 下流の制約ではない — script は init container の **stdin** で渡すので Linux の 1 引数 128 KiB (MAX_ARG_STRLEN) は関係しない。<br><br>1 MiB は `configBodyMaxBytes` / `workspaceBodyMaxBytes` に**揃えただけ**の値で、それが**受理した script を返せない**状態を作っていた: export は script を yaml envelope に入れるので文書は必ず 1 MiB を超え、`workspace apply` はその文書を**同じ** `workspaceBodyMaxBytes` で読む。 つまり上限ちょうどの script は set 成功・export 成功・**その export の apply が 400** — 「export --all が唯一の正式 backup 経路」という契約が API が正当に受理した入力で破れる。<br><br>**apply 側を上げるのではなく init.sh 側を下げた**: apply 側を上げると全 workspace 文書について daemon の buffer 上限が上がる (上限の存在理由そのもの) 一方、得られるのは実測 8.4 KB (移行対象 5 本) / リファレンス実装 5 KB 未満のファイルのための余裕でしかない。 128 KiB は実在する最大の 15 倍で、**observable な数字 (body cap) を 1 つに保ったまま**そこから導出できる。<br><br>予算は `workspaceInitScriptMaxBytes × workspaceInitScriptWorstCaseYAMLExpansion + workspaceInitScriptEnvelopeAllowance ≤ workspaceBodyMaxBytes` = 128 KiB × 6 + 64 KiB = 832 KiB ≤ 1 MiB (192 KiB の余裕)。 **膨張率 6 は実測値**: `TestWorkspaceInitScript_WorstCaseYAMLExpansionIsNotUnderstated` が上限ちょうどのサイズで各 style を計測し、最悪は **5.0018x** (1 文字ごとに改行する script — literal block scalar が `spec.init_script` の 8 space indent で再出力するので 2 byte が 10 byte になる)、次点 4.0018x (全 byte が C0 control → double-quoted の `\xNN`)、通常の shell script は 1.47x。<br><br>**上限は request body ではなく content 側で強制する** (codex 3 巡目 Major 1)。 初版は専用 PUT の `MaxBytesReader` にしか上限が無く、apply 側の事前検証は NUL しか見ていなかった: `spec.init_script` に 128 KiB + 1 byte を入れた**全体 1 MiB 未満**の envelope を apply すると 200 で保存され、guide / CLI リファレンス / この doc が揃って書いている上限が経路を選ぶだけで破れた。 `validateWorkspaceInitScriptContent` に移したので 「init.sh を保存する経路」= 「この関数を通る経路」になっている (`SetWorkspaceInitScript` の lock 取得前と `validateWorkspaceApplyInitScript` の transaction 前)。 door 側の `MaxBytesReader` は**残す** — read 済み content の検査では「そもそもメモリに載せない」ができない |
| **D4** | 空 content の意味 | **「init.sh 無し」= 削除**。空のスクリプトは表現しない | 空の実在ファイルは marker に sha256(0 bytes) を記録し、不在は "" を記録する — 挙動は同一 (wrapper は bytes を file に書いて bash に渡すだけ) なのに**切り替えるたび再 init が走る**。統一すると envelope の `init_script: ""` と CLI の `unset` が同じ意味になる。空白は trim しない (改行 1 個のスクリプトも書いた人の意図) |
| **D5** | ETag | content hash (sha256 先頭 16 byte)。**不在は `"absent"` sentinel** | 不在を `""` にすると「If-Match 未送信」(428) と区別できず、**workspace 初回の script 作成だけ楽観ロックが効かない**。sentinel なら「読んだ時点では無かった」を If-Match に載せられ、間に他経路で作られていれば 412 になる。GET は不在でも 404 に ETag を付ける (CLI はこの ETag の有無で「script が無い」と「daemon が古くて route 自体が無い」を判別する) |
| **D6** | 削除の HTTP verb | **DELETE を作らず PUT の空 body で表現** | D4 で空 = 不在に統一した以上、DELETE は「同じことを言う 2 つ目の綴り」になり、以後ずっと一致を保つ義務が生じる。CLI の `unset-init-script` は空 body の PUT |
| **D7** | store の置き場 | mechanism = `dispatcher.WorkspaceInitScriptStore` (path / read / write / remove)、policy = `internal/api` (If-Match / 上限 / NUL / 空=不在) | 論点 a-2 の D2 と同じ分割。**store に注入可能な root を持たせない**のが要件: reader (`resolveWorkspaceHome` → `workspaceInitScriptPath`) は ambient な `os.UserConfigDir` を見るので、writer 側に別の root を持たせられると「書けたのに dispatch は読まない」が成立する。テストは env 側で隔離する (`testutil/homeenv`) |
| **D8** | 書き込みの作法 | `config.WriteFileAtomic` で temp + chmod + fsync + rename、**file 0600 / dir 0700、実行権限なし**。 dir mode は `MkdirAll` に渡すだけでなく**毎回 `Chmod` する** (codex 2 巡目 Minor 4) | fast path の `readIfExists` は lock 無しで読むので、非 atomic な書き込みは半端な bytes を hash されうる。実行権限を付けないのは boid が **exec しない**から — 付けると存在しない実行方法を示唆する (shebang を検証しないのと同じ理由)。`WriteFileAtomic` の temp prefix は 2 個目の caller が付いたので basename 由来に一般化した (config.yaml 側は byte 単位で不変)。<br><br>`MkdirAll` の mode は**自分が作った** directory にしか効かないので、既存 install では 0700 は成り立っていなかった: 移行対象の実機は PR9 以前の手作業運用で作られた **0775 (group 書き込み可)** だった (実測 2026-07-27)。 file の 0600 はこの露出の答えにならない — 守っているのは**中身**であって、group 書き込み可の directory は誰でも init.sh を unlink して**自分のスクリプトに差し替えられる**。 それは workspace の init container で、その workspace の network と認証情報を持って走る。 chmod 失敗は write 自体を失敗させる (このツリーは daemon 自身の `$XDG_CONFIG_HOME` なので、mode を設定できないのは operator に伝えるべき障害であり、直後に「保証できない」と判定した場所へ script を置くのは筋が通らない)。<br><br>**内容が同一でも `Write` を通す** (codex 3 巡目 Minor)。 service 側が同じ bytes を `unchanged` として短絡していたので、chmod を足しても**実際にそれが必要な唯一のケース**に届いていなかった: 手作業で置いた init.sh を同じ内容で `set-init-script` / `apply` しても、実測された 0775 の directory はそのまま残る。 `Write` は「この file をちょうどこの状態にする」であって mode はその一部なので、短絡は content の比較にだけ効かせる (action は `unchanged` のまま — revision も marker の hash も変わらないので再 init は走らない)。 失うのは「内容が最後に変わった時刻」としての mtime だけ |
| **D9** | apply と DB transaction の関係 | init.sh の書き込みは **commit 後**。失敗は「metadata は適用済み・init.sh は未適用」と明記したエラー | file 書き込みと DB transaction は原子化できない。逆順にすると「workspace が作られなかったのに script だけある」になる。この順なら再 apply で収束する。`--dry-run` は action を計算して書かない |
| **D10** | 誤誘導エラーの修正 | `claude`/`codex`/`opencode` の `missingCLIError` を **3 ファイル lockstep** で CLI コマンド案内に差し替え | 旧文面は `~/.config/boid/workspaces/<slug>/init.sh` を案内していたが、volume-only 後これは **daemon 自身の** path。指示に従うとホスト側に誰も読まないファイルができ、次の dispatch で同じエラーが出る — 「直し方」を書いたエラーとして最悪の形 |
| **D11** | `workspace remove` と init.sh (codex 2 巡目 Major 1) | **row / HOME volume と同じ場所で init.sh も削除する**。 home volume と同じ **best-effort** (失敗しても 200、`init_script_delete_error` に載せる)、予約 slug `default` は skip、結果は `init_script_deleted` / `init_script_delete_error` で報告 | 残すと無害な残骸ではない: dispatch は init.sh を **slug だけ**で引くので、**同名で作り直した workspace が削除済み workspace の script を継承**し、まっさらな HOME volume に対して走る。 しかも row が消えた後は `/api/workspaces/{slug}/init-script` が全部 404 なので、`unset-init-script` で事前に掃除することもできない。 best-effort なのは row 削除が先に commit 済みだから — ここで 500 にすると「不可逆な成功」の上に失敗を報告することになり、しかも `workspace remove` は再実行できない (row が無いので 404)。 error 文面には daemon 側 path を載せる (手で消せるように)。<br><br>**削除は row 削除と同じ critical section の中で行う** (codex 3 巡目 Major 3)。 初版は handler が `RemoveWorkspace` (内部で `s.mu` を取得・解放) の**後に別呼び出し**で init.sh を消していて、その隙間に `apply` が同じ slug を upsert し新しい init.sh を書いて 200 を返せた — 直後に remove がその script を消すので、**「row はあるのに成功した apply の script が無い」**が残る。 `RemoveWorkspace` 自身が row 削除の直後・mutex 保持のまま消すことで、apply は「完全に前」(row も script も消える) か「完全に後」(新しい workspace が script を保つ) のどちらかになる。 結果は `WorkspaceRemoval` で返して handler が response に載せる (原子性は主張しない — 報告のために別呼び出しにする必要は無い)。 PR8 の in-flight 登録簿 (`BeginWorkspaceHomeRemoval`) は**別の排他**で、migration だけを相手にしていて apply は参加していない — この呼び出しごと bracket してはいるが、apply を締め出しているのは `s.mu` の方。<br><br>**test は「削除の最中」で止める** (codex 4 巡目 Minor)。 3 巡目の回帰 test は HOME volume の削除で止めていたが、そこは `RemoveWorkspace` が**返った後** — mutex はどちらにせよ解放済みなので、init.sh の削除を mutex の外 (return より前) に出しても緑のままだった。 `WorkspaceInitScriptStore.Remove` の**中**で止めて、その窓の間に `ApplyWorkspace` が完走できないことを見る形にしてある (`TestWorkspaceHandler_Remove_DeletesTheInitScriptInsideTheRowsCriticalSection`)。 mutation (`s.mu.Unlock()` を削除の前に移す) で赤くなることを確認済み。 HOME volume 削除中の窓の方は別 test に残した (`…_KeepsAScriptWrittenWhileTheHomeVolumeIsDeleted`) |
| **D12** | 存在確認と mutex の関係 (codex 2 巡目 Major 2) | `SetWorkspaceInitScript` は **lock 取得後に workspace row を再確認**する (`confirmWorkspaceRowExists`) | 元は確認が `s.mu` の**外**にあり、確認と lock の間に `RemoveWorkspace` が入ると「row の無い workspace の init.sh」が出来ていた — この API 自身から到達不能 (全 route が 404)、次に同名で作られた workspace が継承する。 `?force=true` では revision 比較を飛ばすので**確実に**成立した。 PR8 が `beginMigration` 直後に `ConfirmWorkspaceExists` を置いたのと同型の解 (答えが使われる前に陳腐化しないよう、排他の内側で訊く)。 **D11 の cleanup を足すだけでは不十分** — cleanup が先に走り、この write が後から作り直す。 「2 回訊く」だけでも不十分なので、`TestSetWorkspaceInitScript_ChecksTheWorkspaceRowWhileHoldingTheServiceMutex` が**確認中に並行 remove が完了できないこと**を直接 pin している |
| **D13** | export→apply の round-trip 保証 (codex 2 巡目 Major 3 → **4 巡目 Major 1 で全 string に一般化**) | **成功した export は必ず apply でき、同じ値として戻る**。 fidelity は `WorkspaceEnvelope.MarshalYAML` が **envelope の全 string scalar** について「複数行かつ 1 行目が space / tab / 改行始まり」の値を double-quoted style で出す (`internal/orchestrator/workspace_envelope_marshal.go`)。 サイズは D3-b、強制は D13-b | gopkg.in/yaml.v3 v3.0.1 は複数行文字列を literal block scalar で出すが、block scalar の indent は**読み手が 1 行目から推測する**ので、1 行目が空 / tab / space 始まりだと推測できず emitter が indentation indicator (`\|4`) を明示する必要がある。 v3.0.1 の壊れ方は 3 つあり、どれも export 時は無言だった: ①**先頭 tab** → indicator を出さないので出力が YAML として不正、`workspace apply` が `found a tab character where an indentation space is expected` で落ちる (復元不能な backup)、②**先頭改行** → 先頭の空行が**黙って落ちる** (parse は通り、apply も成功し、戻ってきた値は export したものと違う — script なら hash も違うので再 init する)、③**先頭 space** → mapping value の位置では正しい `\|4` を出すが、**block sequence の要素や explicit key (`? `) の位置では indicator が実際の indent と合わず** `did not find expected '-' indicator` で parse に失敗する。 どれも珍しくない (Makefile / heredoc から貼った tab 始まり、editor が残す先頭空行)。<br><br>**これは emitter の性質であって schema の性質ではない**ので、init_script だけを特別扱いした初版の実装は同じ穴を `env` の値・`env` の**キー**・`host_commands` / `allowed_domains` / `extra_repos` の要素・`container_image` / `projects[].name` / `projects[].url` に残していた。 **この穴は PR9 由来ではなく PR-1d (`docs/plans/volume-only-daemon.md` §論点g、commit `c490e2c`) からの既存欠陥**で、HEAD `97e6dc8` の export 経路で再現を確認済み (tab 始まりの `env` 値 → `found a tab character…`、改行始まり / space 始まり → `did not find expected '-' indicator`)。 PR9 が新たに宣言した「成功した export は必ず apply できる」を実際に成立させるためにここで直す。<br><br>**field を列挙しない実装にする** (取りこぼしが構造的に起きないように): `markRoundTripStrings` が**値を reflection で走査**して該当 string に内部 marker を 前置し (`(*yaml.Node).Encode` は marshal + 再 parse なので、素のままでは自分の出力を読めない / 先頭空行を落とす)、`unmarkRoundTripScalars` が**出来た node tree を走査**して marker を剥がしつつ style を固定する。 struct field / slice / array / map の**キーと値**・pointer・interface の全部を辿るので、spec に field が増えた瞬間に自動で対象になる。 marker は接頭辞 (U+E000) で、「該当する」か「既に marker で始まる」string の両方に付けるので**単射**であり、戻すときにちょうど 1 個剥がせば厳密に元に戻る。 **不正 UTF-8 は marker を付けない**: encoder が `!!binary` (base64) を選ぶため出力側に marker が残らず、剥がせないまま復元 bytes に混ざってしまう (`!!binary` は 1 行目問題が無く元から round-trip する)。<br><br>③の position 依存を利用して「mapping value の先頭 space だけは block のまま」にはしなかった — 位置ごとに emitter の挙動を推論し直すのは、まさにこの欠陥を生んだやり方だから。 代償は 2 形: **1 行目が space 始まりの init.sh が `\|4` から double-quoted に変わる**のと、**値が marker (U+E000) で始まる string が 1 行でも double-quoted になる** (codex 4 巡目 Minor)。 後者は marker の単射性の帰結で避けられない — 「既に marker で始まる」string にも marker を付ける (上記) 以上、剥がした後の style 固定も同じように効く。 読み戻る値はどちらも不変で、marker 始まりの値が壊れないことは `TestMarshalWorkspaceEnvelope_MarkerBearingValuesRoundTrip`、style がこう変わること自体は `TestWorkspaceEnvelope_AMarkerPrefixedValueIsQuotedNotPlain` が pin している。 **この 2 形以外**は byte 単位で不変であることを、修正前後の emitter で 12 種類の envelope (通常 script / tab を含む script / CRLF / 不正 UTF-8 / unicode / 複数行 env / 空値 / trailing 空行 / projects / capabilities …) を出し比べて確認し、golden test `TestWorkspaceEnvelope_OrdinaryValuesAreEmittedUnchanged` で pin した。<br><br>`workspace apply` は文書を **decode して再 marshal** してから POST する (`SplitWorkspaceEnvelopeDocuments`) ので、そこにも同じ規則の node pass (`forceRoundTrippableScalars`) を入れてある — 手書きで `\|2` を使えば先頭 tab は合法に書けてしまい、再 marshal がそれを壊す。 網羅性は init_script でやっていた {a, space, tab, 改行} 長さ 4 以下の全探索 (340 通り) を、**全 slot に同時に置く**形で回して pin した (`TestWorkspaceEnvelope_EveryStringSlotRoundTripsThroughExport` / `…_KeepsEveryStringSlotApplicable` / `TestMarshalWorkspaceEnvelope_VerifiesItsOwnOutput`) |
| **D13-b** | 「export が出す文書は必ず apply できる」の**強制** (codex 3 巡目 Major 2 + **4 巡目 Major 1**) | **export 側で拒否する**。 サイズ: 1 文書が `workspaceBodyMaxBytes` を超えるなら workspace 名とサイズを挙げて **409 で失敗** (`checkWorkspaceEnvelopeIsApplicable`)。 復元性: **emit した bytes を自分で再 parse して node の値単位で照合**し、一致しなければ失敗する (`orchestrator.MarshalWorkspaceEnvelope`)。 CLI: 応答の decode 失敗・空 body・**`spec.init_script` を欠く文書**を error にし、**復元できないものをファイルに書かない** (`checkExportedWorkspaceEnvelopes`) | D3-b の 64 KiB (`workspaceInitScriptEnvelopeAllowance`) は**計算上の定数でしかなく**、metadata を実際に制限する実装が無かった: 通常の workspace API で 400 KiB の `env` 値を保存 → 上限ちょうどの script を保存 → export すると script だけで 640 KiB、metadata と合わせて約 1,040 KiB になり、**その export の apply は body 読み取り時に 400**。 script 側の上限だけでは閉じない。<br><br>**apply 側の上限を上げても閉じない**: metadata 半分を縛っているのはその同じ上限なので、apply が受理する文書の export は常にそれより大きく、次の上限も同じ理屈で破れる (循環)。 metadata 各 field に上限を設けるのは create/update/import/apply の 4 surface に 新しい拒否を足すことになり、実在しない workspace のために大きすぎる。<br><br>**export 側の拒否は write 経路に一切拒否を足さない**。 変わるのは「黙って復元不能なファイルを出す export」が「そう言う export」になることだけで、operator が当てにしたはずのファイルは元々存在しなかった。 先例はすぐ隣にある — export は既に、apply で名前が解決できなくなる project 名を持つ workspace を 409 で拒否している。<br><br>**サイズだけでは「apply できる」を強制しきれない** (codex 4 巡目 Major 1)。 初版の check は**サイズしか見ていなかった**ので、D13 の壊れ方 (壊れた scalar) はそのまま marshal されて「成功」した。 そこで `orchestrator.MarshalWorkspaceEnvelope` を export の唯一の出口にし、**自分が出した bytes を再 parse して、emit した node tree と値単位で突き合わせる** (`diffYAMLValues`)。 一致しなければ該当キーを挙げて失敗する。 これで保証が「規則が正しいはず」という論証ではなく**実際に検査された性質**になる — yaml.v3 の upgrade で style の選択が変わっても、想定していない形の field が増えても、黙って壊れた backup が出る代わりに export が落ちる。 誤検出はしない (比較相手は emitter 自身の node tree)。<br><br>**CLI 側も同じ扱いにする**: `boid workspace export` は応答を `workspace apply` と同じ decoder に通していたが、**decode 失敗を握り潰して 0 を返し**、`-o/--output` にはそのまま書いて `exported 0 workspace(s)` と表示していた (これも PR-1d 由来の既存挙動)。 復元できない可能性のあるものを黙って backup として書き出さないよう、decode を**gate** にした (`checkExportedWorkspaceEnvelopes`)。 空 body も同様に拒否する。 警告 (project の url 未取得 / env の host path) は従来どおり advisory のまま。<br><br>**decode が通るだけでは足りない** (codex 4 巡目 final Major)。 decoder は `spec.init_script` の**欠落を正当な「触らない」として受理する** (D4 / apply の merge 意味論) ので、**PR9 以前の daemon の応答はそのまま decode を通り**、CLI は「成功」としてファイルに書いていた。 その backup を新環境に apply すると metadata だけ戻り、**init.sh は復元されず次の dispatch で harness が入らない**。 `workspace *` は全部 `scopeRemote` で、この PR 自身の init-script CLI も旧 daemon (`404` + ETag 無し) を明示的に扱っている以上、版ずれは想定内の状態。 そこで export 側は**全文書に `init_script` キーの存在を要求**し、欠けていれば workspace 名を挙げて daemon の upgrade を促し、ファイルを書かずに失敗する (`checkExportedDocumentIsComplete`、`--all` は文書ごとに判定)。 **要求は export 側だけ**: apply 側で必須にすると、`env` だけを直す手書き文書に init.sh 全文の貼り付けを強いることになる。 非対称の根拠は「欠落が何を意味しうるか」が両者で違うこと — apply が読むのは人が**書いた**部分的な変更 (欠落 = 触らない) で、export が書くのは workspace **全体**の復元用文書 (daemon は全 field を必ず出すので、欠落 = daemon がその field を知らない) 。 両側を test で pin した (`TestRunWorkspaceExport_RefusesADocumentWithoutInitScript` / `…_All_…` / `TestWorkspaceApply_AcceptsADocumentWithNoInitScriptKey`)。<br><br>**metadata 側が大きすぎる場合は塞がない** (それがこの拒否の報告内容)。 これは envelope の既存性質で (env だけで body cap に迫る workspace は init.sh の有無に関わらず round-trip しない)、script 上限の役目は**script が原因にならない**ことの保証。 予算の両側を test で pin した: 算術は `TestWorkspaceInitScriptMaxBytes_IsDerivedFromTheApplyBodyCap`、実物は `TestWorkspaceInitScript_TheLargestAcceptedScriptSurvivesExportAndApply` が **allowance を埋める非空の `WorkspaceMeta`** で (空 meta では script の取り分しか測れない) |
| **D14** | Content-Type の扱い (codex 2 巡目 Minor 1) | **deny-list**: structured data の media type (yaml/json/xml/html/tar/gzip/zip) だけ 400、それ以外 (未指定を含む) は全部受理 | D3 で確定した入口検証は「NUL のみ」。 allow-list はそこに 2 つ目の拒否条件を足していて、**正当な script でも拒否される**ものだった: `curl --data-binary @init.sh` の既定は `application/x-www-form-urlencoded` で、これは CLI を使わずにこの endpoint を叩く最も素直な方法。 deny-list はこの check の実際の値打ち (yaml/json 文書を shell として保存させない) を残したまま、script を**ラベルだけを理由に**拒否する部分を落とす。 PR8 の home-import が allow-list + Content-Type 必須なのとは非対称だが、その理由はそちらに書いてある (body を読む**前に** HOME volume を破壊するので、ラベル間違いが認証情報の損失になる)。 error 文面には送るべき type と、yaml 文書の正しい宛先を書く |

**PR9 で挙動が変わる点**:

1. `boid workspace get-init-script` / `set-init-script` / `edit-init-script` / `unset-init-script` が生えた
2. `boid workspace export` の出力に **`spec.init_script` が増えた** (script を持たない workspace は `init_script: ""`)。`apply` はこのキーがあれば init.sh を復元・削除する
3. harness CLI が見つからないときのエラー文面が変わった (host path の案内 → CLI コマンドの案内)
4. `GET/PUT /api/workspaces/{slug}/init-script` が増えた
5. doc drift 2 件を修正: `workspace-home.md` の「init.sh はホスト側 config dir、サンドボックスから書き換えられない」(volume 化後は事実に反する) と、`workspace export` の body に slug key が無いという 3 箇所の doc comment (実装は含んでいる)
6. **`boid workspace remove` が workspace の init.sh も削除するようになった** (D11)。 レスポンスに `init_script_deleted` / `init_script_delete_error` が増えた
7. **init.sh の上限が 128 KiB になった** (D3-b。 実在する最大の 15 倍なので実害は無いが、1 MiB を前提にしていた場合は影響する)
8. `workspace export` が、**1 行目が space / tab / 改行始まりの複数行の値**を double-quoted style で出すようになった (D13)。 対象は `spec.init_script` だけでなく `env` の値とキー・`host_commands` / `allowed_domains` / `extra_repos` の要素・`container_image` / `projects[]` を含む **文書中の全 string**。 加えて **値が U+E000 で始まる string は 1 行でも double-quoted になる** (内部 marker の単射性の帰結、値は不変 — D13。 `TestWorkspaceEnvelope_AMarkerPrefixedValueIsQuotedNotPlain` で pin)。 **この 2 形以外**の値の出力は byte 単位で不変 (`TestWorkspaceEnvelope_OrdinaryValuesAreEmittedUnchanged` で pin)
9. **`boid workspace export` が、apply で読めないサイズになる文書を書き出さずに 409 で失敗する** (D13-b)。 引っかかるのは metadata が数百 KB 級の workspace だけで、そういう workspace の export は元々 apply できなかった (黙って壊れた backup が出ていた)
10. **`boid workspace export` が、apply で読み戻せない応答を書き出さずに失敗する** (D13-b)。 daemon 側は emit した文書を自分で再 parse して突き合わせ、CLI 側は応答の decode 失敗と空 body を error にする (従来は decode 失敗を握り潰して `-o/--output` に書き、`exported 0 workspace(s)` と表示していた)
10-b. **`boid workspace export` が、`spec.init_script` を持たない応答 (= PR9 以前の daemon) を書き出さずに失敗する** (D13-b)。 新しい CLI から旧 daemon を叩いたときに出る。 対処は daemon の upgrade で、error 文面がそう案内する。 **`boid workspace apply` 側は従来どおり `init_script` 無しの文書を受理する** (欠落 = そのキーは触らない) — 手書きの部分的な文書を書けなくしないため
11. **同じ内容での `set-init-script` / `apply` でも file/dir の mode が 0600 / 0700 に直る** (D8)。 action は `unchanged` のままで revision も marker の hash も変わらない (再 init は走らない) が、file は毎回 atomic に置き直されるので **mtime は更新される**
...
| 9 | **[landed]** init.sh の CLI 経路 (論点 d の決定表)。 envelope spec に `init_script` (D1、`omitempty` 無し) + 専用 CLI 4 つ (D2: `get-/set-/edit-/unset-init-script`) + `GET/PUT /api/workspaces/{slug}/init-script` (D5 の ETag/If-Match、不在は `absent` sentinel) + `dispatcher.WorkspaceInitScriptStore` (D7: reader と同じ path 解決に固定、注入可能な root を持たない) + 入口検証は NUL のみ (D3)・上限 **128 KiB** (D3-b)・空 content は削除 (D4) + adapter 3 ファイルの誤誘導エラー修正 (D10) + doc drift 2 件。 **codex 2 巡目**: `workspace remove` が init.sh も削除 (D11)、存在確認を mutex 内へ (D12)、export→apply の round-trip 保証 = サイズ予算 + yaml block scalar の fidelity 修正 (D13)、Content-Type を deny-list へ (D14)、dir mode を毎回 chmod (D8)。 **codex 3 巡目**: 上限を content 側で強制し apply 経路の迂回を塞ぐ (D3-b)、export が apply 不能な文書を出さない (D13-b)、init.sh 削除を row 削除と同じ critical section へ (D11)、同一内容でも mode を修復 (D8)。 **codex 4 巡目**: round-trip 対策を **envelope の全 string** に一般化し (D13、既存欠陥 = PR-1d 由来)、export の出口で**自分の出力を再 parse して検証**・CLI の decode 失敗を gate 化 (D13-b)、init.sh 削除が critical section 内であることを test で pin (D11) | **`workspace export` の出力に `spec.init_script` が増える** (script 無しの workspace は `init_script: ""`、apply 時は削除の意味)。 **`workspace remove` が init.sh も消す** (レスポンスに `init_script_deleted` / `init_script_delete_error`)。 harness CLI 不在時のエラー文面が host path の案内から CLI コマンドの案内に変わる。 それ以外の既存挙動は不変 (dispatch 側は 1 行も変えていない) | — (独立) |
