...
**並走しない。** core と khi を同じ回で切り替える。

並走を挟むと、khi が action 列を直読みしている裏で inbox にも同じ事実が signal として
積まれる —— **二重に判断する危険を抱えたまま日をまたぐ**ことになる。等価性は切り替え後の
実データで採る (§10 グループ C)。
...
無い (§4.3)。1 と 2 の間隔は短いほどよく、**同じ回で両方デプロイするのが既定**。
