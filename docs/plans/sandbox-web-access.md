...
### 案 2（本命）: boid が WebFetch / WebSearch 相当をホスト仲介で提供

fetch/検索をホスト側で実行し、サンドボックスには結果だけ返す。boid 仲介なので GET 限定・URL ログ・
ボディ禁止を強制でき、案 1 の「GET-only 保証」を TLS を割らずにアプリ層で実現する（CA 不要・pinning 影響なし）。

#### アーキテクチャ

1. **ホスト側 fetch/search 実体**
   - `boid fetch <url>`: ホストで GET → HTML を markdown 変換して返す。GET 固定・ボディ不可・URL ログ・
     private/metadata レンジ拒否・任意の allowlist/denylist。
   - `boid search <query>`: 検索 API（Brave / Google / Bing 等）に問い合わせ。**外部 API + 鍵が必要**（`config.yaml`）。
   - 実装形態の選択肢:
...
2. **サブエージェント・ラップ（トークン対策）**
   ...
   - これを **サブエージェント**で再現する: 主エージェントがスキルを呼ぶ → サブエージェント（安いモデル）が
     `boid fetch`/`boid search` を実行し、生コンテンツを**自分のコンテキストで消費**して要約だけ主に返す。
...
- Phase 1: `boid fetch`（外部依存なし）+ サブエージェント・ラップ・スキル。
- Phase 2: `boid search`（検索 API + 鍵管理）。

#### 未解決事項

- 検索 API の選定と鍵管理（`config.yaml` のどこに置くか）。
