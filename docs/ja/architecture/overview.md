...
`kit.yaml` というファイル形式自体は無くなっていません。 `WorkspaceMeta.Kits` フィールド (workspace.yaml の `kits:`) は Phase 2.5 PR7 でコードから完全撤去され、 `POST`/`PUT /api/workspaces` は `kits:` を含む body を reject するようになりましたが、 legacy `kits: [...]` 参照を一度だけ展開する互換経路 (`workspace_migration.go`) は残っています。呼び出し元は (1) `boid workspace assign` の auto-create 補助、 (2) daemon 起動時の一度きりの DB 移行の 2 経路のみです。展開される runtime 設定は `host_commands` / `env` で、`additional_bindings` は撤去済みのため適用されません。
