...
- **worktree_manager** — project トップで `worktree: true` を宣言したプロジェクトの executor タスク用に git worktree を作成・再構築・破棄
...
6. **hook 評価**: `Evaluator` が `executing` (または `awaiting`) で発火すべき hook を kit メタから選ぶ
