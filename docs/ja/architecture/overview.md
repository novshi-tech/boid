...
`kit.yaml` というファイル形式自体は無くなっていません — workspace 作成時や `boid project migrate` 実行時に `kits: [...]` 参照を一度だけ展開して `host_commands` / `env` / `additional_bindings` に畳み込む経路 (`workspace_migration.go`) が唯一残った kit 展開経路です。
