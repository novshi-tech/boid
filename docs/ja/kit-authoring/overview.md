...
hook と gate は同じ I/O プロトコルを使います。
...
payload を更新したい場合、次の形の JSON を 1 つだけ stdout に出します。
...
`payload_patch` の中身は payload に対する JSON merge 指示です。 ネストしたキーをそのまま書けば、その階層だけが更新されます。 何も更新しない場合は stdout に何も出さなくて構いません。
