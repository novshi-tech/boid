...
gates:
  - id: my-gate
    on: [verifying]
    phase: exit                 # entry または exit (既定 exit)
    traits:
...
詳細は次章のスクリプトプロトコルを参照。 `traits.consumes` と `traits.produces` は、 hook / gate がどの payload trait を読み書きするかを宣言します。 `consumes` の末尾に `?` を付けると optional (なくてもエラーにしない) になります。
