...
「kit が何か」 は [概念](../guide/concepts.md#kit) で定義済みです。 ここでは ディスク上のレイアウト、 `kit.yaml` の主要フィールド、 hook スクリプトのプロトコル、 配布の仕方をまとめます。
...
`boid project init` のようなセットアップフローで、 「このプロジェクトにこの kit が適用できるか」を判定するためのスクリプトです。 POSIX sh で書き、 stdout 1 行目に次の文字列のいずれかを出します。
