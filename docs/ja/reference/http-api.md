...
| Method | Path | 役割 |
|---|---|---|
| GET | `/api/tasks` | タスク一覧 (クエリ: `status`、 `behavior`、 `workspace_id`、 `has_depends_on`、 `no_depends_on`) |
...
| GET | `/api/tasks/{id}/gates` | 現在の status で発火する gate 一覧 |
| POST | `/api/tasks/{id}/gates/{gate_id}/replay` | 特定 gate を再実行 |
...
C2 の Q&A フロー (エージェントがユーザに質問し、回答で再開する) を制御する 2 つのエンドポイントです。詳細は [C2 フロー](../architecture/c2-flow.md) を参照してください。
...
| POST | `/api/jobs/{id}/done` | (内部) handler の終了を通知。 `--exit-code` と payload patch ファイルを受け取る |
...
| Method | Path | 役割 |
|---|---|---|
| POST | `/api/broker/register` | hook / gate 起動時に shim 用トークンを発行 |
...
- [Handler スクリプトプロトコル](handler-contract.md) — `POST /api/jobs/{id}/done` を呼ぶ EXIT trap の挙動
