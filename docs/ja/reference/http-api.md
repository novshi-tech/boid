...
| `ask` | | 質問テキスト。 指定時は `awaiting` に遷移する。 daemon は answer 時の resume hook を dispatch しないので、 実用 Q&A は `POST /api/tasks/{id}/ask` 相当の blocking RPC (`boid task ask`) を使うこと。 `done`/`fail`/`progress` と排他 |
