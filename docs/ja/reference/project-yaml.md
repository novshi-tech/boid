...
> **注意:** `project.yaml` はもう実行環境 (kits / `host_commands` / `env` / `additional_bindings` / `secret_namespace` / `capabilities`) を設定しません。 これらの machine-local な設定は代わりに **workspace** に置きます (`boid workspace create/edit/import`)。 何がどこへ移動したかは下記 [トップレベルのフィールド](#トップレベルのフィールド) の表を、 現行のセットアップ手順は [オンボーディング](../guide/onboarding.md) を参照してください。
