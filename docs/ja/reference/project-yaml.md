...
| キー | 型 | 役割 |
|---|---|---|
| `agent` | string | この instruction を受け取る harness の識別子。`claude-code` は claude harness (kit 配線が必要)、`codex` は builtin codex adapter、`opencode` は builtin opencode adapter、省略または未知値は shell adapter に fallback する |
