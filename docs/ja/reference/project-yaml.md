...
| `worktree` | bool | `false` | `true` にすると、 executor / supervisor タスクに専用の isolated branch を割り当てる。 **root タスク** (親なし) の HEAD は `base_branch` そのもの (case 1 = project HEAD と一致する場合は新規 branch を作らず `base_branch` を直接 checkout)。 **child タスク** (親あり) は常に `boid/<task_id8>` branch を新規作成する。 実装は sandbox 内 clone のブランチ宣言であり、 host 側の git worktree は作らない ([git gateway / sandbox 内 clone](#git-gateway--sandbox-内-clone) 参照)。 詳細は [タスク種別と HEAD branch](#タスク種別と-head-branch) を参照 |
...
`worktree: true` のプロジェクトでは、 タスク種別によって HEAD branch と fork 元が異なります。 この表は clone モデルでも変わりません — 変わったのは「どうやってその branch にたどり着くか」(host worktree → sandbox 内 clone + checkout) だけです:
...
- **root タスク** (`parent_id == ""`): `base_branch` を HEAD に直接乗る。 `base_branch` が project HEAD と一致する場合 (case 1) は sandbox 内 clone した上でその `base_branch` を直接 checkout する (新規 branch は作らない)。 不一致の場合 (case 2/3) も同様に clone した上で `base_branch` を HEAD として checkout・必要なら新規作成する
- **child タスク** (親あり): 常に `boid/<task_id8>` branch を新規作成して checkout する。 fork 元は **親タスクの HEAD branch** (親が root なら `base_branch`、 親が child なら `boid/<parent_id8>`)。 直接の親のみを参照する (1 hop) — **親の branch は origin に push 済みでなければならない** (sandbox 内 clone は origin の pushed ref しか見えないため)
- `task.BaseBranch` は PR target として全子タスクに継承され、 `BOID_BASE_BRANCH` env で executor に渡る

### 同一 HEAD branch を対象とする複数タスクの並行実行

以前は同一 `<projectID>:<HEAD branch>` を対象とする複数タスクを FIFO ロックで直列化していました (同じ host git worktree を複数タスクが同時に使えないため)。 **この直列ロックは git gateway cutover で廃止済みです**: 各タスクは独立した sandbox 内 clone を持つため、 同じ branch を対象とする複数タスクも並行して dispatch されます。 同時に push した場合は通常の git のとおり non-fast-forward で reject されるので、 fetch + merge/rebase して再 push してください。
...
A (executor) が done → A の PR を merge
            ...
            sub sup が git fetch && merge で自 branch (boid/<subid8>) を更新
                         ↓
            sub sup が B を dispatch → B の clone が更新済み boid/<subid8> から fork
```
...
`boid` 自身のリポジトリにある `.boid/project.yaml` (抜粋) を載せておきます。 2 つの behavior (`supervisor`, `executor`) を定義し、 project トップで `worktree: true` を宣言することで executor タスクが専用の isolated branch (sandbox 内 clone 上) を持つ構成です。

```yaml
id: boid
name: boid

# Project トップの worktree フラグ: タスク種別に応じて isolated branch を割り当てる。
# root タスクの HEAD = base_branch (case 1 = base_branch を直接 checkout、 case 2/3 も base_branch が HEAD)。
# child タスクは常に boid/<id8> branch を新規作成する。
# いずれも sandbox 内 clone 上で解決される — host 側に git worktree は作らない。
worktree: true
