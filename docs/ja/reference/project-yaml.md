...
| `default_instructions` | map (role → Instruction) | (空) | タスク作成時の active instruction として `Task.Instructions` 配列に積まれる雛形 (慣例的に `main` のみ使用) |
...
`role` は文字列で、 タスク作成時に同じ key の Instruction が `Task.Instructions` 配列に append されます。 慣例として `main` を使い、 これが executing で agent に渡される最初の active instruction になります。

reopen 時は `boid task reopen <id> --message "..."` で新しい Instruction を append し、 配列の最後の要素 (= 直近の active instruction) が agent に渡されます。 `consumer` / `model` / `interactive` は前回 active から継承されます。
