...
| `worktree` | bool | `false` | `true` にすると、このプロジェクト内の **executor** タスクごとに専用の git worktree を作る。 supervisor タスクの worktree 割り当ては `base_branch` の 3-case 分類 (後述) で決まる: case 1 (`base_branch` が HEAD と一致、または省略) は project root で走り、 case 2/3 (`base_branch` が HEAD と異なる) は readonly のまま worktree が割り当てられる |
| `base_branch` | string | リポジトリの既定ブランチ | executor と、 動的解決を必要とする supervisor (case 2/3) の worktree のベースブランチ。 `${TASK_REMOTE_ID}` / `${current_branch}` の展開をサポート (後述 [動的 base_branch](#動的-base_branch)) |
...
| `worktree` | project トップの `worktree:` と `base_branch` の 3-case 分類から決まる。 executor は project トップ `worktree: true` の時のみ worktree あり。 supervisor は case 1 (`base_branch` = HEAD または省略) で worktree なし、 case 2/3 で readonly な worktree あり |
...
- `${current_branch}` — worktree を作る瞬間に project リポジトリの daemon の HEAD ブランチ

`base_branch` を省略すると、 executor worktree は daemon の現 HEAD ブランチから切られ (= `${current_branch}` と同じ挙動)、 supervisor は case 1 として project root 上で走ります。 エンドツーエンドの例 ([ワークフロー 3](../../workflows.md#workflow-3--1-supervisor-1-pr) は supervisor の動的 `base_branch` の用例) は [docs/workflows.md](../../workflows.md) を参照。
...
# supervisor タスクの worktree 割り当ては base_branch の 3-case 分類で決まる
# (case 1 は project root、 case 2/3 は readonly worktree)。
worktree: true
