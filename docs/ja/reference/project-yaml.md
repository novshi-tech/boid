...
# project トップの `worktree:` は branch-policy-simplification Phase 2 (v0.0.12)
# で撤去済み。 root / child を問わず全タスクが sandbox 内 clone 上で
# base_branch を直接 checkout する — host 側に git worktree は作らない。
# 既存の `worktree:` 行は BC のため silent ignore される。
