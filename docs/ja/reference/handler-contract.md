...
書ける trait は `artifact` / `tasks` / `verification` / `instructions` / `lifecycle` 等で、何が許されるかは [`kit.yaml`](../kit-authoring/overview.md) の `traits.produces` 宣言で決まります。 trait の意味は [概念 / payload と trait](../guide/concepts.md#payload-と-trait) を参照。
