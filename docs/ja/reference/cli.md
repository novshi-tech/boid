...
| `boid task reopen <id>` | done のタスクを reworking に戻す (auto-merge コンフリクト時など) |
...
| コマンド | 役割 |
|---|---|
| `boid task findings <id>` | `verification.findings` をテーブル整形 |
...
主な `<action-type>`: `start` / `done` / `reopen` / `abort`。詳細は [状態機械 / 手動遷移](../guide/state-machine.md#手動遷移) を参照。
