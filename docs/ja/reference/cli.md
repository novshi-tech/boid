...
daemon が止まっているときに以下のコマンドを呼ぶと、自動で `boid start` が実行されます。自動起動をスキップする例外コマンドは `start` / `stop` / `gc` / `check` / `init` / `fetch` / `web set-url` / `web set-addr` です。手動で起動・停止する必要はありません。
...
| `boid init [DIR]` | **(廃止)** 廃止ガイダンスを表示。`boid kit init` / `boid project init|add` / `boid workspace configure` の 3 段を使ってください。詳細は [オンボーディング](../guide/onboarding.md) を参照 |
