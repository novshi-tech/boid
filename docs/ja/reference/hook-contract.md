...
[Kit 作者向け 概要](../kit-authoring/overview.md) には抜粋を載せていますが、 このページでは入力 (stdin、 環境変数、 作業ディレクトリ)、 出力 (`boid task update --payload-patch`、 stdout、 stderr)、 終了コード、 データ構造の正規仕様を網羅します。
...
### 経路 1: `boid task update --payload-patch` (推奨)
...
`@-` は curl と同じ規約で stdin から読む指定です（`@<path>` でファイル、裸の値でインラインも可）。 broker RPC 経由で即座に task の payload にマージされ、 hook プロセスの正常終了を待ちません。 中身は payload patch の**本体そのもの**で、 経路 2 のような `payload_patch` キーによるラップは不要です。

過去 (Phase 6 PR8 以前) は `$HOME/.boid/output/payload_patch.json` にファイルを書き出す経路が推奨でしたが、 ワークスペース共有の `$HOME` 上に置かれた well-known path であるため並行 job 間の隔離のために追加の tmpfs マウントが必要という負債があり、 このファイル経由の出力は撤廃されました。 `$HOME/.boid/output/` に書き込んでも `boid` 本体には読み取られません。

### 経路 2: stdout (フォールバック)

stdout に書いた JSON が payload patch として扱われます。 1 行で出しても複数行で出しても構いませんが、有効な JSON 1 件であること。 経路 1 と異なり、 トップレベルは `payload_patch` キーでラップします。

```bash
echo '{"payload_patch":{"artifact":{"result":"ok"}}}'
```

新規に書く hook では経路 1 (`boid task update --payload-patch`) を推奨します。 複雑な agent 系 hook (`claude-code` など) は agent の stdout に副次的な出力を含むため、 RPC 経路で誤認を避けます。 経路 2 はジョブ完了時に一度だけ評価されるのに対し、 経路 1 は呼んだ時点で即座に適用されます。

### payload patch の構造

経路 1 (`--payload-patch`) は payload patch の本体そのものをそのまま渡します:
...
経路 2 (stdout) はトップレベルが必ず `payload_patch` キーで、 その下が現在の payload にマージされます:
...
どちらの経路も同じマージロジック (`orchestrator.MergePayloadPatch`) を通ります。 hook が書ける trait は実質 `artifact` のみで、 何が許されるかは [`project.yaml`](project-yaml.md) の `task_behaviors.<name>.hooks[].traits.produces` 宣言で絞られます (kit は hook を提供しないため `kit.yaml` にこの宣言はありません)。 trait の意味は [概念 / payload と trait](../guide/concepts.md#payload-と-trait) を参照。
...
非 0 で終わった場合でも、 経路 1 (`--payload-patch`) を呼んでいればその時点でマージ済みです。 経路 2 (stdout) の場合も出力があればマージは試みられます。

## agent 用の追加コンテキスト

`kind: agent` で宣言した hook は、 instruction routing の対象になります。 routing 済みの instruction は `boid task instructions` 経由で参照します — 環境変数のミラーは無く、これが唯一の経路です。 たとえば claude-code kit の hook は `instructions.main` をこの RPC の出力から読んで agent への message として組み立てます。
