...
`boid` の payload で扱える trait は次の 2 つです。 状態機械の自動遷移は payload trait を直接見ず、 hook の終了 (`boid job done`) と gate の exit code だけで駆動されます。

| Trait | 書き込み可能 | 内容 |
|---|---|---|
| `artifact` | hook / gate が produce | 実装系タスクが残す成果物 (commit / PR URL / 変更ファイル等) を格納する自由形マップ |
| `tasks` | hook / gate が produce | 計画系タスクが生成したサブタスク配列 |
...
`artifact` と対称な役割を持ち、計画系のタスク (例: `plan` behavior) が使います。値はサブタスクの配列です (フォーマットは [boid-plan SKILL](../../../internal/skills/data/boid-plan/SKILL.md) を参照)。
