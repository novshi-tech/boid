...
セッションや対話型 exec ジョブでは、プログラムが
出力する端末タイトル（OSC 0 / OSC 2）をジョブの表示名に使います。ブラウザを
閉じていても daemon が取得・保存します。実行中のジョブ詳細画面では、端末を
再接続せずにタイトルだけを2秒ごとに更新します。

名前を明示したジョブでも端末タイトルを優先し、端末タイトルがない場合は
設定された名前または自動生成名を表示します。ジョブの名前変更は、この代替名を
変更するもので、端末タイトルの表示を妨げません。取得済みの端末タイトルがある
既存ジョブにも、この表示順序が適用されます。
