...
`WX7K-4QJP` (英数 8 桁にハイフン)。単回使用、 5 分有効、 IP あたり 5 回 / 5 分のレート制限 (超えると 15 分ロック)。

入力は正規化されるので、 **ハイフンの有無・大文字小文字・前後や途中の空白・全角文字は問いません** (`wx7k4qjp` でも通ります)。 字形が紛らわしい `O` `0` `I` `1` はコードに使われません。

QR / マジックリンクを開いた場合は確認ページが出るので、 「このデバイスを登録」を押してください。 リンクを開いただけではコードは消費されないため、 ブラウザのプリロードや QR スキャナのリンクプレビューにコードを焼かれることはありません。

失敗したときはログイン画面が理由を出し分けます:

| 表示 | 意味 | 対処 |
|---|---|---|
| 有効期限が切れています | 発行から 5 分超過 | `boid web pair` で再発行 |
| 使用済みです | 既に 1 回使われた | `boid web pair` で再発行 |
| 見つかりません | コードが違う | 入力を確認して再入力 |
...
名前を明示せずに開始した新しいセッションや対話型 exec ジョブでは、プログラムが
出力する端末タイトル（OSC 0 / OSC 2）をジョブの表示名に使います。ブラウザを
閉じていても daemon が取得・保存します。実行中のジョブ詳細画面では、端末を
再接続せずにタイトルだけを2秒ごとに更新します。

明示した名前を端末タイトルより優先し、端末タイトルがない場合は自動生成名を
表示します。端末タイトルへの追従に戻すには、ジョブの名前変更ボタンで名前を
空にして保存してください。既存ジョブは名前の設定元を判別できないため、
アップグレード後も従来の名前を保持します。
