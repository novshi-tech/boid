...
この判定の前に daemon は project の work_dir で `git fetch origin` を実行するため、 この検証はローカルリポジトリの状態ではなく実質的に **origin** の状態に対して行われます — 実際に `origin` に push 済みの release commit だけが見つかります。 `notify --done` を呼ぶ前に必ず push してください。
